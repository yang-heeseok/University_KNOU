...
5. 🟡 **BERT 이후 트렌드**: BERT를 기점으로 **경량화** 방향(ALBERT, ELECTRA)과 **성능 극대화** 방향(RoBERTa, XLNet, T5, GPT-3)의 두 축으로 발전하고 있다.
...
| **Masked LM (MLM)** | 입력 토큰의 15%를 `[MASK]`로 치환 후 원래 토큰 예측. 양방향 문맥 활용 가능 (→← BERT 핵심) | `[MASK]`, 양방향, 사전 학습 |
...
| **마스크드 언어 모형** | Masked Language Model (MLM) | $P(w_{mask}\mid context)$ | 입력의 15%를 `[MASK]`로 치환하고 양방향 문맥으로 복원하는 사전 학습 기법 |
