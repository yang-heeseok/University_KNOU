...
# Python: DecisionTreeClassifier() → export_text() → max_depth 조절 → predict()
...
# 3단계: 가지치기 (R의 prune에 해당 - max_depth 제한)
clf_pruned = DecisionTreeClassifier(max_depth=3, random_state=42)
clf_pruned.fit(X_train, y_train)
print("=== 가지치기 후 (Pruned Tree, max_depth=3) ===")
...
> 테스트 정확도: 0.8833
>
> === 가지치기 후 (Pruned Tree, max_depth=3) ===
> 깊이: 3, 리프 수: 7
> 훈련 정확도: 0.9357
> 테스트 정확도: 0.9167
> ```
> **해석**: 가지치기 전 모형은 훈련 정확도 100%이지만 테스트 정확도가 낮아 과대적합 경향이 있다. 가지치기(max_depth=3) 후 훈련 정확도는 소폭 하락하나 테스트 정확도가 향상되어 일반화 성능이 개선된다. 이는 R의 prune 함수가 수행하는 역할과 동일한 효과이다.
...
3. **나무 구축**: `rpart(formula, data, control)`로 의사결정나무를 적합한다.
...
| **시각화** | `plot()` + `text()` | `export_text()`, `plot_tree()` | |
| **가지치기** | `prune()` | `max_depth` 등 파라미터 | sklearn은 사후 가지치기 미제공 |
...
- **Good Practice (좋은 예시)**
  ...
  tree <- rpart(y ~ ., data = train, control = ctrl)
  plot(tree); text(tree, use.n = TRUE)
  printcp(tree)  # cp 테이블 확인
  tree_pruned <- prune(tree, cp = tree$cptable[which.min(tree$cptable[,"xerror"]), "CP"])
  pred <- predict(tree_pruned, newdata = test)
  ```
- **설명**: rpart.control로 분할 조건을 제어하고, printcp로 최적 cp를 확인한 뒤 prune으로 가지치기하는 것이 과대적합을 방지하고 해석 가능한 모형을 얻는 바람직한 방법이다.
