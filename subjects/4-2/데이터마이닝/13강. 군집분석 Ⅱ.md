...
4. 🔴 **kmeans 함수**: 비계층적 군집분석 방법인 K-평균 군집화를 수행하는 함수. 초기값을 지정해 주어야 한다.
...
| :--- | :--- | :--- | :--- | :--- |
| `dist()` | 거리 계산 | 데이터 행렬 | 하삼각 거리행렬 | base |
| `hclust()` | 응집분석 (계층적) | 거리행렬 | 덴드로그램 object | base |
| `diana()` | 분할분석 (계층적) | 데이터/거리행렬 | 덴드로그램 object | cluster |
| `kmeans()` | K-평균 (비계층적) | 데이터 행렬 | cluster, centers, withinss | base |
| `cutree()` | 군집 할당 | hclust/diana 결과 | 개체별 군집 id 벡터 | base |
...
> **정답: ③ diana**
> **해설**: `diana()`는 `cluster` 패키지에 포함된 함수이다. `hclust`, `cutree`, `kmeans`는 R base에 포함되어 별도 패키지 설치가 불필요하다.
...
# === 1. 비유사성행렬 (기출 데이터) ===
# 하삼각 행렬 재구성
D_condensed = np.array([
    1.0,              # d(2,1)
    5.0, 4.0,         # d(3,1), d(3,2)
    8.0, 7.0, 3.0,    # d(4,1), d(4,2), d(4,3)
    10.5, 9.5, 5.5, 2.5  # d(5,1), d(5,2), d(5,3), d(5,4)
])
...
>   단계3: [2, 5] 거리=3.0 원소수=3    → {3,4,5} 생성
>   단계4: [5, 6] 거리=5.0 원소수=5    → 전체 하나로
>
> === 완전연결법: {1,2}와 {3}의 거리 ===
