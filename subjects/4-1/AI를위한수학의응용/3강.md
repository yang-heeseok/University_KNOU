...
> **해설**: $W \in \mathbb{R}^{4 \times 3}$은 정사각이 아니라 직접적인 역행렬이 존재하지 않는다. 풀열랭크(column rank = 3)면 입력은 단사(injective)로 보존되지만, 정사각이 아니므로 일반적인 의미의 역행렬이 아니라 **의사역행렬** $W^+ = (W^T W)^{-1} W^T$이 필요하다. 단, 이 공식은 $W$가 **열 풀랭크(column full rank)** 일 때만 유효하다 — 열이 선형종속이면 $W^TW$ 가 가역이 아니어서 이 형태가 정의되지 않으며, 일반적으로 무어–펜로즈 의사역행렬은 **SVD** $W=U\Sigma V^\top$ 로부터 $W^+=V\Sigma^+U^\top$ 로 정의된다($\Sigma^+$ 는 0이 아닌 특이값의 역수를 취해 전치). ①②③은 모두 옳다(특히 ③은 비선형 활성화의 본질적 필요성을 보여줌). (근거: https://en.wikipedia.org/wiki/Moore%E2%80%93Penrose_inverse )
>
> **🟢 보강 — 조건수 제곱 $\kappa(X^\top X)=\kappa(X)^2$**: 최소제곱 정규방정식 $X^\top X\hat{\boldsymbol\beta}=X^\top\mathbf{y}$ 를 풀 때 $X^\top X$ 를 직접 만들어 역행렬을 구하면, 그 조건수가 원래 $X$ 의 조건수의 **제곱**으로 커진다($\kappa(X^\top X)=\kappa(X)^2$). 조건수가 클수록 수치 오차가 증폭되므로 제곱은 정밀도 손실을 대략 두 배(자릿수 기준)로 키운다 → 그래서 실무에서는 $X^\top X$ 를 명시적으로 형성하지 않고 **QR 분해나 SVD**로 최소제곱을 푸는 것이 권장된다. (근거: https://en.wikipedia.org/wiki/Condition_number )
...
| **의사역행렬** | Moore-Penrose Pseudoinverse | $A^+ = (A^T A)^{-1} A^T$ (열 풀랭크 시) | 비정사각/특이 행렬의 일반화 역행렬. 일반적 경우는 SVD로 계산 |
...
- **행렬 곱의 순서와 계산량(연쇄 행렬 곱)**: $(AB)C$ 와 $A(BC)$ 는 결과가 같지만(결합법칙) 계산량은 크게 다를 수 있다. 예컨대 $A$가 $1\times 1000$, $B$가 $1000\times 1000$, $C$가 $1000\times 1$ 일 때, 왼쪽부터 $(AB)C$ 로 계산하면 $AB$($1\times 1000$)에 약 $10^6$ 번, 오른쪽부터 $A(BC)$ 로 계산하면 $BC$($1000\times 1$)에 약 $10^6$ 번이 들지만, 행렬 크기 배치를 바꾸면 곱셈 순서 선택이 수십~수백 배의 연산량 차이를 만든다(연쇄 행렬 곱 최적화). 딥러닝 컴파일러는 이 순서를 자동으로 정한다.
- **어텐션은 결국 내적의 묶음**: 트랜스포머의 핵심 $\text{softmax}\!\left(\dfrac{QK^\top}{\sqrt{d_k}}\right)V$ 에서 $QK^\top$ 은 모든 쿼리-키 쌍의 **내적**(2강 개념)을 한 번의 행렬곱으로 동시에 계산한 유사도 행렬이다. $\sqrt{d_k}$ 로 나누는 것은 차원 $d_k$ 가 커질수록 내적의 분산이 커져 softmax가 포화(gradient 소실)되는 것을 막는 스케일링이다. "행렬 곱 = 한 묶음의 내적"이라는 관점에서 벡터(2강)와 행렬(3강)이 하나로 이어진다.
- **저차원 근사와 경량화(LoRA)**: 큰 가중치 행렬 $W$ ($d\times d$) 를 두 개의 가는 행렬 곱 $W \approx BA$ ($B$는 $d\times r$, $A$는 $r\times d$, $r\ll d$)로 근사하면 파라미터를 $d^2$ 에서 $2dr$ 로 줄일 수 있다. LLM 미세조정 기법 **LoRA**가 정확히 이 저랭크(low-rank) 아이디어를 사용한다 — 랭크가 모델 압축의 언어가 되는 셈이다.
- **역행렬을 직접 구하지 마라**: $\mathbf{x}=A^{-1}\mathbf{b}$ 를 구할 때 $A^{-1}$ 을 명시적으로 계산하는 것은 거의 항상 나쁜 선택이다(느리고 수치적으로 불안정). 대신 $A\mathbf{x}=\mathbf{b}$ 를 `solve`(LU 분해)나 `lstsq`(SVD)로 푼다. "수학적으로 존재한다"와 "수치적으로 안전하게 계산한다"는 별개의 문제임을 기억하자.
...
- Deisenroth, Faisal, Ong, *Mathematics for Machine Learning*, Ch. 2 Linear Algebra (행렬·선형변환·랭크) — https://mml-book.github.io
- Gilbert Strang, *MIT 18.06 Linear Algebra* (OpenCourseWare) — https://ocw.mit.edu/courses/18-06-linear-algebra-spring-2010/
- 3Blue1Brown, *Essence of Linear Algebra* — Linear transformations and matrices, Matrix multiplication as composition — https://www.3blue1brown.com/topics/linear-algebra
- Vaswani et al. (2017), *Attention Is All You Need* (어텐션의 $QK^\top/\sqrt{d_k}$ 스케일링 확인) — https://arxiv.org/abs/1706.03762
- Wikipedia, "Determinant" ($\det(cA)=c^n\det(A)$, $\det(AB)=\det A\det B$, $\det(A^\top)=\det A$ 확인) — https://en.wikipedia.org/wiki/Determinant
- Wikipedia, "Rouché–Capelli theorem" (랭크에 의한 해 존재·유일성 조건 확인) — https://en.wikipedia.org/wiki/Rouch%C3%A9%E2%80%93Capelli_theorem
- Wikipedia, "Moore–Penrose inverse" (열 풀랭크 시 $A^+=(A^\top A)^{-1}A^\top$ 확인) — https://en.wikipedia.org/wiki/Moore%E2%80%93Penrose_inverse
