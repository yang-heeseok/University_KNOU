# 11. AI를위한수학의응용: 다변량 확률분포(2) — 모멘트, 공분산, 조건부 모멘트
...
> **AI 적용**: 선형회귀의 OLS 추정량 $\hat{\beta} = \mathrm{Cov}(X,Y)/\mathrm{Var}(X)$, PCA의 공분산 행렬 고유분해, Kalman 필터의 갱신 단계 등에서 사용.
...
3. **편향-분산 분해와의 연결**: 전체 분산의 법칙은 예측오차의 편향-분산 분해와 같은 대수 구조를 가진다. 둘 다 "교차항이 직교성으로 소거된다"는 동일한 아이디어에서 나온다 — 이 직교성이 통계학과 ML 전반을 관통하는 핵심 원리임을 음미해보자.
