...
- [ ] **필수**: 고윳값·고유벡터 분해 (대각화·스펙트럼 분해) — 이 강의 PCA 도출의 핵심 도구
- [ ] **권장**: 결합·주변·조건부 확률분포, 공분산과 상관계수의 정의
- [ ] **권장**: 라그랑주 승수법(주성분 분석의 최적화 도출 시 사용)
- **관련 단원**: [12강. 다변량 확률분포(3) — 독립·조건부독립·변수변환](./12강.md), [11강. 다변량 확률분포(2) — 공분산·상관·조건부 기댓값](./11강.md)
...
$$
\mathbf{X}\sim\mathcal{N}_p(\boldsymbol{\mu},\Sigma),\ \Sigma_{ij}=0 \iff X_i \perp X_j \quad(\text{**결합정규(MVN)** 가정 하에서})
$$
> ⚠️ **단서**: 이 동치는 $(X_i,X_j)$ 가 **결합정규(jointly Gaussian)** 일 때만 성립한다. 각 주변이 정규라도 결합이 정규가 아니면 무상관이어도 독립이 아닐 수 있다(예: $X\sim N(0,1)$, $W$ 는 $\pm1$ 동전, $Y=WX$ 이면 $Y$ 도 표준정규이고 $\text{Cov}(X,Y)=0$ 이지만 $|X|=|Y|$ 라 독립이 아니며, 이때 $(X,Y)$ 는 결합정규가 아니다). 근거: [Normally distributed and uncorrelated does not imply independent — Wikipedia](https://en.wikipedia.org/wiki/Normally_distributed_and_uncorrelated_does_not_imply_independent)
> **직관적 해석**: 일반 분포에서는 "무상관 ≠ 독립"이지만, 다변량(결합) 정규분포는 예외적으로 두 조건이 동치다.
...
3.  **공분산이 아니라 정밀행렬을 보면?**: $\Sigma^{-1}$(정밀행렬, precision matrix)의 0 원소는 **조건부 독립**을 뜻한다(가우시안 그래피컬 모델). 정확히는 결합정규에서 $(\Sigma^{-1})_{ij}=0 \iff X_i \perp\!\!\!\perp X_j \mid \mathbf{X}_{\setminus\{i,j\}}$, 즉 **나머지 모든 변수를 조건부로 줄 때 $i,j$ 가 독립**이라는 뜻이다. 공분산 $\Sigma_{ij}=0$ 은 (정리3에 따라) 두 변수의 **주변** 독립을, 정밀행렬의 0 원소는 **조건부** 독립을 표현한다는 점이 핵심 차이다. 그래서 변수 간 "직접적" 연결 구조(간접 상관을 제거한 그래프의 엣지)를 알고 싶으면 공분산이 아니라 정밀행렬을 봐야 하며, 고차원에서 표본 $\hat\Sigma$ 가 가역이 아니거나 잡음이 많을 때는 $\ell_1$ 벌점으로 정밀행렬을 **희소하게** 추정한다 — 이것이 **Graphical Lasso**: $\hat\Theta=\arg\max_{\Theta\succ0}\big(\log\det\Theta-\text{tr}(\hat\Sigma\Theta)-\rho\|\Theta\|_1\big)$. 0 이 된 $\Theta_{ij}$ 가 곧 "엣지 없음(조건부 독립)"이 되어 가우시안 그래피컬 모델의 구조를 학습한다. 마할라노비스 거리에 $\Sigma^{-1}$ 이 등장하는 것도 같은 맥락 — 거리 계산이 곧 변수 간 조건부 관계를 반영하는 것이다. 근거: [Graphical lasso — Wikipedia](https://en.wikipedia.org/wiki/Graphical_lasso)
