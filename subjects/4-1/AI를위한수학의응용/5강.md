...
> **실행 결과 (예시)**:
> ```
> U shape: (4, 4)   Σ(특이값): [6.32455532 3.16227766 2.        ]   V^T shape: (3, 3)
...
> **실행 결과 (예시)**:
> ```
> 상위 10개 특이값: [119.924 108.094  90.55   78.492  71.474   1.916   1.805   1.779   1.733   1.7  ]
> k=  1: Frobenius 오차= 176.758, 저장량=  2.0%
> k=  5: Frobenius 오차=   9.493, 저장량= 10.1%
> k= 10: Frobenius 오차=   8.610, 저장량= 20.1%
> k= 50: Frobenius 오차=   3.356, 저장량=100.5%
> ```
> **해석**: 신호의 실제 랭크가 5이므로 $k=5$ 이후 오차가 거의 감소하지 않는다(나머지는 노이즈). 큰 특이값 5개와 작은 특이값 사이에 *큰 갭(elbow)* 이 보이고, 이것이 적절한 절단 차수의 기준이 된다.
>
> **왜 $k=5$에서 갭이 큰가 — Eckart–Young 스펙트럴 노름 버전으로 정량 설명**: 프로베니우스 노름이 아니라 *스펙트럴 노름(연산자 2-노름)* 으로 보면 최적 저랭크 근사의 오차가 **다음 특이값 하나로** 정확히 결정된다. 즉 Eckart–Young–Mirsky 정리는 $\|A - A_k\|_2 = \sigma_{k+1}$을 보장한다(2-노름 버전). 위 실행값에서 신호 5개의 특이값은 $\sigma_1{\approx}119.9,\dots,\sigma_5{\approx}71.5$로 모두 70 이상인 반면, $\sigma_6{\approx}1.92$로 한 자릿수로 급락한다. 따라서 $k=4$까지는 잔여 오차 $\sigma_{k+1}\ge 71.5$로 "아직 신호를 버리는" 큰 손실이 남지만, $k=5$를 택하는 순간 잔여 오차가 $\sigma_6\approx1.92$로 **약 37배 급감**한다 — 이것이 elbow의 정체다. $\sigma_5/\sigma_6 \approx 37$이라는 *특이값 갭(spectral gap)* 이 클수록 신호 부분공간과 잡음 부분공간이 깨끗하게 분리되어, "$k=5$ 절단 = 신호만 남기고 잡음 제거(디노이징)"가 수치적으로 정당화된다. 프로베니우스 오차표에서 $k=5$의 $9.493$이 잔여 잡음 특이값들의 제곱합 $\sqrt{\sum_{i\ge6}\sigma_i^2}$에 해당하며, $k=10,50$으로 더 키워도 잡음만 일부 흡수할 뿐 거의 줄지 않는다. 근거: https://en.wikipedia.org/wiki/Low-rank_approximation
