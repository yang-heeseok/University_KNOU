...
| **삼각부등식** | $\|\mathbf{u}+\mathbf{v}\|\le\|\mathbf{u}\|+\|\mathbf{v}\|$ | 노름·거리함수의 핵심 공리(우회보다 직선이 짧다) |
...
$$
> **직관적 해석**: "한 변의 길이는 나머지 두 변의 길이 합을 넘을 수 없다" — 두 점을 잇는 최단 경로는 직선이라는 사실의 벡터 표현이다. 이로부터 거리함수 $d(\mathbf{u},\mathbf{v})=\|\mathbf{u}-\mathbf{v}\|$ 에 대해 $d(\mathbf{u},\mathbf{w})\le d(\mathbf{u},\mathbf{v})+d(\mathbf{v},\mathbf{w})$ 가 따라 나온다.
> **유도**: $\|\mathbf{u}+\mathbf{v}\|^2=\|\mathbf{u}\|^2+2(\mathbf{u}\cdot\mathbf{v})+\|\mathbf{v}\|^2\le\|\mathbf{u}\|^2+2\|\mathbf{u}\|\|\mathbf{v}\|+\|\mathbf{v}\|^2=(\|\mathbf{u}\|+\|\mathbf{v}\|)^2$. 중간 단계에서 $\mathbf{u}\cdot\mathbf{v}\le|\mathbf{u}\cdot\mathbf{v}|\le\|\mathbf{u}\|\|\mathbf{v}\|$ 라는 **코시–슈바르츠 부등식**을 사용한다. 즉 삼각부등식은 코시–슈바르츠에서 곧장 유도된다.
> **AI 적용**: 노름이 "거리(metric)"가 되기 위한 세 공리(비음수성·대칭성·삼각부등식) 중 핵심으로, k-NN·클러스터링의 거리 비교, 손실함수의 정규화 항 설계, 임베딩 공간의 기하 보장에 쓰인다. (근거: https://en.wikipedia.org/wiki/Triangle_inequality )
...
7. ①
8. ③
9. $d \approx 3.16\,(=\sqrt{10})$, $\cos\theta \approx 0.861$ — 척도 비교는 해설 참조
10. 풀이형 정답 (해설 참조)
...
>
> **🟢 심화 유도 (판별식 방법, 내적공간 일반으로 확장)**: $\cos\theta$ 정의에 의존하지 않고 **내적의 양반정부호성**만으로 코시–슈바르츠를 증명할 수 있어, $\mathbb{R}^n$ 뿐 아니라 함수공간 $\langle f,g\rangle=\int f g\,dx$ 같은 임의의 내적공간으로 확장된다.
> 1) $\mathbf{v}\ne\mathbf{0}$ 이라 두고, 실수 $t$ 에 대한 함수 $f(t)=\|\mathbf{u}+t\mathbf{v}\|^2$ 를 생각한다. 노름의 제곱이므로 모든 $t$ 에서 $f(t)\ge 0$.
> 2) 내적의 쌍선형성으로 전개하면 $f(t)=\langle\mathbf{u}+t\mathbf{v},\,\mathbf{u}+t\mathbf{v}\rangle=\|\mathbf{v}\|^2\,t^2+2(\mathbf{u}\cdot\mathbf{v})\,t+\|\mathbf{u}\|^2$ 로, $t$ 에 대한 **아래로 볼록한 2차식**이다($\|\mathbf{v}\|^2>0$).
> 3) 이 2차식이 모든 실수 $t$ 에서 음이 되지 않으려면 실근을 많아야 하나 가져야 하므로 **판별식이 0 이하**여야 한다:
> $$D=(2\,\mathbf{u}\cdot\mathbf{v})^2-4\|\mathbf{v}\|^2\|\mathbf{u}\|^2\le 0.$$
> 4) 정리하면 $(\mathbf{u}\cdot\mathbf{v})^2\le\|\mathbf{u}\|^2\|\mathbf{v}\|^2$, 양변에 제곱근을 취해 $|\mathbf{u}\cdot\mathbf{v}|\le\|\mathbf{u}\|\|\mathbf{v}\|$ 를 얻는다.
> 5) 등호는 $D=0$, 즉 $f(t_0)=\|\mathbf{u}+t_0\mathbf{v}\|^2=0$ 인 $t_0$ 가 존재할 때 — 곧 $\mathbf{u}=-t_0\mathbf{v}$ 로 **두 벡터가 선형종속(평행)** 일 때만 성립한다. (근거: https://en.wikipedia.org/wiki/Cauchy%E2%80%93Schwarz_inequality )
