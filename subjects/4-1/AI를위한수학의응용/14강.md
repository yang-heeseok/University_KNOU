...
| **KL 발산** | $D_{KL}(p\|q) = \sum_i p_i \log\dfrac{p_i}{q_i} \ge 0$ | 분포 간 **비대칭 다이버전스**(거리 아님: 비대칭·삼각부등식 불성립) |
...
$$
이때 **절편 $b$ 의 명시식**은 (이차항 $-\tfrac12 x^\top\Sigma^{-1}x$ 가 두 클래스에서 상쇄되고 남는 상수항들로부터)
$$
b = -\tfrac{1}{2}\big(\mu_1^\top\Sigma^{-1}\mu_1 - \mu_0^\top\Sigma^{-1}\mu_0\big) + \log\frac{\pi_1}{\pi_0}
$$
이다. 즉 $w$ 는 두 평균의 (공분산으로 화이트닝된) 차이 방향을, $b$ 는 두 평균의 마할라노비스 노름 차이와 **사전확률 비 $\log(\pi_1/\pi_0)$** 를 함께 담아 결정 임계값을 정한다(사전확률이 같으면 마지막 로그항은 0). 근거: [Linear discriminant analysis — Wikipedia](https://en.wikipedia.org/wiki/Linear_discriminant_analysis)
...
> **결론**: 두 관점은 정확히 같은 판별 **방향**을 산출한다 ($S_W \leftrightarrow \Sigma$).
> **각주 — "같다"의 정확한 의미**: 피셔 해 $w_{\text{Fisher}}\propto S_W^{-1}(\mu_1-\mu_0)$ 와 통계적 해 $w_{\text{stat}}\propto \Sigma^{-1}(\mu_1-\mu_0)$ 는 표본에서 $S_W$ 가 (풀링된) $\Sigma$ 의 추정량이라는 점에서 **같은 방향**을 가리키는 추정량이다. 다만 피셔 기준 $J(w)$ 는 스케일 불변($J(cw)=J(w)$)이라 크기를 정하지 않으므로 두 해는 일반적으로 **방향은 같고 스케일(비례상수)만 다른** 추정량이다. 분류 결정경계 $w^\top x+b=0$ 자체는 $w$ 의 스케일에 불변이므로 같은 경계를 주지만, 로지스틱처럼 $w^\top x$ 의 **절대 크기**(로짓의 기울기)가 의미를 갖는 경우에는 통계적 해의 절대 스케일(공통 공분산 가우시안 가정에서 유도된 $\Sigma^{-1}(\mu_1-\mu_0)$)을 써야 한다.
...
2.  **포워드 KL vs 리버스 KL의 성격 차이**: $D_{KL}(p\|q)$(forward)는 $p$가 큰 곳을 $q$가 반드시 덮게 하여 **평균 추구(mean-seeking, 분포를 넓게)**, $D_{KL}(q\|p)$(reverse)는 $q$가 $p$의 한 봉우리에 몰리는 **모드 추구(mode-seeking)** 경향을 보인다. VAE는 reverse KL을 쓰기 때문에 생성물이 흐릿해지는 경향이, 변분추론의 근사 사후분포가 좁아지는 현상이 여기서 나온다. **RLHF로의 연결**: 인간 피드백 강화학습(RLHF/PPO)의 목적함수는 보상 $r(x)$ 최대화에 **KL 정규화 항** $-\beta\,D_{KL}\big(\pi_\theta(\cdot\mid x)\,\|\,\pi_{\text{ref}}(\cdot\mid x)\big)$ 를 더한 형태로, 학습 중인 정책 $\pi_\theta$ 가 사전학습된 기준 정책 $\pi_{\text{ref}}$ 에서 너무 멀어지지 않도록 묶는다. 여기서 쓰이는 것은 **reverse KL**($q=\pi_\theta$ 를 $p=\pi_{\text{ref}}$ 에 가깝게)이라 mode-seeking 성격을 띠어 보상이 높은 소수 모드로 정책이 몰리는 "보상 해킹/모드 붕괴" 경향과 직결되며, $\beta$ 는 이 쏠림을 억제하는 다이얼이다. 같은 KL 한 항이 분류 손실(forward), 변분추론(reverse), 정책 정규화(reverse)에서 서로 다른 역할로 재등장한다. 근거: [Kullback–Leibler divergence — Wikipedia](https://en.wikipedia.org/wiki/Kullback%E2%80%93Leibler_divergence)
