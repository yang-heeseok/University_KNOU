...
**5. ⭐⭐ [분석] 다음 중 "비상관이지만 독립이 아닌" 예시로 옳은 것은? (다중 선택 형식)[(해설)](#prob-5)** 📌
   ...
   > 다. $\theta\in\{-1,1\}$ 균등, $X=\theta,\ Y=\theta^2$
...
> - 다: $E[XY]=E[\theta\cdot\theta^2]=E[\theta^3]=0$, $E[X]=E[\theta]=0$ → $\text{Cov}=0$. 그러나 $Y=\theta^2=1$ (상수) 이므로 $Y$ 와 $X$ 는 어떤 의미에선 trivial하지만, "독립이 아닌 비상관" 예시로 자주 인용된다(혹은 보다 강건한 변형: $\theta\in\{-2,-1,1,2\}$).
