...
**6. 📌 ⭐⭐ [적용] `name` 컬럼에서 "김"으로 시작하고 끝에서 두 번째 글자가 "수"인 이름(예: 김민수아, 김수아 O, 김수 ✕)을 찾는 쿼리로 옳은 것은?[(해설)](#prob-6)**
...
> **보완**: BETWEEN A AND B는 `A <= col AND col <= B`와 완전히 동일한 닫힌 구간([A, B])이다. 날짜/시각 타입에서는 상한값 포함 여부에 주의가 필요하다. 예를 들어 `BETWEEN '2025-01-01' AND '2025-12-31'`은 '2025-12-31 23:59:59' 같은 시각 값을 포함하지 않으므로, DATETIME 컬럼에 대해 날짜 범위를 지정할 때는 `AND reg_date < '2026-01-01'` 형태로 작성하는 것이 더 안전하다.
