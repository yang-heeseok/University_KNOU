...
| **🟢 대용량 청크 읽기** | `pd.read_sql_query(sql, conn, chunksize=N)` | 대용량 테이블을 N행 단위 청크로 읽어 generator를 반환. 메모리 초과 방지용. |

### chunksize 사용 예시

```python
for chunk in pd.read_sql_query("SELECT * FROM 대용량테이블", conn, chunksize=10000):
    process(chunk)  # 청크 단위 처리
```
...
- **오개념 분석**
  ...
  - **오개념 4**: "pandas 2.0에서도 sqlite3.Connection을 to_sql/read_sql_query에 직접 전달하면 항상 정상 동작한다."
    - **분석**: pandas 2.0부터 순수 sqlite3.Connection 객체를 to_sql/read_sql_query에 직접 전달하면 FutureWarning이 발생할 수 있다. SQLAlchemy engine 사용이 권장된다.

**pandas 2.0+ SQLAlchemy 권장 패턴**:
```python
from sqlalchemy import create_engine
engine = create_engine("sqlite:///mydb.sqlite")
df.to_sql("테이블명", con=engine, if_exists="replace", index=False)
df2 = pd.read_sql_query("SELECT * FROM 테이블명", con=engine)
```
> 단, 학습용 인메모리 DB나 단순 실습에서는 sqlite3.Connection 직접 전달도 동작하므로, 현재 pandas 버전에 따라 선택한다.
