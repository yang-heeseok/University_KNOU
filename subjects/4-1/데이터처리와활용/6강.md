...
> **⚠️ SQLite 외래키 제약 주의사항**: SQLite는 기본적으로 외래키(FOREIGN KEY) 제약 검사가 **OFF** 상태이다. 외래키 무결성을 실제로 강제하려면 DB 연결 직후 반드시 `PRAGMA foreign_keys = ON;`을 실행해야 한다. 이 설정은 연결(connection) 단위로 적용되므로 매번 연결할 때마다 실행해야 한다. DB Browser에서는 *Edit → Preferences → General → "Enable foreign key support"* 체크로 동일한 효과를 얻는다.
>
> ```python
> conn = sqlite3.connect("School.db")
> conn.execute("PRAGMA foreign_keys = ON")  # 반드시 연결 직후 실행
> ```
