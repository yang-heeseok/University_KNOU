...
3. 🔴 **가중치 초깃값**: Sigmoid 활성화 함수에는 Xavier 초깃값 $N\!\left(0,\frac{2}{n_{in}+n_{out}}\right)$, ReLU에는 He 초깃값 $N\!\left(0,\frac{4}{n_{in}+n_{out}}\right)$을 사용한다.
...
| **He 초깃값** | ReLU용. $N\!\left(0,\frac{4}{n_{in}+n_{out}}\right)$ 분포로 초기화 | 히, Xavier의 2배 분산 |
...
**2. ⭐ [기억] 가중치 초깃값 선택**
> **정답: ③**
> **해설**: He 초깃값은 $N\!\left(0,\frac{4}{n_{in}+n_{out}}\right)$으로 ReLU 사용 시 적합하다. Xavier 초깃값 $N\!\left(0,\frac{2}{n_{in}+n_{out}}\right)$은 Sigmoid/tanh에 적합하다. 모든 가중치를 0으로 초기화하면 역전파 시 모든 가중치가 동일하게 갱신되어 학습 불가.
...
> <a id="ox-2"></a>
> **2. 정답: X**
> **해설**: Xavier 초깃값은 Sigmoid/tanh에 적합하다. ReLU에는 분산이 2배인 He 초깃값 $N\!\left(0,\frac{4}{n_{in}+n_{out}}\right)$이 적합하다.
...
he     = np.random.normal(0, np.sqrt(4 / (n_in + n_out)), (n_in, n_out))

print("\n=== 초깃값 표준편차 비교 ===")
print(f"Xavier 표준편차: {xavier.std():.6f}  (이론값: {np.sqrt(2/(n_in+n_out)):.6f})")
print(f"He     표준편차: {he.std():.6f}  (이론값: {np.sqrt(4/(n_in+n_out)):.6f})")
...
| **He 초깃값** | He Initialization | $N\!\left(0,\frac{4}{n_{in}+n_{out}}\right)$ | ReLU용. Xavier보다 분산 2배 |
