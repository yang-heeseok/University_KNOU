...
| **YOLOv1** | 1단계 | 그리드 분할 (7×7) | 분류+위치 동시 예측 | 매우 빠름 |
| **YOLOv3** | 1단계 | FPN 다중 스케일 | 앵커 박스 3종 × 3 스케일 | 빠름 + 정확도 개선 |
| **YOLOv8** | 1단계 | Anchor-free | 중심점 기반 직접 예측 | 최신 속도·정확도 균형 |
...
| **MS COCO** | **80** | 약 12만 | 7.5개 | 90만 객체, 가장 도전적 (정확히 80개 범주) |
...
> <a id="ox-6"></a>
> **6. 정답**: X
> **해설**: MS COCO는 정확히 **80개** 범주와 이미지당 약 7.5개 객체로 구성되어, PASCAL VOC(20개 범주, 2.4개 객체)보다 훨씬 크고 도전적인 데이터셋이다.
...
> **📌 심층 확장 — YOLO 계열 발전사와 속도·정밀도 트레이드오프**
>
> ### YOLOv1 → YOLOv3 → YOLOv8 진화
>
> **YOLOv1 (Redmon et al., 2016)**
>
> 이미지를 $S \times S = 7 \times 7$ 그리드로 분할하고 각 셀에서 B개의 바운딩 박스와 C개의 클래스 확률을 **단일 순전파**로 동시 예측한다. 출력 텐서는 $7 \times 7 \times (5B + C)$ 형태이다. 45fps의 실시간 추론이 가능하지만 작은 객체나 밀집 객체에 취약하다.
>
> **YOLOv3 (Redmon & Farhadi, 2018)**
>
> FPN(Feature Pyramid Network)을 도입하여 3가지 해상도의 특징 맵($13 \times 13$, $26 \times 26$, $52 \times 52$)에서 각각 예측을 수행한다. 각 스케일마다 3종의 앵커 박스를 사용하여 총 9종의 앵커로 다양한 크기의 객체를 포착한다. COCO 기준 mAP 33.0%에서 28.2 ms 추론.
>
> **YOLOv8 (Ultralytics, 2023)**
>
> 앵커 박스를 완전히 제거한 **앵커 프리(Anchor-free)** 설계를 채택한다. 분류 헤드와 회귀 헤드를 분리(Decoupled Head)하고 DFL(Distribution Focal Loss)로 바운딩 박스를 분포 형태로 예측한다. C2f(Cross Stage Partial with 2 bottlenecks) 블록으로 기울기 흐름을 개선했다.
>
> | 버전 | 연도 | 핵심 혁신 | 앵커 | COCO mAP |
> | :--- | :---: | :--- | :---: | :---: |
> | **YOLOv1** | 2016 | 7×7 그리드, 단일 예측 | 있음 | 63.4% (VOC) |
> | **YOLOv3** | 2018 | FPN 다중 스케일, Darknet-53 | 있음 | 33.0% |
> | **YOLOv8-m** | 2023 | Anchor-free, Decoupled Head | 없음 | 50.2% |
>
> **속도·정밀도 트레이드오프**
>
> ```
...
> ```
>
> YOLOv8은 n(nano)/s(small)/m(medium)/l(large)/x(extra-large) 5단계 모델을 제공하여 엣지 디바이스부터 서버까지 배포 환경에 맞게 선택할 수 있다.
>
> 출처: Redmon et al. (2016). You Only Look Once. *CVPR 2016*; Redmon & Farhadi (2018). YOLOv3. *arXiv:1804.02767*; Ultralytics (2023). YOLOv8.

---

> **📌 심층 확장 — 앵커 프리(Anchor-free) 탐지 방법**
>
> ### 앵커 박스의 한계와 앵커 프리 해결책
>
> **앵커 박스(Anchor Box) 방식의 문제점**
> - 앵커의 크기·비율을 데이터셋에 맞게 수동 설계해야 하는 **하이퍼파라미터 의존성**
> - 앵커 수가 많아질수록 연산량 증가 (Faster R-CNN: 이미지당 ~20,000개 앵커 후보)
> - 도메인 이전(Domain Transfer) 시 앵커 재설계 필요
>
> **FCOS (Tian et al., 2019): 픽셀별 직접 예측**
>
> 각 픽셀 위치에서 해당 픽셀이 속한 객체의 바운딩 박스 경계까지의 거리 $(l, t, r, b)$를 직접 예측한다. FPN의 각 스케일 레이어에서 객체 크기에 따라 예측 담당을 분리하여 중복 예측 문제를 완화한다.
>
> $$\text{출력}: (l, t, r, b, \text{centerness}, c_1, \ldots, c_K)$$
>
> **Centerness Score**: 픽셀이 객체 중심에 얼마나 가까운지를 나타내는 보조 예측값:
>
> $$\text{centerness} = \sqrt{\frac{\min(l, r)}{\max(l, r)} \cdot \frac{\min(t, b)}{\max(t, b)}}$$
>
> 객체 경계 근처의 저품질 예측을 억제하는 역할로, NMS 후처리의 부담을 줄인다.
>
> **CenterNet (Zhou et al., 2019): 중심점 히트맵**
>
> 각 객체를 **중심점(Keypoint)**으로 표현하고, 특징 맵에서 중심점의 가우시안 히트맵을 예측한다. 중심점이 탐지되면 해당 위치에서 너비·높이를 추가 예측한다.
>
> $$\text{출력: 히트맵 } \hat{Y} \in [0,1]^{H/R \times W/R \times C} + \text{크기 맵}$$
>
> NMS가 불필요하여 후처리가 단순하다. CornerNet(2018)의 코너 쌍 탐지보다 단순하고 빠르다.
>
> **앵커 기반 vs 앵커 프리 비교**
>
> | 구분 | 앵커 기반 (Faster R-CNN, YOLOv3) | 앵커 프리 (FCOS, CenterNet, YOLOv8) |
> | :--- | :--- | :--- |
> | **하이퍼파라미터** | 앵커 크기·비율 수동 설계 필요 | 없음 |
> | **연산 복잡도** | 앵커 수에 비례 | 픽셀 수에 비례 (더 단순) |
> | **도메인 이전** | 앵커 재설계 필요 | 불필요 |
> | **소형 객체** | 작은 앵커 추가 시 처리 가능 | FPN 상위 스케일에서 처리 |
> | **대표 모델** | Faster R-CNN, SSD, YOLOv3 | FCOS, CenterNet, YOLOv8, DETR |
>
> 출처: Tian et al. (2019). FCOS. *ICCV 2019*; Zhou et al. (2019). Objects as Points (CenterNet). *arXiv:1904.07850*.

---

> **📌 심층 확장 — Semantic / Instance / Panoptic Segmentation 비교**
>
> ### 세 가지 영상 분할 태스크의 개념과 차이
>
> | 태스크 | 목표 | 출력 | 같은 종류 객체 구분 | 배경 처리 | 대표 모델 |
> | :--- | :--- | :--- | :---: | :--- | :--- |
> | **Semantic** | 픽셀 → 클래스 레이블 | 클래스 맵 | X (모두 동일 레이블) | 클래스로 분류 | FCN, DeepLab |
> | **Instance** | 개별 객체 마스크 | 인스턴스 ID + 마스크 | O (각각 다른 ID) | 무시 | Mask R-CNN |
> | **Panoptic** | Semantic + Instance 통합 | 클래스 + 인스턴스 ID | O | stuff 클래스로 분류 | Panoptic FPN, Mask2Former |
>
> **직관적 비교 예시** (도로 장면 이미지):
>
> ```
> Semantic:  [도로][보행자][보행자][차량][하늘]  ← 보행자 2명 구분 불가
> Instance:  [보행자₁][보행자₂][차량₁]          ← 도로·하늘(배경) 무시
> Panoptic:  [도로₀][보행자₁][보행자₂][차량₁][하늘₀]  ← 모든 픽셀 처리
> ```
>
> **Panoptic Segmentation (Kirillov et al., CVPR 2019)**
>
> "stuff"(셀 수 없는 배경 영역: 도로, 하늘, 잔디)와 "things"(셀 수 있는 개별 객체: 사람, 차)를 통합 처리하는 태스크를 정의한다. 평가 지표로 **PQ(Panoptic Quality)**를 제안한다:
>
...
>
> PQ = SQ(분할 품질) × RQ(인식 품질)로 분해되어 두 측면을 독립적으로 진단할 수 있다.
>
> **DeepLab 계열 (Semantic Segmentation의 발전)**
>
> FCN이 픽셀별 분류의 기초를 세웠다면, DeepLab 시리즈는 **팽창 합성곱(Atrous/Dilated Convolution)**과 **CRF(Conditional Random Field)** 후처리로 경계 정밀도를 높였다.
>
> 팽창 합성곱은 필터 사이에 간격(dilation rate $r$)을 두어 receptive field를 $r$배 확장하면서도 파라미터 수는 동일하게 유지한다:
>
...
> import torch.nn as nn
>
> # 일반 합성곱 vs 팽창 합성곱 receptive field 비교
> conv_normal  = nn.Conv2d(64, 64, kernel_size=3, padding=1)           # RF: 3×3
> conv_dilated = nn.Conv2d(64, 64, kernel_size=3, padding=2, dilation=2)  # RF: 5×5 (파라미터 동일)
> conv_dilated6 = nn.Conv2d(64, 64, kernel_size=3, padding=6, dilation=6) # RF: 13×13
>
> # DeepLabv3의 ASPP(Atrous Spatial Pyramid Pooling):
> # dilation=1, 6, 12, 18을 병렬로 적용하여 다중 스케일 컨텍스트 포착
> ```
>
> 출처: Kirillov et al. (2019). Panoptic Segmentation. *CVPR 2019*; Long et al. (2015). FCN. *CVPR 2015*; Chen et al. (2018). DeepLabv3+. *ECCV 2018*.
...
1. **R-CNN의 문제**: 선택적 검색으로 추출한 2,000개 영역 각각에 CNN을 별도 적용 → 연산 중복 극심, 이미지당 **47초** 소요(VGG16 기준, Girshick 2014).
...
- Redmon et al. (2016) — You Only Look Once: Unified, Real-Time Object Detection (YOLOv1)
- Redmon & Farhadi (2018) — YOLOv3: An Incremental Improvement (arXiv:1804.02767)
- Ultralytics (2023) — YOLOv8: Anchor-free object detection
- Tian et al. (2019) — FCOS: Fully Convolutional One-Stage Object Detection (ICCV 2019)
- Zhou et al. (2019) — Objects as Points: CenterNet (arXiv:1904.07850)
- Kirillov et al. (2019) — Panoptic Segmentation (CVPR 2019)
