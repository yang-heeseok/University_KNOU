...
| **분할분석** | `diana()` (cluster pkg, 분할/top-down=divisive) | **직접 대응 함수 없음** (scikit-learn에는 분할군집 전용 API가 없어 재귀적 분할을 직접 구현해야 함) | ⚠️ `AgglomerativeClustering`은 응집(bottom-up)이므로 방향이 정반대 → 대응 불가 |
...
| **kmeans** | K-means | - | K-평균 군집화 수행 R 함수, `centers`에 **군집 수 K(정수) 또는 초기 중심**을 지정(K만 줘도 무작위 초기중심 자동 선택) (→←12강 K-평균) |
...
### 6. 혼합분포군집(EM/가우시안혼합, GMM)

K-평균이 각 개체를 하나의 군집에 **딱 하나로(hard assignment)** 배정하는 것과 달리, **혼합분포군집**은 데이터가 여러 확률분포(보통 정규분포)의 **혼합(mixture)** 에서 생성되었다고 보고, 각 개체가 각 군집에 속할 **확률**을 부여하는 **소프트 할당(soft assignment)** 방식이다.

- **가우시안혼합모형(GMM)**: 전체 분포를 $K$개의 정규분포의 가중합으로 모형화한다.
$$
p(x) = \sum_{k=1}^{K} \pi_k \, \mathcal{N}(x \mid \mu_k, \Sigma_k), \qquad \sum_{k=1}^{K}\pi_k = 1
$$
여기서 $\pi_k$는 혼합비율(군집 $k$의 사전확률), $\mu_k, \Sigma_k$는 군집 $k$의 평균·공분산이다.

- **책임도(responsibility)**: 개체 $x_i$가 군집 $k$에서 나왔을 사후확률 $\gamma_{ik}=P(k \mid x_i)$. 이것이 곧 "소프트 할당" 값이다.
$$
\gamma_{ik} = \frac{\pi_k\, \mathcal{N}(x_i \mid \mu_k, \Sigma_k)}{\sum_{j=1}^{K} \pi_j\, \mathcal{N}(x_i \mid \mu_j, \Sigma_j)}
$$

- **EM 알고리즘(기댓값-최대화)**: 모수 $(\pi_k,\mu_k,\Sigma_k)$를 모르므로 다음 두 단계를 수렴할 때까지 반복한다.
    1. **E-step (Expectation)**: 현재 모수로 모든 개체의 **책임도 $\gamma_{ik}$를 계산**한다.
    2. **M-step (Maximization)**: 책임도를 가중치로 사용해 **모수를 갱신**한다. 예) $\pi_k = \frac{1}{N}\sum_i \gamma_{ik}$, $\mu_k = \frac{\sum_i \gamma_{ik}x_i}{\sum_i \gamma_{ik}}$.

> **K-평균과의 관계**: K-평균은 GMM에서 모든 군집의 공분산이 같은 구형($\Sigma_k=\sigma^2 I$)이고 책임도가 0/1(하드 할당)인 **특수한 경우**로 볼 수 있다. 따라서 GMM은 타원형 군집과 소속 확률을 다룰 수 있다는 점에서 K-평균의 일반화이다. (R: `mclust::Mclust`, Python: `sklearn.mixture.GaussianMixture`)

### 7. K-평균 중심 갱신 손계산 예제 (1회 반복)

1차원 데이터 6개 $\{1, 2, 4, 7, 8, 10\}$을 $K=2$로 군집화한다고 하자.

1. **초기 중심 선택**: 초기 중심을 $c_1=2,\ c_2=8$로 잡는다(무작위 선택 가정).
2. **할당 단계(가까운 중심에 배정)**: 각 점을 더 가까운 중심에 배정한다.

| 점 $x$ | $\lvert x-c_1\rvert$ (=2) | $\lvert x-c_2\rvert$ (=8) | 배정 군집 |
| :--: | :--: | :--: | :--: |
| 1 | 1 | 7 | $C_1$ |
| 2 | 0 | 6 | $C_1$ |
| 4 | 2 | 4 | $C_1$ |
...
→ $C_1=\{1,2,4\}$, $C_2=\{7,8,10\}$.

3. **중심 재계산(평균)**: $c_1' = (1+2+4)/3 = 7/3 \approx 2.33$, $c_2' = (7+8+10)/3 = 25/3 \approx 8.33$.
4. **수렴 확인(2회차 할당)**: 새 중심 $c_1'\approx2.33,\ c_2'\approx8.33$으로 다시 할당하면 군집 구성이 $C_1=\{1,2,4\}$, $C_2=\{7,8,10\}$으로 **변하지 않는다**. 할당이 더 이상 바뀌지 않으므로 **수렴 완료**.

> **핵심 흐름**: K-평균은 **(초기중심) → 할당 → 평균 재계산 → 할당 …** 을 군집 배정이 바뀌지 않을 때까지 반복한다. 각 반복에서 군집 내 제곱합(`tot.withinss`)은 단조 감소하므로 반드시 수렴하지만, 초기중심에 따라 다른 국소최적에 도달할 수 있어 `nstart`로 여러 번 시도하는 것이 권장된다.

### 8. 실루엣 기반 자동 K 선택 (factoextra `fviz_nbclust`) — 2차 심층확장

앞 4절에서 "평균 실루엣이 최대인 K가 유력 후보"라고 했는데, R에서 이를 **자동으로** 찾아주는 표준 도구가 `factoextra::fviz_nbclust`이다. 평균 실루엣 폭(average silhouette width)이 가장 큰 K를 그래프에서 한눈에 골라준다.

```r
library(cluster)      # silhouette()
library(factoextra)   # fviz_nbclust()
data <- scale(iris[, 1:4])   # 변수 스케일 통일(거리 왜곡 방지)

# (a) 실루엣 기준 최적 K 자동 탐색: 평균 실루엣 폭이 최대인 K에 점선 표시
fviz_nbclust(data, kmeans, method = "silhouette") +
  labs(title = "Optimal number of clusters (Silhouette)")

# (b) 엘보우(군집내 제곱합 WSS) 기준 — 비교용
fviz_nbclust(data, kmeans, method = "wss") +
  labs(title = "Elbow method (WSS)")

# (c) 특정 K의 개체별 실루엣 폭 직접 확인
km <- kmeans(data, centers = 2, nstart = 25)
sil <- silhouette(km$cluster, dist(data))
mean(sil[, "sil_width"])   # 평균 실루엣 폭(이 값이 최대가 되는 K 선택)
fviz_silhouette(sil)       # 군집별 실루엣 막대그림
```

| 기준 | 함수/측도 | K 선택 규칙 | 특징 |
| :--- | :--- | :--- | :--- |
| **실루엣(Silhouette)** | `method="silhouette"`, `silhouette()` | 평균 실루엣 폭이 **최대**인 K | 군집 응집도·분리도 모두 반영, 자동 선택이 명확 |
| **엘보우(Elbow/WSS)** | `method="wss"`, `tot.withinss` | 기울기가 완만해지는 **꺾임점** | 꺾임점이 주관적·모호할 수 있음 |
| **갭 통계량(Gap)** | `method="gap_stat"`, `clusGap()` | 기댓값 대비 갭이 최대(1-SE 규칙) | 무작위 기준분포와 비교, 계산비용 큼 |

> **실루엣 폭 정의**: 개체 $i$에 대해 $s(i)=\dfrac{b(i)-a(i)}{\max(a(i),\,b(i))}$. 여기서 $a(i)$는 같은 군집 내 평균 거리, $b(i)$는 가장 가까운 **다른** 군집까지의 평균 거리. $s(i)$가 1에 가까울수록 잘 군집화된 것, 0 근처는 경계, 음수는 잘못 배정된 개체이다. **엘보우가 "WSS 감소가 둔해지는 점"이라는 주관적 판단에 의존**하는 반면, 실루엣은 평균값을 **최대화**하는 명확한 자동 규칙을 제공한다는 점이 핵심 차이다. (근거: Rousseeuw 1987; factoextra `fviz_nbclust` 문서)

### 9. K-means의 한계와 현대 대안 (K-means++ · DBSCAN/HDBSCAN · 스펙트럴) — 2차 심층확장

5절에서 K-means++ 와 DBSCAN을 짧게 언급했는데, 여기서 **왜 K-means가 실패하는지**와 현대 대안을 한 줄씩 정리한다.

- **K-means++ (초기중심 개선)**: 첫 중심은 무작위, 이후 중심은 **기존 중심에서 멀수록 높은 확률**로 뽑아 초기중심을 골고루 퍼뜨린다. 표준 K-means의 무작위 초기화보다 국소최적에 빠질 위험과 반복 횟수를 줄인다(sklearn `KMeans`의 기본값 `init="k-means++"`). R `kmeans`에는 내장돼 있지 않아 `nstart`를 키우거나 `flexclust::kcca`를 쓴다. (근거: Arthur & Vassilvitskii 2007)
- **K-means의 구조적 한계**: ① 군집이 **구형(등방성)·비슷한 크기**라고 가정 → 길쭉하거나 초승달·동심원 형태에서 실패. ② K를 **미리 지정**해야 함. ③ 평균 기반이라 **이상치에 민감**.

| 대안 | 핵심 아이디어 | K-means 대비 강점 | 약점 |
| :--- | :--- | :--- | :--- |
| **DBSCAN** | 밀도(`eps` 반경 내 `minPts`개 이상)로 핵심점을 잇대어 군집 형성 | K 불필요, **임의 형상** 군집, 이상치=잡음(noise) 자동 분리 | `eps`/`minPts` 민감, **밀도가 다른** 군집 동시 처리 곤란 |
| **HDBSCAN** | DBSCAN을 **계층화**하여 다양한 밀도에 적응, `eps` 없이 안정적 군집 추출 | 밀도 변화에 강건, `eps` 튜닝 불필요 | 계산 복잡도·구현 의존(R `dbscan::hdbscan`) |
| **스펙트럴(Spectral)** | 유사도 그래프의 **라플라시안 고유벡터** 공간에서 군집화 | 비볼록(non-convex)·얽힌 군집 분리 우수 | 유사도 행렬 고유분해로 **대용량에 비쌈**, K 필요 |

> **한 줄 비교**: **구형·등크기 군집이면 K-means(+nstart/k-means++)**, **임의 형상·이상치 분리가 필요하면 DBSCAN/HDBSCAN**, **얽혀 있는 비볼록 군집이면 스펙트럴 군집**이 적합하다. R에서는 `dbscan::dbscan`, `dbscan::hdbscan`, `kernlab::specc`로 각각 사용한다. (근거: scikit-learn "Clustering" 문서의 알고리즘 비교; Ester et al. 1996 DBSCAN)
...
- Arthur, D. and Vassilvitskii, S.(2007), "k-means++: The Advantages of Careful Seeding," *Proc. 18th Annual ACM-SIAM Symposium on Discrete Algorithms (SODA)*, 1027-1035.
- Ester, M., Kriegel, H.-P., Sander, J., and Xu, X.(1996), "A Density-Based Algorithm for Discovering Clusters in Large Spatial Databases with Noise," *Proc. 2nd Int. Conf. on Knowledge Discovery and Data Mining (KDD-96)*, 226-231.
- Campello, R. J. G. B., Moulavi, D., and Sander, J.(2013), "Density-Based Clustering Based on Hierarchical Density Estimates (HDBSCAN)," *PAKDD 2013*, LNCS 7819, 160-172.
