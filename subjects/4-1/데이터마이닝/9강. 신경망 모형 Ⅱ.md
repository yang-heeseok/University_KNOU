...
4. 🟡 **summary / print 함수**: `summary(nn)`은 threshold·steps·error와 가중치 행렬 등 모형 요약을, 학습 로그 및 `nn$result.matrix`는 오차·수렴 step 수·가중치 추정값 등 계산과정 상세를 담는다.
...
| **summary 함수** | threshold·steps·error와 가중치 행렬 등 모형 요약 정보를 출력 | 모형 요약 |
...
| `summary()` | 신경망 오브젝트 | threshold·steps·error + 가중치 행렬 등 요약 | 모형 요약 파악 |
...
**5. ⭐ [이해] 신경망 시각화 함수**
> **정답: ③ plot**
> **해설**: neuralnet 패키지의 `plot` 함수는 작성된 신경망모형의 구조 및 가중치를 그림으로 출력하며, 이는 neuralnet 패키지의 주요 특징이다. summary는 오브젝트 구조, print는 계산과정, predict는 예측값 산출 함수이다.
...
> <a id="ox-3"></a>
> **3. 정답**: X
> **해설**: plot 함수는 신경망모형의 구조 및 가중치(검은선)·bias(파란선)를 **그림으로** 출력한다. 오차·수렴 step 수·가중치 추정값 등 계산과정 상세는 `nn$result.matrix`로 확인하고, `summary(nn)`은 threshold·steps·error와 가중치 행렬 등 요약 정보를 텍스트로 출력한다.

> <a id="ox-4"></a>
> **4. 정답**: O
> **해설**: summary 함수는 신경망모형 오브젝트의 구조를 정리하여 출력하는 기능을 수행한다.
...
# summary(nn): threshold·steps·error 및 가중치 행렬 등 모형 요약 정보 출력
summary(nn)
...
4. **결과 확인**: `plot()` → 구조 시각화, `summary()` → 구조 요약, `print()` → 계산과정
5. **예측**: `predict(model, newdata)` → 새 데이터에 대한 예측값 산출

#### 2. plot vs summary vs print 비교표

| 구분 | plot | summary | print |
| :--- | :--- | :--- | :--- |
| **출력 형태** | 그래프 (시각화) | 텍스트 (구조 요약) | 텍스트 (계산과정) |
| **주요 정보** | 네트워크 구조, 가중치 | 오브젝트 내부 구조 | 학습 계산 상세과정 |
| **활용 시점** | 모형 구조 직관적 파악 | 프로그래밍적 구조 확인 | 학습 과정 디버깅 |
...
#### 6. neuralnet에서 keras/torch로의 다리 (현대 딥러닝 비교)
...
| **summary** | summary | - | threshold·steps·error와 가중치 행렬 등 모형 요약을 출력하는 함수명 |
