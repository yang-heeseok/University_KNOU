...
>
> **약 37%/63%의 수학적 근거**: 크기 $n$인 자료에서 복원추출로 $n$개를 뽑을 때 특정 관측치가 한 번도 선택되지 않을 확률은 $\left(1-\frac{1}{n}\right)^n$이며, $\lim_{n\to\infty}\left(1-\frac{1}{n}\right)^n = e^{-1}\approx 0.368$이다. 즉 각 부트스트랩 표본에서 평균적으로 약 36.8%(≈37%)가 제외(OOB)되고 약 63.2%(≈63%)가 포함된다.
...
>
> [변수 중요도 Top 5]
>   color_intensity: 0.1800
>   flavanoids: 0.1660
>   alcohol: 0.1420
>   proline: 0.1260
>   od280/od315_of_diluted_wines: 0.0960
> ```
> ※ sklearn 버전·랜덤시드에 따라 정확도·중요도 수치는 변동 가능하다.
...
| 비교 항목 | adabag (배깅/부스팅) · randomForest | XGBoost | LightGBM | CatBoost |
| :--- | :--- | :--- | :--- | :--- |
| **정규화(Regularization)** | 사실상 없음 (가지치기·트리 수에 의존) | L1·L2 정규화 항을 목적함수에 내장 | L1·L2 정규화 내장 | L2 정규화 + ordered boosting으로 과적합 억제 |
| **결측치 처리** | 별도 전처리/대치 필요 | 결측치 자동 학습(default direction) | 결측치 자동 처리 | 결측치 자동 처리 |
| **분할 방식** | 정확 분할(exact, 전체 후보 탐색) | 히스토그램/근사 분할 지원 | 히스토그램 기반 + leaf-wise 성장 | 히스토그램 기반 + **대칭(oblivious) 트리** |
| **병렬화** | 트리 단위 병렬(RF)·순차(부스팅) | 분할 탐색 수준의 병렬화 | 고도 병렬화 + GPU 지원 | GPU 지원 + 대칭 트리로 빠른 추론 |
| **대용량 적합성** | 중소규모에 적합 | 대규모에 강함 | 초대규모·고차원에 가장 빠름 | 범주형 변수가 많은 표 형식 데이터에 강함 |
| **범주형 변수** | 사전 인코딩 필요 | 사전 인코딩 필요(one-hot 등) | 정수 인코딩 후 내부 처리 | **자동 처리**(ordered target statistics, 인코딩 불필요) |

> **요약**: adabag·randomForest가 개념 학습·해석에 적합한 교과서 표준이라면, XGBoost/LightGBM은 정규화·결측치 자동처리·히스토그램 분할·병렬화로 대규모 실무 성능을 끌어올린 현대적 구현이다.

##### CatBoost의 차별점 (Yandex, 2017)

CatBoost는 범주형 변수(categorical features)가 많은 표 형식(tabular) 데이터에서 특히 강점을 가지는 그래디언트 부스팅 라이브러리이다. 세 가지 핵심 특징을 갖는다.

- **범주형 변수 자동 처리**: one-hot 인코딩 없이 범주형 변수를 직접 입력받아, **타깃 통계량(target statistics)** 기반으로 수치화한다. 범주별 목표변수 평균 등을 인코딩 값으로 사용하되, 아래의 ordered 방식으로 누수를 막는다.
- **Ordered boosting (타깃 누수 방지)**: 일반적인 타깃 인코딩은 해당 관측치 자신의 목표값을 인코딩에 사용해 **타깃 누수(target leakage)**가 생긴다. CatBoost는 관측치들을 무작위 순열(permutation)로 정렬한 뒤, 각 관측치의 인코딩·잔차 계산에 **그 관측치 이전에 등장한 데이터만** 사용하여 누수와 과적합(prediction shift)을 억제한다.
- **대칭(oblivious) 트리**: 한 깊이의 모든 노드가 동일한 분할 기준을 쓰는 균형 잡힌 대칭 트리를 사용한다. 이는 정규화 효과를 주고 추론 속도를 크게 높인다(XGBoost의 leaf-wise/level-wise, LightGBM의 leaf-wise 성장과 대비된다).

> **요약**: CatBoost는 "범주형 변수 자동 처리 + ordered boosting으로 타깃 누수 방지 + 대칭 트리"의 세 축으로, 별도 인코딩 없이 범주형이 풍부한 데이터에서 안정적 성능을 내는 현대적 부스팅 구현이다.

#### 5. 변수중요도의 한계와 SHAP (Shapley Additive Explanations)

**기존 변수중요도(MDI/MDA)의 한계**

randomForest의 `importance()`가 제공하는 전통적 변수중요도에는 알려진 한계가 있다.

- **상관 변수 간 중요도 분산**: 서로 강하게 상관된 입력변수들이 있으면, 두 변수가 본질적으로는 동일한 정보를 담더라도 트리들이 분할에 둘 중 하나를 번갈아 선택하므로 중요도가 양쪽으로 **나뉘어(분산되어)** 각각의 중요도가 실제보다 과소평가될 수 있다.
- **MDI(지니 기반, MeanDecreaseGini)의 편향**: 노드 불순도 감소 합으로 계산하는 MDI는 **분할 후보(범주)가 많거나 연속형인 고카디널리티(high-cardinality) 변수에 유리하게 편향**되는 경향이 있다(무관한 변수라도 분할 기회가 많으면 중요도가 부풀려질 수 있다).

**SHAP 값 — 게임이론 기반 개별 예측 기여도**

이러한 한계를 보완하는 현대적 해석 기법이 **SHAP(SHapley Additive exPlanations)**이다.

- **게임이론적 근거**: 협력 게임이론의 **섀플리 값(Shapley value)**을 차용하여, "각 입력변수가 한 예측값에 기여한 몫"을 공정하게 분배한다. 변수들의 가능한 모든 조합(연합)에서 한 변수를 추가했을 때의 예측 변화(한계 기여)를 가중 평균한다.
- **가법적 분해(additive)**: 한 예측값은 기준값(base value, 전체 평균 예측)에 각 변수의 SHAP 값을 더한 합으로 정확히 분해된다 — $\hat{f}(x) = \phi_0 + \sum_{i} \phi_i$. 즉 "이 예측이 평균에서 얼마나, 어떤 변수 때문에 벗어났는가"를 설명한다.
- **국소(local) 설명**: importance()가 모형 전체의 전역 중요도 하나만 주는 것과 달리, SHAP은 **개별 관측치 한 건 한 건의 예측에 대한 변수별 기여도**를 제공한다(국소 설명). SHAP 값의 평균 절댓값을 모으면 전역 중요도로도 활용할 수 있다.
- **일관성(consistency)**: 어떤 변수의 실제 기여가 커지도록 모형이 바뀌면 그 변수의 SHAP 중요도가 결코 줄어들지 않음이 이론적으로 보장된다. 전통적 MDI/MDA에는 이런 일관성 보장이 없다.

**TreeSHAP과 importance()의 관계 (전역 + 국소 설명)**

- `importance()`(MeanDecreaseAccuracy/Gini)는 모형 전체에 대한 **전역(global) 변수 중요도** 하나의 값만 제공한다.
- **TreeSHAP**은 위 SHAP 값을 트리 앙상블(RF·XGBoost·LightGBM·CatBoost 등)에서 다항시간으로 효율적으로 계산하는 알고리즘으로, 전역 중요도(SHAP 값의 평균 절댓값)뿐 아니라 **각 개별 예측에 대한 국소(local) 기여도**까지 제공한다.
