...
**1. ⭐ [기억] R 함수 빈칸 채우기** 📎 기출
> **정답: ① lm, glm**
> **해설**: lm은 선형회귀모형에, glm은 옵션을 binomial로 설정하면 로지스틱 회귀모형 적합에 이용할 수 있다. setp는 존재하지 않는 함수이며, predict는 예측값 계산용 함수이다.
...
| **로지스틱회귀모형** | Logistic Regression | $\log\frac{p}{1-p} = X\beta$ | 이항형 목표변수의 로그 오즈를 선형 결합으로 모형화. `glm(family=binomial)` `→←2강` |
