...
5. 🔴 **분산분석표**: 총변동 $\text{SST}=\text{SSR}+\text{SSE}$ 로 분해되고, $F=\text{MSR}/\text{MSE}$ 로 귀무가설 $H_0:\beta_1=\beta_2=\cdots=\beta_k=0$ (회귀 전체가 무의미)을 검정하며, $R^2=\text{SSR}/\text{SST}$ 로 설명력을 측정한다. 변수 수를 벌점한 $R_a^2=1-\dfrac{\text{SSE}/(n-k-1)}{\text{SST}/(n-1)}$ 은 변수 개수가 다른 모형을 비교할 때 쓴다.
...
**회귀 전체 유의성의 $F$-검정 (귀무가설 명시)**
중회귀에서 "설명변수 전체가 $y$ 와 아무 선형관계가 없다"는 가설은 절편을 제외한 모든 기울기가 0이라는 것이다.
$$
H_0:\beta_1=\beta_2=\cdots=\beta_k=0 \quad\text{vs}\quad H_1:\ \text{적어도 하나의 }\beta_j\neq0
$$
$$
F=\frac{\text{MSR}}{\text{MSE}}=\frac{\text{SSR}/k}{\text{SSE}/(n-k-1)}\ \sim\ F_{(k,\,n-k-1)}\ \ (H_0\text{ 하에서})
$$
> **직관적 해석**: $H_0$ 가 참이면 회귀가 설명하는 변동(SSR)은 잡음 수준에 불과해 $F\approx1$ 이지만, 실제로 어떤 변수든 유의하게 기여하면 SSR이 커져 $F$ 가 커진다. $F$ 가 임계값 $F_{(k,\,n-k-1)}$ 보다 크면 $H_0$ 를 기각하고 "회귀 전체가 유의하다"고 본다. 단, 이는 **전체** 유의성이며, 개별 계수의 유의성은 별도의 $t$-검정으로 본다.

**수정결정계수 $R_a^2$ 의 계산과 의미**
$$
R_a^2=1-\frac{\text{SSE}/(n-k-1)}{\text{SST}/(n-1)}=1-(1-R^2)\frac{n-1}{n-k-1}
$$
> **직관적 해석**: $R^2$ 은 분자(SSE)와 분모(SST)를 **그냥** 나눈 값이라 설명변수를 추가하면 SSE가 절대 늘지 않아 무조건 증가/유지된다. 반면 $R_a^2$ 은 SSE와 SST를 각각 **자유도**($n-k-1$, $n-1$)로 나눈 평균제곱의 비를 쓰므로, 변수 $k$ 를 늘리면 분자 쪽 자유도 $n-k-1$ 이 줄어 **벌점**이 붙는다. 따라서 새로 넣은 변수가 SSE를 충분히 줄이지 못하면 $R_a^2$ 은 오히려 **감소**한다. 예컨대 본 단원 코드 예제는 $R^2=0.9497$, $R_a^2=1-(1-0.9497)\times\dfrac{49}{47}=0.9475$ 로 두 설명변수 모두 자유도 손실을 상쇄할 만큼 기여해 $R_a^2$ 이 거의 줄지 않았다. 변수 개수가 다른 모형을 비교할 때는 $R^2$ 대신 $R_a^2$ (또는 AIC/BIC, 검증오차)로 판단한다.
...
> **실행 결과 (실제, seed=0 재현)**
> ```
> beta_hat = [ 2.595   1.4965 -0.8777]
> sum(e) = 0.0
> X^T e  = [0. 0. 0.]
> H idempotent?  True
> trace(H) = 3.0
> SST=1006.95, SSR=956.25, SSE=50.70
> R^2 = 0.9497,  F = 443.24 (df=(2,47))
> ```
> **수학적 해석**: 기울기 추정 $(\hat\beta_1\approx1.50,\ \hat\beta_2\approx-0.88)$ 은 실제 모수 $(1.5,\,-0.8)$ 에 매우 가깝지만, 절편 추정은 $\hat\beta_0\approx2.60$ 으로 참값 $2$ 에서 다소 벗어나 있다 — 이는 표본 1개에서 나타나는 **표본변동(sampling variability)** 의 자연스러운 결과로, 절편은 $x_1,x_2$ 가 모두 0인 외삽점의 값이라 일반적으로 표준오차가 크다. 잔차합과 $X^\top\mathbf{e}$ 가 모두 0 — 정규방정식의 직교성이 코드로 확인된다. $\text{tr}(H)=3=k+1$ 은 모형의 유효 자유도이고, $R^2=0.95$ 와 큰 $F=443.24$ 는 모형이 매우 유의함을 보여준다. (수정결정계수도 $R_a^2=0.9475$ 로 $R^2$ 와 거의 같아, 두 설명변수가 모두 실질적으로 기여함을 시사한다.)
...
#### 4. 지렛대(leverage) 임계값: 영향점 진단 기준의 유도

햇행렬의 대각원소 $h_{ii}=[H]_{ii}=\mathbf{x}_i^\top(X^\top X)^{-1}\mathbf{x}_i$ 는 관측치 $i$ 의 **지렛대(leverage)** 로, $0\le h_{ii}\le1$ 의 값을 가지며 $\hat y_i=\sum_j h_{ij}y_j$ 에서 $\partial\hat y_i/\partial y_i=h_{ii}$, 즉 자기 자신의 관측값이 자기 적합값에 미치는 직접 영향력이다.

**평균 지렛대의 유도.** $H$ 가 대칭·멱등 정사영행렬이므로 $\text{tr}(H)=\text{rank}(X)=k+1$ 이고(3강 햇행렬 성질), 대각원소의 합이 곧 trace이므로
$$
\sum_{i=1}^{n}h_{ii}=\text{tr}(H)=k+1
\;\Longrightarrow\;
\bar h=\frac{1}{n}\sum_{i=1}^{n}h_{ii}=\frac{k+1}{n}.
$$
즉 **모든 관측치가 균등하게 기여한다면** 각 지렛대는 평균 $\bar h=(k+1)/n$ 을 가져야 한다. 여기서 $k+1=p$ 는 절편 포함 모수(추정 계수) 개수다.

**경험적 임계값.** 어떤 관측치가 평균의 **2배를 초과**하면 설계공간에서 비정상적으로 멀리 떨어진 고지렛대점(high-leverage point)으로 본다.
$$
\boxed{\;h_{ii}>2\bar h=\dfrac{2(k+1)}{n}\;}\quad(\text{Hoaglin–Welsch 1978 경험적 기준})
$$
표본이 작거나 $p$ 가 크면 $3(k+1)/n$ 을 쓰기도 한다. 지렛대가 큰 점은 그 자체로 이상치는 아니지만(잔차가 작을 수도 있음), **잠재적 영향점**이다. 실제 영향력은 잔차와 지렛대를 결합한 **Cook 거리** $D_i=\dfrac{e_i^2}{p\,\hat\sigma^2}\cdot\dfrac{h_{ii}}{(1-h_{ii})^2}$ 로 판단하며, $D_i>1$ (또는 $4/n$)이면 영향점으로 진단한다.

> **직관적 해석**: 지렛대는 "설명변수 공간에서 그 점이 무게중심($\bar{\mathbf{x}}$)에서 얼마나 멀리 떨어졌는가"를 마할라노비스 거리로 잰 것이다. 시소의 받침점에서 먼 점일수록 작은 변화로도 회귀선을 크게 흔든다 — 그래서 'leverage(지레)'다.
> **AI 적용**: $h_{ii}$ 는 LOOCV(leave-one-out 교차검증)에서 핵심이다. PRESS 잔차가 $e_{(i)}=e_i/(1-h_{ii})$ 로 한 번의 적합만으로 LOO 오차를 계산할 수 있는 근거이며, 가우시안 과정·커널 능형회귀의 유효 자유도 $\text{tr}(H)$ 와 직결된다.

**실측 (위 코드 예제 데이터, seed=0, $n=50$, $k=2$)**
```python
H = X @ np.linalg.inv(X.T @ X) @ X.T
h = np.diag(H)                      # 지렛대
p = 3                               # k+1
print("h_bar = (k+1)/n      =", p/50)          # 0.06
print("임계 2(k+1)/n        =", 2*p/50)        # 0.12
print("초과 관측치          =", (h > 2*p/50).sum())
print("sum(h_ii)            =", round(h.sum(), 4))  # = k+1
```
> **실행 결과 (실제, seed=0 재현)**
> ```
> 평균 leverage h_bar = (k+1)/n = 0.0600  (실제 mean(h_ii)=0.0600)
> 경험적 임계값 2(k+1)/n = 0.1200
> 임계값 초과 관측치 개수 = 2, 인덱스 = [16, 20]
> max h_ii = 0.1621, sum h_ii = 3.0000 (= k+1)
> ```
> 평균 지렛대가 정확히 $0.06=(k+1)/n$ 으로 떨어지고, 대각합이 $3.0=k+1$ 임이 확인된다. 임계값 $0.12$ 를 넘는 관측치 2개(설계공간 가장자리의 점)가 고지렛대 후보로 걸러진다 — 이상치 진단의 1차 필터다.

#### 5. 수치 안정성 — 조건수 $\kappa(X^\top X)=\kappa(X)^2$

정규방정식 $X^\top X\hat{\boldsymbol{\beta}}=X^\top\mathbf{y}$ 를 $\hat{\boldsymbol{\beta}}=(X^\top X)^{-1}X^\top\mathbf{y}$ 로 **직접** 푸는 방식의 치명적 약점은 $X^\top X$ 를 형성하는 순간 조건수가 **제곱**된다는 데 있다.

**조건수 제곱의 유도.** 행렬 $X$ ($n\times p$, full column rank)의 특잇값분해(SVD)를 $X=U\Sigma V^\top$ 라 하면 $\sigma_1\ge\cdots\ge\sigma_p>0$ 에 대해 2-노름 조건수는 $\kappa(X)=\sigma_1/\sigma_p$ 이다. 그러면
$$
X^\top X=V\Sigma^\top U^\top U\Sigma V^\top=V\Sigma^2 V^\top
$$
이므로 $X^\top X$ 의 고윳값은 $\sigma_i^2$ 이고,
$$
\boxed{\;\kappa_2(X^\top X)=\frac{\sigma_1^2}{\sigma_p^2}=\left(\frac{\sigma_1}{\sigma_p}\right)^2=\kappa_2(X)^2\;}
$$
**오차 증폭의 의미.** 선형계의 상대오차는 대략 조건수 $\times$ 기계입실론($\varepsilon_{\text{mach}}\approx10^{-16}$)에 비례한다. 따라서

| 방법 | 푸는 대상 | 유효 조건수 | 정밀도 손실(자릿수) |
| :--- | :--- | :--- | :--- |
| 정규방정식 직접 풀이 | $X^\top X$ | $\kappa(X)^2$ | $\approx2\log_{10}\kappa(X)$ |
| QR 분해 ($X=QR$) | $R$ (= $X$와 동급) | $\kappa(X)$ | $\approx\log_{10}\kappa(X)$ |
| SVD (`lstsq`) | $\Sigma$ | $\kappa(X)$ | $\approx\log_{10}\kappa(X)$ |

예컨대 $\kappa(X)=10^8$ 이면 정규방정식은 $\kappa(X^\top X)=10^{16}$ 으로 배정밀도(약 16자리)의 유효숫자를 **거의 전부** 잃는 반면, QR/SVD는 약 8자리만 잃는다. 이것이 `np.linalg.lstsq`·QR·`scipy.linalg.lstsq` 가 정규방정식 `inv`보다 권장되는 정량적 근거다.

**실측 — 조건수와 추정오차 비교 (Vandermonde 설계, 차수 9, $n=60$)**
```python
import numpy as np
np.random.seed(0)
n, deg = 60, 10
t = np.linspace(0, 1, n)
X = np.column_stack([t**j for j in range(deg)])   # 강한 다중공선성
beta_true = np.arange(1, deg+1, dtype=float)
y = X @ beta_true + np.random.normal(0, 1e-6, n)

kX   = np.linalg.cond(X)
kXtX = np.linalg.cond(X.T @ X)
print(f"cond(X)     = {kX:.4e}")
print(f"cond(X^T X) = {kXtX:.4e}")
print(f"cond(X)^2   = {kX**2:.4e}")

beta_normal = np.linalg.inv(X.T @ X) @ X.T @ y     # 정규방정식 inv
beta_lstsq, *_ = np.linalg.lstsq(X, y, rcond=None) # QR/SVD
print("정규방정식 오차 =", np.linalg.norm(beta_normal - beta_true))
print("lstsq    오차 =", np.linalg.norm(beta_lstsq  - beta_true))
```
> **실행 결과 (실제, seed=0 재현)**
> ```
> cond(X)      = 3.6000e+06
> cond(X^T X)  = 1.2960e+13
> cond(X)^2    = 1.2960e+13
> 비율 cond(XtX)/cond(X)^2 = 1.0000  (이론상 1)
>
> 정규방정식 inv 오차 ||beta_hat - beta_true|| = 1.0346e-01
> lstsq(QR/SVD) 오차 ||beta_hat - beta_true|| = 2.7976e-02
> 오차 비율 (normal/lstsq) = 3.70배
> ```
> $\kappa(X^\top X)=1.296\times10^{13}$ 이 $\kappa(X)^2$ 과 **비율 1.0000으로 정확히 일치**해 항등식 $\kappa(X^\top X)=\kappa(X)^2$ 이 수치로 확인된다. 그 결과 정규방정식 직접 풀이의 추정오차가 QR/SVD 기반 `lstsq` 보다 약 **3.7배 크다**. 조건수가 더 큰 설계(예: 차수↑, 표본↓)일수록 격차는 급격히 벌어지며, $\kappa(X)\gtrsim10^8$ 부터는 정규방정식이 사실상 무의미한 해를 내놓는다.
> **AI 적용**: 딥러닝 프레임워크가 선형 최소제곱을 풀 때 `torch.linalg.lstsq`(QR/SVD)를 기본으로 쓰고, 능형회귀의 $\lambda I$ 추가가 $X^\top X+\lambda I$ 의 최소고윳값을 $\sigma_p^2+\lambda$ 로 끌어올려 조건수를 $\dfrac{\sigma_1^2+\lambda}{\sigma_p^2+\lambda}$ 로 낮추는 것(정규화=조건수 개선)도 같은 맥락이다.
