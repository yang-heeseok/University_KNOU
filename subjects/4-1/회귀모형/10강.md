...
# ── (참고) 중심화로 다항항 다중공선성 완화 ───────────────────
# x_c = x - x.mean() 로 중심화한 뒤 x_c, x_c² 의 VIF를 다시 계산
xc = df.x - df.x.mean()
Xc = np.column_stack([np.ones(n), xc, xc**2, D])
vifs_c = [variance_inflation_factor(Xc, j) for j in range(1, Xc.shape[1])]
print("centered VIF [x_c, x_c^2, D] =", np.round(vifs_c, 2))
```

> **실행 결과**
> ```
> R^2 = 0.3681  F p = 0.0
> 지렛대 임계 = 0.125, 최대 h_ii = 0.200 (idx 20)    # 최대 지렛대는 20번 관측치
> 최대 Cook's D = 1.334 (idx 0)                      # 0번(삽입점)이 D>1 → 영향점
> Shapiro p = 0.0
> Breusch-Pagan p = 0.1377
> Durbin-Watson = 1.158
> VIF [x, x^2, D] = [15.65 15.71 1.02]               # x와 x^2 공선성
> centered VIF [x_c, x_c^2, D] = [1.02 1.01 1.02]    # 중심화 후 공선성 거의 소멸
> ```
> **수학적 해석**: 인위로 넣은 0번 관측치는 **Cook's D가 1.334로 최댓값($D>1$)** 이라 적합에 가장 큰 영향을 주는 **영향점**으로 탐지된다. 다만 **최대 지렛대는 0번이 아니라 20번 관측치($h_{ii}=0.200$)** 다 — 0번의 $x=9.5$ 가 설계공간 구석은 아니어서 지렛대 자체보다 **큰 잔차×지렛대의 곱**(Cook's D)으로 영향력이 드러난 경우다(영향점이 반드시 최대 지렛대점인 것은 아님을 보여준다). $x$ 와 $x^2$ 의 VIF가 15.6~15.7 로 크게 나오는 것은 다항항의 전형적 다중공선성으로, **중심화 $x_c=x-\bar x$ 를 적용하면 위 코드의 마지막 줄처럼 VIF가 [1.02, 1.01, 1.02] 로 거의 1까지 떨어져** 공선성이 사실상 사라진다(7강 참고). 중심화는 적합값·예측을 바꾸지 않으면서 $x$ 와 $x^2$ 의 상관만 제거하는 간단한 처방이다. Breusch–Pagan $p=0.1377>0.05$ 라 등분산 가정은 기각되지 않지만, **Durbin–Watson은 1.158 로 2에서 꽤 떨어져 있어 잔차의 양의 자기상관(독립성 위배)이 시사**된다(2 부근이 무상관이며, 여기서는 무상관으로 보기 어렵다).
...
### 종합 영향력 진단: `OLSInfluence(model).summary_frame()` 한 번에 모두 뽑기

영향력 측도를 `inf.hat_matrix_diag`, `inf.cooks_distance[0]`, `inf.dffits` … 처럼 **하나씩** 꺼내는 대신, `OLSInfluence(model).summary_frame()` 으로 **leverage·표준화 잔차·외적 스튜던트화 잔차·Cook's D·DFFITS·DFBETAS** 를 **한 장의 DataFrame** 으로 받을 수 있다(R `influence.measures()` 의 Python 대응). 단, **COVRATIO 는 `summary_frame()` 에 포함되지 않으므로** `inf.cov_ratio` 속성으로 따로 붙인다.
...
np.random.seed(0)
n = 30
x1 = np.random.uniform(0, 10, n); x2 = np.random.uniform(0, 5, n)
y  = 3 + 1.5*x1 - 2*x2 + np.random.normal(0, 1.5, n)
x1[0], y[0] = 9.8, 40.0                      # 영향점 1개 인위 삽입
df = pd.DataFrame({'y': y, 'x1': x1, 'x2': x2})

m    = smf.ols('y ~ x1 + x2', df).fit()
infl = OLSInfluence(m)
sf   = infl.summary_frame()                  # leverage·표준화/스튜던트화 잔차·Cook's D·DFFITS·DFBETAS
print("열 이름:", list(sf.columns))

view = sf[['hat_diag', 'student_resid', 'cooks_d', 'dffits', 'dfb_x1', 'dfb_x2']].copy()
view['cov_ratio'] = infl.cov_ratio           # COVRATIO는 summary_frame에 없어 별도 추가
print(view.head(6).round(3).to_string())
```

> **실행 결과** (`c:\tmp\influence_frame.py`)
> ```
> 열 이름: ['dfb_Intercept', 'dfb_x1', 'dfb_x2', 'cooks_d', 'standard_resid', 'hat_diag', 'dffits_internal', 'student_resid', 'dffits']
>    hat_diag  student_resid  cooks_d  dffits  dfb_x1  dfb_x2  cov_ratio
> 0     0.103         24.164    0.994   8.205   5.804  -2.685      0.000
> 1     0.099         -0.029    0.000  -0.010  -0.003  -0.008      1.243
> 2     0.033         -0.251    0.001  -0.047  -0.001  -0.001      1.150
> 3     0.041         -0.404    0.002  -0.083   0.009  -0.033      1.146
> 4     0.154          0.004    0.000   0.002  -0.001  -0.002      1.323
> 5     0.050          0.026    0.000   0.006   0.001   0.003      1.179
>
> 임계값 참고: h_ii>2(k+1)/n = 0.2 | Cook's D>4/n = 0.133 | |DFFITS|>2*sqrt((k+1)/n) = 0.632 | |DFBETAS|>2/sqrt(n) = 0.365
> ```

**각 측도와 임계값(rule of thumb) 한눈 정리**

| 측도(열) | 의미 | 흔히 쓰는 임계값 ($n=30,k=2$) | 0번 관측치 판정 |
| :--- | :--- | :--- | :--- |
| `hat_diag` $h_{ii}$ | 설명변수 공간 내 위치(지렛대) | $>2(k{+}1)/n=0.20$ | $0.103$ → 비고지렛대 |
| `student_resid` $t_i$ | 외적 스튜던트화 잔차(이상치) | $\lvert t_i\rvert>2{\sim}3$ | $24.16$ → **극단 이상치** |
| `cooks_d` $D_i$ | 적합 전체에 주는 영향 | $>4/n=0.133$ (또는 $>1$) | $0.994$ → **영향점** |
| `dffits` | $i$ 제외 시 자기 적합값 변화(표준화) | $\lvert\text{DFFITS}\rvert>2\sqrt{(k{+}1)/n}=0.632$ | $8.205$ → **초과** |
| `dfb_x1`,`dfb_x2` (DFBETAS) | $i$ 제외 시 **개별 계수** 변화(표준화) | $\lvert\text{DFBETAS}\rvert>2/\sqrt n=0.365$ | $5.80,\,-2.69$ → **둘 다 초과** |
| `cov_ratio` (별도) | $i$ 제외 시 추정 정밀도(공분산) 변화 | $\lvert\text{COVRATIO}-1\rvert>3(k{+}1)/n=0.30$ | $0.000$ → **정밀도 급변** |

> **직관적 해석**: 인위 삽입한 0번 관측치는 **지렛대 $h_{ii}=0.103$ 자체는 임계값 0.20을 넘지 않아 "구석에 있는 점"은 아니지만**, 잔차가 $t_0=24.16$ 으로 극단적이라 **Cook's D(0.994)·DFFITS(8.2)·DFBETAS(5.8, −2.7)** 가 모든 임계값을 한참 넘어 **계수를 실제로 바꾸는 영향점**으로 드러난다. 특히 `cov_ratio≈0` 은 이 점이 추정 공분산을 비정상적으로 줄여(겉보기 정밀도를 왜곡해) 추론을 오염시킨다는 신호다. 큰 잔차 하나가 지렛대가 낮아도 영향점이 될 수 있음을 한 줄 호출(`summary_frame`)로 동시에 확인한 셈이다.
> **주의(열 이름 매핑)**: `summary_frame()` 의 DFBETAS 열은 `dfb_<설명변수명>`(예 `dfb_x1`) 형태이고, **외적** 스튜던트화 잔차는 `student_resid`, 표준화 잔차는 `standard_resid`, **내부** DFFITS 는 `dffits_internal` 이다. COVRATIO 는 프레임에 없어 `OLSInfluence` 객체의 `cov_ratio` 속성으로 가져온다.
> **AI 적용**: data-centric AI 의 학습데이터 품질관리에서 이 한 장의 영향력 프레임은 라벨 오류·이상 샘플을 일괄 스캔하는 진단 시트가 된다. `cooks_d`·`dffits` 상위 샘플을 자동 플래그해 재검토 큐로 보내는 파이프라인이 전형적 활용이다.
