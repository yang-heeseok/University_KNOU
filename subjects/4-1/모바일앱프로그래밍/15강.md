...
# 가속도계는 중력을 포함해 측정하므로, 화면을 위로 둔 채 정지 시 y축이 약 +9.81 m/s²(중력 g).
# 아래 예시 [0.12, -9.81, 0.04]은 단말이 거의 정지한 한 시점의 (x, y, z) 가속도값을 의미한다.
...
> 💡 **좌표계·중력 보충**: 가속도계 `values[0..2]`는 안드로이드 **디바이스 좌표계**(x=화면 오른쪽, y=화면 위, z=화면 바깥)를 따르며, **중력을 포함**해 측정한다. 따라서 화면을 위로 둔 채 단말이 정지해 있어도 한 축이 약 **9.81 m/s²(중력 g)** 를 가리킨다(예: `y ≈ -9.81`은 측정 방향·자세에 따른 중력 성분). (근거: https://developer.android.com/guide/topics/sensors/sensors_overview )
