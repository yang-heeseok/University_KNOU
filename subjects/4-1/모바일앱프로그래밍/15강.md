...
> 📌 **계층도 캡션**: ③ 네이티브 C/C++ 라이브러리와 ④ 안드로이드 런타임은 **같은 레벨**(②와 ⑤ 사이)에 나란히 위치한다 — 위아래 관계가 아니라 **병렬 계층**이므로 도식에서 ②→③, ②→④로 갈라졌다가 ③→⑤, ④→⑤로 다시 합쳐진다.
...
- **오개념 분석**
    ...
    - **분석**: ❌. HAL은 **JAVA API 프레임워크가 하드웨어를 표준화된 방식으로 제어**하기 위한 인터페이스 계층이다. 앱은 보통 JAVA API 프레임워크를 통해 간접적으로 HAL을 거치게 된다.
        - 💡 **호출 경로 보충**: 실제 흐름은 **앱 → JAVA API 프레임워크 → (system_server 등 시스템 프로세스의) 네이티브 서비스 → HAL** 순서다. 즉 앱은 **HAL 라이브러리를 직접 링크하지 않고**, 프레임워크 API(예: `SensorManager`)를 호출하면 그 요청이 시스템 서비스를 거쳐 HAL로 내려간다. (근거: https://source.android.com/docs/core/architecture/hal )
...
# 디바이스 좌표계: x=화면 오른쪽, y=화면 위, z=화면 바깥(앞쪽).
# 가속도계는 중력을 포함해 측정하므로, 정지 상태에서도 한 축이 약 +9.81 m/s²(중력 g)를 가리킨다.
# 자세에 따라 어느 축이 +9.81을 갖는지가 달라진다:
#   - 단말을 책상에 화면이 위를 향하도록 "평평히" 두면 → 중력은 +z축 (z ≈ +9.81, x≈y≈0)
#   - 단말을 화면이 정면을 보도록 "세워서" 들면      → 중력은 +y축 (y ≈ +9.81, x≈z≈0)
# 아래 예시 [0.0, 0.0, 9.81]은 단말을 화면 위로 평평히 둔 채 거의 정지한 시점의 (x, y, z) 가속도값이다.
sm._fire([0.0, 0.0, 9.81])
```
...
>   [onSensorChanged] x=0.0, y=0.0, z=9.81
> ```
...
> 💡 **좌표계·중력 보충**: 가속도계 `values[0..2]`는 안드로이드 **디바이스 좌표계**(x=화면 오른쪽, y=화면 위, z=화면 바깥)를 따르며, **중력을 포함**해 측정한다. 따라서 단말이 정지해 있어도 한 축이 약 **+9.81 m/s²(중력 g)** 를 가리키는데, **어느 축인지는 자세에 따라 달라진다**. 표준 좌표계에서 단말을 **화면이 위를 향하게 평평히** 두면 중력은 **+z축(z ≈ +9.81, x≈y≈0)**, 단말을 **화면이 정면을 보게 세우면** 중력은 **+y축(y ≈ +9.81)** 이다. 위 예시 `[0.0, 0.0, 9.81]`은 평평히 둔 자세에 해당한다(측정 방향·자세에 따른 중력 성분). (근거: https://developer.android.com/guide/topics/sensors/sensors_overview#sensors-coords )
...
## 🟢 심층 확장: 최신 아키텍처 · 런타임 · 센서 패턴

> 이 절은 공식 문서(source.android.com / developer.android.com) 기반의 리서치 심층 확장입니다. 앞서 정리한 6계층 아키텍처, ART, 센서 활용을 최신 동향과 권장 패턴으로 확장합니다.

### 1. Project Treble — HAL을 vendor 파티션으로 분리하여 OS 업데이트 가속

본문 아키텍처 표의 **HAL(하드웨어 추상화 계층)** 은 Android 8.0(Oreo)의 **Project Treble** 로 크게 재구성되었습니다.

- Android 8.0 이전에는 정식 **vendor interface** 가 없어, 제조사가 새 OS로 올리려면 Android 코드의 많은 부분을 함께 수정해야 했습니다.
- Treble은 **Android OS 프레임워크와 vendor 구현(HAL 등)을 분리**하고 둘 사이에 **vendor interface** 를 도입했습니다. HAL은 별도의 **vendor 파티션**에 위치하며, 이 인터페이스는 **VTS(Vendor Test Suite)** 로 검증됩니다.
- 그 결과 **프레임워크를 vendor 구현과 독립적으로 업데이트**할 수 있어 OS 업데이트가 빨라집니다.

```
[Treble 이전]                        [Treble 이후 (Android 8.0+)]
Android Framework                    Android Framework  (system 파티션)
       │ (강결합)                            │  ── vendor interface (VTS 검증)
HAL + 드라이버  ← OS 업데이트 시              │
   전부 함께 수정                      HAL + 드라이버   (vendor 파티션)
                                      → 프레임워크만 독립 업데이트 가능
```

> 핵심: 6계층 중 HAL/커널 영역이 vendor 파티션으로 분리되어, 프레임워크 업데이트가 하드웨어 구현 재작업 없이 가능해졌습니다.
> 근거: https://source.android.com/docs/core/architecture

**Treble을 떠받치는 두 축: VNDK와 GSI (다른 각도 보강)**

Treble의 "프레임워크 ↔ vendor 분리"가 실제로 동작하려면 둘이 약속된 경계로만 통신해야 한다. 이를 강제하는 장치가 다음과 같다.

| 구성요소 | 역할 |
| :--- | :--- |
| **vendor interface** | system 파티션(프레임워크)과 vendor 파티션(HAL) 사이의 **공식 경계**. HIDL/AIDL로 정의된 HAL 인터페이스가 여기 해당 |
| **VNDK** (Vendor Native Development Kit) | vendor 코드가 의존해도 되는 **안정적 네이티브 라이브러리 집합**. 프레임워크가 바뀌어도 vendor가 쓰던 C/C++ 라이브러리 ABI가 유지되도록 함 |
| **VTS** (Vendor Test Suite) | vendor interface 구현이 규약을 지키는지 **자동 검증**하는 테스트 모음 |
| **GSI** (Generic System Image) | 순정 AOSP 프레임워크 이미지를 vendor 파티션 위에 그대로 올려 **부팅 가능 여부로 분리 성공을 입증**(Treble 호환성 증명) |

> 핵심: Treble은 단순 선언이 아니라 **VNDK(라이브러리 ABI 안정화) + VTS(규약 검증) + GSI(분리 증명)** 라는 구체적 강제 장치로 뒷받침된다. 본문 6계층의 ③ 네이티브 C/C++ 라이브러리 일부가 "vendor가 의존 가능한 VNDK"로 따로 분류된 것이 핵심 변화다.
> 근거: https://source.android.com/docs/core/architecture/vndk , https://source.android.com/docs/core/tests/vts

### 2. ART 최신 — JIT+AOT 하이브리드(7.0+)와 Baseline Profiles

본문에서 다룬 **ART(Android Runtime)** 는 초기엔 "설치 시 전체 AOT" 방식이었지만, 현재는 다음과 같이 발전했습니다.

| 단계 | 방식 | 도입 |
| :--- | :--- | :--- |
| 초기 ART | AOT(설치 시 전체 컴파일) | Android 5.0 |
| 하이브리드 | **JIT + AOT + 프로파일 가이드 컴파일(PGO)** | Android 7.0(Nougat) |
| 배포 최적화 | **Baseline Profiles** | Jetpack 라이브러리 |

- **JIT+AOT 하이브리드(Android 7.0~)**: 앱 실행 중 JIT가 코드 사용 프로파일을 수집하고, 기기가 **유휴(idle)·충전 중**일 때 그 프로파일로 자주 쓰는 경로만 AOT 컴파일합니다. 설치 시간·저장공간·업데이트 속도를 개선합니다.
- **Baseline Profiles**: 자주 실행되는 클래스/메서드 목록을 APK/AAB에 번들하여 **설치 시점에 해당 경로를 미리 AOT 컴파일**합니다. 해석(interpret)·JIT 단계를 건너뛰어 **첫 실행부터 코드 실행 속도가 향상**되고 startup이 빨라집니다.

> 핵심: ART는 "전부 AOT"에서 "프로파일이 알려주는 핫패스를 AOT"로 진화했습니다. Baseline Profiles는 이 프로파일을 개발자가 미리 제공하는 메커니즘입니다.
> 근거: https://source.android.com/docs/core/runtime/jit-compiler , https://developer.android.com/topic/performance/baselineprofiles/overview

**Baseline Profiles의 정량 효과와 Cloud Profiles와의 차이 (다른 각도 보강)**

Baseline Profiles가 "첫 실행부터 빠른" 이유를 수치·배포 경로로 구체화하면 다음과 같다.

- **정량 효과**: 공식 문서는 Baseline Profiles가 포함 코드 경로의 **해석(interpret)·JIT 단계를 건너뛰어 코드 실행 속도를 "첫 실행부터 약 30%" 개선**한다고 명시한다("Baseline Profiles improve code execution speed by about 30% from the first launch"). DEX 레이아웃을 최적화하는 **Startup Profiles** 와 결합하면 시작 성능이 추가로 약 15%(대형 앱은 더) 향상된다.
- **배포 경로**: 사람이 읽을 수 있는 규칙이 빌드 시 바이너리 `baseline.prof`로 컴파일되어 **APK/AAB에 번들**되고, Google Play가 **설치 시점에 함께 배포**한다. 그 결과 ART가 설치 중 해당 메서드를 AOT 컴파일한다.

| 구분 | Baseline Profiles | Cloud Profiles |
| :--- | :--- | :--- |
| 출처 | 개발자가 빌드 시 동봉(앱에 번들) | 실제 사용자 실행 데이터를 Play가 집계 |
| 적용 시점 | **설치 즉시**(첫 실행부터 최적화) | 집계·배포까지 **수 시간~수일** 소요 |
| 대상 | 라이브러리/앱이 지정한 핫패스 | 실사용 패턴 기반 핫패스 |
| API 7~8.1 (24~27) | Baseline만 부분 AOT | 미지원 |
| API 9+ (28~) | Baseline 즉시 적용 | 이후 Cloud로 추가 최적화 |

> 핵심: Cloud Profiles는 "실사용 데이터를 모아 나중에" 최적화하므로 초기 사용자·앱 업데이트 직후에는 효과가 늦다. Baseline Profiles는 **그 공백을 설치 즉시 메워** 첫 실행부터 startup·스크롤 jank를 줄인다(본문 ART의 "설치 시 AOT" 철학을 핫패스 단위로 정밀화한 것).
> 근거: https://developer.android.com/topic/performance/baselineprofiles/overview

### 3. 센서 최신 패턴 — Lifecycle-aware 등록과 Coroutine 스트림

본문 §심화 예시 답안은 `onResume()`에서 `registerListener`, `onPause()`에서 `unregisterListener` 하는 전통 방식입니다. 현대 권장 패턴은 두 가지입니다.

**(1) Lifecycle-aware 등록/해제 (DefaultLifecycleObserver)**

생명주기 콜백을 액티비티에 흩뜨리지 않고, 옵저버가 스스로 등록/해제하도록 분리합니다.
...
// 등록: lifecycle.addObserver(AccelObserver(sensorManager, listener))
```

**(2) Coroutine `callbackFlow` 로 SensorEvent 스트림화**

콜백 기반 `SensorEventListener`를 Flow로 감싸 코루틴에서 안전하게 소비합니다. `awaitClose`에서 반드시 해제하여 누수를 막습니다.

```kotlin
fun SensorManager.accelerometerFlow(): Flow<SensorEvent> = callbackFlow {
    ...
    awaitClose { unregisterListener(listener) }   // 구독 종료 시 자동 해제
}
```

> 핵심: `callbackFlow { ... awaitClose { unregisterListener } }` 는 콜백 API를 Flow로 변환하는 표준 패턴이며, 구독이 취소되면 `awaitClose` 블록이 호출되어 해제가 보장됩니다.
> 근거: https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines.flow/callback-flow.html

**(3) 센서(Sensor) vs 위치(Location)의 구분**

| 구분 | 가속도/자이로 등 물리 센서 | 위치 |
| :--- | :--- | :--- |
| API | `SensorManager` + `SensorEventListener` | `FusedLocationProviderClient` |
| 소속 | Android 프레임워크(기기 내장) | Google Play services(Location API) |
| 데이터 | 가속도/회전/조도 등 원시 센서값 | GPS·Wi-Fi·셀·센서를 **융합한** 위경도 |
| 권한 | 보통 불필요(일부 예외) | 위치 권한 필요(`ACCESS_FINE/COARSE_LOCATION`) |

> 핵심: "센서"와 "위치"는 별개 시스템입니다. 위도/경도가 필요하면 `SensorManager`가 아니라 **FusedLocationProviderClient** 를 쓰며, 이는 여러 신호를 융합해 정확도·전력 효율을 높입니다.
> 근거: https://developer.android.com/develop/sensors-and-location/sensors/sensors_overview , https://developer.android.com/develop/sensors-and-location/location

**(3-1) 하드웨어 센서 vs 소프트웨어(가상) 센서 — 센서 fusion**

본문 §3의 센서 4종 클래스는 **하드웨어 센서**(가속도계·자이로·자기장계)를 다루지만, 안드로이드 센서 프레임워크는 두 부류로 나뉜다.

| 구분 | 하드웨어 센서 (physical) | 소프트웨어 센서 (virtual/synthetic/composite) |
| :--- | :--- | :--- |
| 정의 | 기기에 내장된 **물리 부품**이 환경을 직접 측정 | 물리 부품이 아니라 **하나 이상의 하드웨어 센서값을 융합·가공**해 산출 |
| 예 | `TYPE_ACCELEROMETER`, `TYPE_GYROSCOPE`, `TYPE_MAGNETIC_FIELD` | `TYPE_GRAVITY`, `TYPE_LINEAR_ACCELERATION`, `TYPE_ROTATION_VECTOR` |
| 데이터 | 가속도/각속도/지자기 등 **원시값** | 중력/순수가속도/방향(rotation vector) 등 **상위 의미값** |

- 공식 문서 인용: *"Software-based sensors ... derive their data from one or more of the hardware-based sensors and are sometimes called **virtual sensors or synthetic sensors**. The linear acceleration sensor and the gravity sensor are examples of software-based sensors."*
- **센서 fusion**: 가속도계는 "중력 + 운동 가속도"를 섞어 측정하므로, 시스템은 **가속도계 + 자이로 + 자기장계를 융합**해 다음을 분리·산출한다.
  - **`TYPE_GRAVITY`** — 중력 성분만(정지 시 약 9.81 m/s²)
  - **`TYPE_LINEAR_ACCELERATION`** — 중력을 제거한 **순수 운동 가속도**(`가속도 = 중력 + 선형가속도` 관계)
  - **`TYPE_ROTATION_VECTOR`** — 기기 **방향(orientation)** 을 회전 벡터로 표현(자기장계까지 융합하면 절대 방위 기준)

```kotlin
// 소프트웨어(융합) 센서 사용 — 코드 패턴은 하드웨어 센서와 동일
val rotation = sensorManager.getDefaultSensor(Sensor.TYPE_ROTATION_VECTOR)
if (rotation == null) {
    // 융합 센서 미지원 기기 → getRotationMatrix()/getOrientation()로 수동 융합
}
// onSensorChanged에서: SensorManager.getRotationMatrixFromVector(R, event.values)
```

> 핵심: `TYPE_ROTATION_VECTOR`·`TYPE_GRAVITY`·`TYPE_LINEAR_ACCELERATION`은 **물리 센서가 아니라 가속도+자이로+자기장 fusion으로 만든 가상(소프트웨어) 센서**다. 앞의 (3) 표에서 본 **FusedLocationProvider**(여러 위치 신호 융합)와 이름은 비슷하나, 이쪽은 **모션/방향 센서 융합**으로 전혀 다른 시스템임에 유의한다. 코드상으로는 둘 다 `SensorManager`+`SensorEventListener`로 동일하게 다룬다.
> 근거: https://developer.android.com/guide/topics/sensors/sensors_overview

**(4) `registerListener` 반환값과 만보계용 센서 타입(`TYPE_STEP_COUNTER` / `TYPE_STEP_DETECTOR`)**

`SensorManager.registerListener(...)`는 **`boolean`을 반환**한다. 등록에 성공하면 `true`, 해당 **센서가 기기에 없거나 등록에 실패하면 `false`** 를 돌려주므로, 반드시 `getDefaultSensor()`가 `null`인지 확인하거나 반환값을 점검하는 것이 안전하다. 만보계(걸음 수 측정) 시나리오에서는 두 가지 보행 센서를 쓸 수 있다: **`TYPE_STEP_COUNTER`** 는 기기 부팅 이후 **누적 걸음 수**를 반환(앱 시작 시점의 값을 기준으로 차감해 사용)하고, **`TYPE_STEP_DETECTOR`** 는 **한 걸음을 내디딜 때마다 이벤트 1건**을 발생시킨다.

```kotlin
val stepSensor = sensorManager.getDefaultSensor(Sensor.TYPE_STEP_COUNTER)
if (stepSensor == null) {
    // 기기에 만보 센서 없음 → 대체 로직(가속도 기반 추정 등)
} else {
    val ok = sensorManager.registerListener(listener, stepSensor, SensorManager.SENSOR_DELAY_NORMAL)
    if (!ok) { /* 등록 실패 처리 */ }
}
```

> 핵심: 만보계는 `TYPE_STEP_COUNTER`(누적값) 또는 `TYPE_STEP_DETECTOR`(걸음 이벤트)를 사용하며, 센서 미존재 시 `getDefaultSensor`가 `null`·`registerListener`가 `false`를 반환하므로 방어 코드가 필요하다.
> 근거: https://developer.android.com/guide/topics/sensors/sensors_overview

**(5) 🟢 고빈도 우회 경로 — `SensorDirectChannel`(API 26+)**

§심화 예시 답안의 "보완 1"에서 `SENSOR_DELAY_*`(FASTEST/GAME/UI/NORMAL)는 **고정값이 아니라 시스템이 조정할 수 있는 권장(요청) 주기**라고 했다. 콜백이 `onSensorChanged()`를 거쳐 **자바 프레임워크·메인 스레드를 경유**하기 때문에, VR·모션 트래킹처럼 **수백 Hz 이상**의 매우 높은 주기가 필요하면 이 일반 경로로는 한계가 있다.

이를 위해 **API 26(Android 8.0)부터 `SensorDirectChannel`** 이 도입되었다. 앱이 **공유 메모리(`MemoryFile`/하드웨어 버퍼)** 로 직접 채널을 열면, 센서 하드웨어가 **자바 콜백을 거치지 않고** 그 버퍼에 센서값을 직접 써 주므로 일반 `registerListener` 경로보다 **더 높고 안정적인 주기**(`RATE_VERY_FAST` 등)로 데이터를 받을 수 있다. `SensorManager.createDirectChannel(...)`로 채널을 만들고 `configure(sensor, rate)`로 주기를 설정한다.

```java
// 개념 골격 (API 26+): 공유 메모리 기반 다이렉트 채널
SensorDirectChannel channel = sensorManager.createDirectChannel(memoryFile);
channel.configure(accelSensor, SensorDirectChannel.RATE_VERY_FAST); // 일반 SENSOR_DELAY_* 우회
// ... 공유 버퍼에서 직접 센서값 읽기 ...
channel.close();
```

> 핵심: 일반 `SENSOR_DELAY_*`는 "권장 주기"이고 콜백이 프레임워크를 경유하지만, `SensorDirectChannel`은 **공유 메모리로 자바 콜백을 우회**해 고빈도 센서 데이터를 받는 API 26+ 기능이다. 일반 앱 대부분은 `registerListener`로 충분하고, 초고빈도가 꼭 필요한 경우에만 사용한다.
> 근거: https://developer.android.com/reference/android/hardware/SensorDirectChannel
...
- Android Developers — Sensors Overview (하드웨어/소프트웨어 센서, fusion): https://developer.android.com/guide/topics/sensors/sensors_overview
- Android Open Source — Architecture (Project Treble): https://source.android.com/docs/core/architecture
- Android Open Source — VNDK: https://source.android.com/docs/core/architecture/vndk
- Android Developers — Baseline Profiles (첫 실행 약 30% 개선): https://developer.android.com/topic/performance/baselineprofiles/overview
