...
**패키지 가시성(package visibility)**: Android 11(API 30)부터 **타깃 SDK 30 이상** 앱은 기기에 설치된 다른 앱 목록이 기본적으로 **필터링(제한)** 된다. 이때 주의할 점은 `startActivity()` 자체는 가시성 없이도 다른 앱의 액티비티를 실행할 수 있지만, **`resolveActivity()`·`queryIntentActivities()`는 가시 범위 밖 앱을 못 찾아 null/빈 목록을 반환**할 수 있다는 것이다. 공식 권장은 "`Intent.resolveActivity()`로 미리 확인하기보다 `startActivity()`를 호출하고 `ActivityNotFoundException`을 처리"하는 방식이다. 특정 앱과 직접 상호작용해야 하면 `<queries>`로 선언한다.
