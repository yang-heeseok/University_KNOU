...
## 📝 요약 (Summary)
1. 🔴 **LinearLayout**: 차일드(자식) View를 **일렬로 배치**하는 레이아웃. `orientation` 속성에 따라 세로(vertical) 또는 가로(**horizontal**: default) 방향으로 자식들이 순차 배치된다. 📎 기출 빈출
...
5. 🟡 **`baselineAligned` 속성**: 수평으로 배치된, **서로 높이가 다른 다수의 TextView**를 배치했을 때 TextView **아래쪽(baseline)을 기준**으로 텍스트 뷰들을 정렬해 주는 속성. 기본값 `true`. 🟡 **실전 함정**: 기본값이 `true`이므로 가로 LinearLayout에 **ImageView + TextView를 섞어 배치**하면, ImageView에는 텍스트 baseline이 없어(기본 baseline = -1) 두 뷰의 정렬 기준이 어긋나 이미지가 위/아래로 떠 보일 수 있다. 이 경우 부모 LinearLayout에 `android:baselineAligned="false"`를 주면 baseline 정렬을 끄고 `layout_gravity`/`gravity`로 의도대로 맞출 수 있다.
...
| **`gravity`** | View **내부 내용물**(텍스트/이미지/자식들)의 **정렬 위치**. ViewGroup에 쓰면 자식들 전체의 정렬, TextView에 쓰면 글자의 정렬. ⚠️ LinearLayout에 쓸 때는 **방향(orientation)과 직교하는 성분만 의미**가 있다: `vertical`이면 자식이 이미 세로로 쌓여 있어 **수평 정렬 성분**(start/center_horizontal/end)만, `horizontal`이면 **수직 정렬 성분**(top/center_vertical/bottom)만 자식 묶음에 적용된다(예: vertical에 `gravity="bottom"` → 자식 묶음 전체가 아래로 모임). 같은 방향 성분은 자식들이 이미 그 축으로 배열돼 있어 효과가 미미하다 — 시험 함정 포인트 | 내부 정렬, center/start/end/top/bottom |
| **`layout_gravity`** | 자기 자신이 **부모 안에서** 어느 위치에 배치될지. LinearLayout 자식만 의미 있음. ⚠️ `gravity`가 orientation과 직교 성분만 적용되는 것과 **대칭**으로, 자식의 `layout_gravity`도 **orientation과 직교하는 축에서만 동작**한다: `vertical` LinearLayout의 자식은 `layout_gravity`로 **수평 위치**(start/center_horizontal/end)만 제어할 수 있고, `horizontal` LinearLayout의 자식은 **수직 위치**(top/center_vertical/bottom)만 제어할 수 있다(배치 축 방향 성분은 무시됨). 근거: https://developer.android.com/reference/android/widget/LinearLayout.LayoutParams#attr_android:layout_gravity | 자기 위치, 부모 내 정렬, 직교 축 |
...
### 3. 🟢 심층 확장 — `weightSum` 부분 점유 수치 예제 & ConstraintLayout 평탄화

> 🟢 **2차 심층 확장**: 아래 두 주제는 공식 문서로 검증한 보강이다. (1) `weightSum`을 활용한 "부분 점유" 수치 예제, (2) LinearLayout 중첩 대비 ConstraintLayout 평탄화의 measure 성능 이점.

#### (1) `weightSum` 수치 예제 — "화면의 정확히 1/4만 점유" 만들기

`layout_weight`만으로 비율을 나눌 때는 **자식들의 weight 합이 분모**가 된다. 즉 자식이 1개뿐이고 그 자식에 `weight=1`을 주면 분모도 1이라 그 자식이 **남는 공간 전부(100%)** 를 차지한다. 그런데 LinearLayout에 `android:weightSum="4"`를 명시하면, 분모가 자식 weight 합이 아니라 **고정값 4**로 바뀌어, 자식 1개(`weight=1`)는 전체의 **1/4만 점유**하고 나머지 3/4은 **빈 공간**으로 남는다. 근거: https://developer.android.com/reference/android/widget/LinearLayout#attr_android:weightSum

| 구분 | `weightSum` 미지정 (자식 weight 합이 분모) | `weightSum="4"` 지정 (분모 고정) |
| :--- | :--- | :--- |
| **자식 1개, weight=1** | 분모 = 1 → 자식이 **남는 공간 100% 점유** | 분모 = 4 → 자식이 **남는 공간의 1/4만 점유**, 3/4은 빈 공간 |
| **자식 2개, weight=1,1** | 분모 = 2 → 각 자식 **1/2씩** | 분모 = 4 → 각 자식 **1/4씩**, 나머지 2/4는 빈 공간 |
| **자식 3개, weight=1,1,1** | 분모 = 3 → 각 자식 **1/3씩** | 분모 = 4 → 각 자식 **1/4씩**, 나머지 1/4은 빈 공간 |
| **공식** | `자식 점유 = (자식 weight) / (Σ 자식 weight)` | `자식 점유 = (자식 weight) / weightSum` (단, Σ weight ≤ weightSum일 때) |

```xml
<!-- 세로 화면 높이의 정확히 1/4만 채우는 헤더 배너 -->
<LinearLayout
    ...
    <!-- weight=1 → weightSum=4의 1/4 = 화면 높이의 25%만 차지 -->
    <View
        android:layout_width="match_parent"
        android:layout_height="0dp"
        android:layout_weight="1"
        android:background="#3F51B5" />
    <!-- 나머지 3/4(높이 75%)는 자식이 더 없으므로 빈 공간으로 남음 -->
</LinearLayout>
```

> **수치 검증**: 화면 높이가 800dp이고 위 예제의 `View`만 있다면, `weightSum=4`이므로 그 View의 높이는 `800 × (1/4) = 200dp`가 되고, 아래 600dp는 비어 있다. 만약 `weightSum`을 빼면 분모가 자식 weight 합(=1)이 되어 같은 View가 `800 × (1/1) = 800dp`, 즉 **화면 전체**를 채운다. "부분 점유 배너/사이드 영역"을 만들 때 `weightSum`이 핵심 도구다.

#### (2) LinearLayout 중첩 vs ConstraintLayout 평탄화 (measure 성능)

깊게 중첩된 LinearLayout(특히 `layout_weight`를 쓰는 LinearLayout)은 자식 크기를 정하기 위해 **이중 측정(double taxation)** 이 발생해 measure 패스가 늘어난다. ConstraintLayout은 동일한 화면을 **플랫(평탄)한 단일 계층**으로 표현해 중첩 ViewGroup을 제거하고, 대부분의 경우 추가 레이아웃 패스를 줄인다. 공식 문서는 "성능과 도구 지원을 위해 RelativeLayout/중첩 레이아웃 대신 ConstraintLayout으로 빌드할 것"을 권장한다. 근거: https://developer.android.com/develop/ui/views/layout/constraint-layout · https://developer.android.com/develop/ui/views/layout/improving-layouts/optimizing-layouts

| 구분 | 중첩 LinearLayout (+ weight) | ConstraintLayout (평탄) |
| :--- | :--- | :--- |
| **계층 깊이** | 행/열 조합마다 LinearLayout을 중첩 → 깊어짐 | 단일 ConstraintLayout 한 겹으로 표현 가능 |
| **measure 패스** | weight LinearLayout은 자식을 2번 측정(이중 과세) | 제약 해석 1회 위주, 추가 패스 최소화 |
| **권장도** | 단순 1차원 줄세우기에는 충분 | 복잡한 2차원 관계 배치에 권장 |
...
> - ③: weight가 1:1:1이라 1:2:1이 안 됨, base도 match_parent로 부적절. 게다가 가로 LinearLayout에서 자식 `width="match_parent"` + `weight`를 함께 주면, 측정의 base가 0이 아니라 **부모 폭 전체**가 되어 남는 공간이 음수가 되고, 이 음수 공간이 weight 비율로 분배되면서 (자식 개수·`weightSum`·각 자식의 본래 폭에 따라) **weight가 클수록 오히려 더 좁아지는(가중치의 역수 비율) 분배 역전이 발생할 수 있다**. 어느 경우든 의도한 정확한 비율 분배는 깨지므로 권장되지 않는다. 근거: https://developer.android.com/develop/ui/views/layout/linear
...
> <a id="ox-19"></a>
> **19. 정답**: O
> **해설**: `layout_marginStart`/`End`는 RTL 환경에서 자동으로 좌우를 뒤바꿔 적용한다. `layout_marginLeft`/`Right`는 절대 방향이므로 RTL 대응에 불리하다. ⚠️ 단, 이 자동 좌우 반전(RTL 미러링)이 실제로 동작하려면 **`AndroidManifest.xml`의 `<application android:supportsRtl="true">`** 가 켜져 있고, 디바이스가 **API 17(Android 4.2, Jelly Bean MR1) 이상**이어야 한다는 전제가 필요하다. 그 이전 버전이나 `supportsRtl`이 꺼진 경우에는 Start/End가 사실상 Left/Right로 고정된다. 근거: https://developer.android.com/training/basics/supporting-devices/languages
