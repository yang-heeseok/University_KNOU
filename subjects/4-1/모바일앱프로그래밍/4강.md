...
10. 🟢 **textAllCaps**: Button의 표시 텍스트를 자동으로 대문자로 변환할지 여부(`true`/`false`). Material Theme에서 기본 `true`로 동작하던 시절의 잔재.
...
| **Switch** | `<Switch android:id="@+id/sw" android:text="알림" android:checked="true" />` | On/Off 토글. AppCompat 사용 시 `SwitchCompat` |
...
> <a id="ox-13"></a>
> **13. 정답**: O
> **해설**: `android:onClick`의 값은 Activity의 `public void 메서드명(View v)` 시그니처 메서드 이름과 정확히 일치해야 하며, 없으면 런타임 예외 발생.
...
- **Bad Practice (나쁜 예시)**
  ...
  <!-- Button onClick 핸들러명 오타 → 런타임 IllegalStateException -->
