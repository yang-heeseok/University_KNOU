...
| **fitStart** | fitCenter처럼 축소하되 **좌상단** 정렬 | O | 한 변만 채움 | X | 좌측 정렬 갤러리 |
| **fitEnd** | fitCenter처럼 축소하되 **우하단** 정렬 | O | 한 변만 채움 | X | 우측 정렬 갤러리 |
...
| :--- | :--- | :--- |
| **API 상태** | API 3 이후 **deprecated** | 현재 권장 |
| **값** | `"true"`/`"false"` (한 줄/여러 줄) | 정수(`"1"`, `"3"` 등) |
| **줄바꿈 동작** | 줄바꿈 강제 차단 | 지정한 줄 수까지 허용 |
| **권장** | 신규 코드에서는 사용 자제 | `android:maxLines="1"` + `android:ellipsize="end"` |
...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        title.setTextSize(28);  // 단위는 sp가 기본(TypedValue.COMPLEX_UNIT_SP)
