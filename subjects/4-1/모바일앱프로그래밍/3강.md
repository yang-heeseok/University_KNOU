...
> 🟠 **"ImageView 8대 속성"은 교재 핵심 속성일 뿐, 전체 목록이 아님**: 본 강의에서 정리하는 8대 속성(`src`/`maxWidth`/`maxHeight`/`minWidth`/`minHeight`/`adjustViewBounds`/`cropToPadding`/`scaleType`)은 **KNOU 교재가 시험 범위로 다루는 핵심 속성**이며, 실제 `android.widget.ImageView`가 지원하는 XML 속성은 이 8개 외에도 `android:tint`(이미지에 색조 입히기)·`android:tintMode`·`android:baselineAlignBottom`·`android:baseline`·`app:srcCompat`(위 표 참조)·`android:foreground` 등 더 많다. 즉 "8개가 전부"가 아니라 "8개가 핵심"으로 이해해야 한다. 근거: [Android Developers — ImageView](https://developer.android.com/reference/android/widget/ImageView)
