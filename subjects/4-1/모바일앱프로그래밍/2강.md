...
> ※ 🟡 **`clickable` vs `focusable` 구분**: 상호작용 속성에서 둘은 별개다. **`clickable`** 은 View가 **클릭 이벤트를 수신할 수 있는지**(탭하면 `OnClickListener`가 호출되는지)를 결정하고, **`focusable`** 은 View가 **포커스를 획득할 수 있는지**(TAB/방향키/D-pad로 선택 대상이 되는지)를 결정한다. 즉 클릭은 받지만 포커스는 안 받게(또는 그 반대로) 따로 설정할 수 있다. 예컨대 일반 텍스트 라벨은 `clickable=false, focusable=false`, 버튼은 둘 다 `true`가 기본이다. (접근성·키보드 내비게이션에서는 `focusable`이 핵심)
...
#### 🟡 gravity vs layout_gravity (padding/margin과 같은 "누가 해석?" 원리)

> 위 padding vs margin의 핵심이 **`layout_` 접두사 = 부모 ViewGroup이 해석**이었다면, `gravity`와 `layout_gravity`도 정확히 같은 원리로 구분된다. 이름이 비슷해 가장 많이 혼동되는 쌍이므로 명시적으로 대비한다.

| 구분 | `android:gravity` | `android:layout_gravity` |
| :--- | :--- | :--- |
| **정렬 대상** | **View 자신의 내부 콘텐츠**(텍스트·자식 View 등)를 View 영역 안에서 정렬 | **부모 ViewGroup 안에서 이 View 자신의 위치**를 정렬 |
| **누가 해석?** | **View 자기 자신**이 해석 (자신의 콘텐츠 배치) | **부모 ViewGroup**이 해석 (`layout_` 접두사 — 부모가 자식의 위치를 정함) |
| **접두사** | `layout_` **없음** | `layout_` **있음** (LayoutParams 속성) |
| **예시** | `android:gravity="center"` → TextView 안의 글자가 가운데로 | `android:layout_gravity="center"` → LinearLayout 안에서 이 View가 가운데로 |
| **비유** | "액자 안에서 그림(콘텐츠)을 어디에 둘까" | "벽(부모) 어디에 액자(이 View)를 걸까" |

> **암기 포인트**: `layout_` 접두사가 붙으면 무조건 **부모 ViewGroup**이 해석하는 LayoutParams 속성이다. 따라서 `layout_margin`·`layout_width`·`layout_gravity`는 모두 "부모와의 관계/부모 안에서의 배치"를 뜻하고, 접두사 없는 `padding`·`gravity`는 "View 자신이 자기 내부를 처리"하는 속성이다. 단, `layout_gravity`는 부모 ViewGroup이 이를 지원해야 의미가 있으며(예: LinearLayout·FrameLayout은 지원, 일부 ViewGroup은 무시), LinearLayout에서는 orientation에 수직인 축의 정렬에만 적용된다. 근거: [Android Developers — LinearLayout.LayoutParams](https://developer.android.com/reference/android/widget/LinearLayout.LayoutParams)
...
## 🟢 2차 심층 확장 (Deep Dive Ⅱ) — `findViewById` → ViewBinding 마이그레이션 실무

> 본 절은 리서치 기반 2차 심층 확장으로, 앞 Deep Dive에서 "왜 ViewBinding이 필요한가(타입·null 안전)"를 다뤘다면, 여기서는 **실제 마이그레이션 절차와 synthetic 폐기 타임라인**을 공식 가이드 기준으로 보강한다. (앞 절의 Compose Modifier 대응과 중복되지 않도록 ViewBinding 실무에 집중) 모든 주장은 공식 문서로 검증하였다.

### 1. `kotlinx.android.synthetic` 폐기 → ViewBinding 이전 타임라인

본문 "코드로 확인하기"의 `findViewById` 방식과 더불어, 과거 Kotlin에서 널리 쓰이던 **Kotlin synthetics**(`import kotlinx.android.synthetic.main.activity_main.*` 후 id를 변수처럼 사용)는 다음 경로로 **공식 폐기**되었다.

| 시점 | 사건 | 의미 |
| :--- | :--- | :--- |
| 2020년 11월 | Kotlin Android Extensions **deprecated 발표** | synthetic 사용 비권장 공식화 |
| 2022년 2월 | "Discontinuing Kotlin synthetics for views" 블로그 | **ViewBinding으로 이전 공식 권고** |
| Kotlin 1.8 (2022년 말) | `kotlin-android-extensions` 플러그인 사실상 제거 단계 | 신규 프로젝트에서 synthetic 사용 불가 흐름 |

- **폐기 사유(공식 정리)**: synthetic은 ① **타입 비안전**(잘못된 캐스팅 가능), ② **null 비안전**(다른 레이아웃의 id까지 자동완성에 노출되어 런타임 NPE 유발), ③ **전역 네임스페이스 오염**, ④ **모듈 코드만 노출** 등의 구조적 한계가 있었다. 공식 대안은 **ViewBinding**이며, 이는 **XML 레이아웃마다 타입 안전한 바인딩 클래스를 생성**하고 다중 구성(configuration) 레이아웃에서도 안전하게 동작한다.
- 근거: [Android Developers Blog — Discontinuing Kotlin synthetics for views](https://android-developers.googleblog.com/2022/02/discontinuing-kotlin-synthetics-for-views.html), [Android Developers — Migrate from Kotlin synthetics to Jetpack view binding](https://developer.android.com/topic/libraries/view-binding/migration)

### 2. 마이그레이션 4단계 (공식 가이드)

공식 마이그레이션 가이드는 다음 4단계를 제시한다.

| 단계 | 작업 | 코드 |
| :--- | :--- | :--- |
| **① 활성화** | 모듈 `build.gradle`에 ViewBinding 켜기 | `buildFeatures { viewBinding = true }` |
| **② import 제거** | `kotlinx.android.synthetic.*` import 전부 삭제 | (해당 줄 삭제) |
| **③ 바인딩 인플레이트** | Activity는 `onCreate()`, Fragment는 `onCreateView()`에서 바인딩 생성 | `binding = ActivityMainBinding.inflate(layoutInflater)` |
| **④ 참조 치환** | `name.text` → `binding.name.text` 처럼 바인딩 인스턴스 경유로 변경 | id `tv_demo`(snake) → `binding.tvDemo`(camel) |

```kotlin
// ② synthetic 방식 (폐기) — id를 전역 변수처럼 직접 사용
// import kotlinx.android.synthetic.main.activity_main.*
// tvDemo.text = "Hello"     // ← 타입/null 비안전, 다른 레이아웃 id도 노출

// ③④ ViewBinding 방식 (권장)
class MainActivity : AppCompatActivity() {
    private lateinit var binding: ActivityMainBinding        // 자동 생성 클래스
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityMainBinding.inflate(layoutInflater) // ③ 인플레이트
        setContentView(binding.root)
        binding.tvDemo.text = "Hello"                        // ④ binding. 경유 (타입·null 안전)
    }
}
```

> 📌 **Fragment 주의**: Fragment는 View 생명주기가 Fragment 자신보다 짧으므로, `onDestroyView()`에서 `binding = null`로 해제(누수 방지)하는 패턴(보통 `_binding` nullable + `binding` non-null getter)을 함께 쓴다. 이는 synthetic 시절에는 신경 쓸 수 없던 부분으로, ViewBinding이 **View 참조 수명까지 명시적으로 관리**하게 해 준다.
> 근거: [Android Developers — View binding (Use view binding in fragments)](https://developer.android.com/topic/libraries/view-binding#fragments)

> **2강 연결**: 본문에서 익힌 **`android:id`(7대 속성 중 식별 속성)** 가 바로 ViewBinding의 출발점이다. id가 있는 View만 바인딩 클래스의 프로퍼티로 노출되므로, "id를 잘 붙이는 습관"이 곧 안전한 View 접근의 토대가 된다.
...
- Android Developers - Migrate from Kotlin synthetics to Jetpack view binding (마이그레이션 4단계): https://developer.android.com/topic/libraries/view-binding/migration
- Android Developers Blog - Discontinuing Kotlin synthetics for views (2022.02): https://android-developers.googleblog.com/2022/02/discontinuing-kotlin-synthetics-for-views.html
