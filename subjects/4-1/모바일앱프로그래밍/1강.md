...
5. 🔴 **ART (Android RunTime)**: 앱을 설치하기 전(AOT, Ahead-Of-Time)에 컴파일을 끝내고 앱을 실행하는 안드로이드 스마트폰의 가상머신. **Android 4.4(KitKat)에서 개발자 옵션으로 실험적 도입 → Android 5.0(Lollipop)부터 기본 런타임으로 채택**.
...
> 🟠 **"Dalvik VM = JIT" 표기 보충(역사적 정밀화)**: 위 핵심 정의 표와 요약(4번 항목)에서 Dalvik을 "JIT 컴파일 기반"으로 정리했지만, 엄밀히는 **Dalvik도 처음부터 JIT였던 것은 아니다.** 초기(Android 2.1 이전)의 Dalvik은 `.dex` 바이트코드를 실행 시점에 한 줄씩 해석하는 **순수 인터프리터(interpreter)** 였고, **JIT(Just-In-Time) 컴파일러는 Android 2.2(Froyo)에서 처음 도입**되어 자주 실행되는 핫(hot) 코드 경로를 기계어로 변환하면서 성능이 크게 향상되었다. 따라서 "Dalvik = JIT"라는 표기는 **2.2 이후의 Dalvik**을 가리키는 표현으로 이해해야 한다. 근거: [Wikipedia — Android Runtime](https://en.wikipedia.org/wiki/Android_Runtime)
...
>
> 🟡 **상태 보존 각주(`onSaveInstanceState`/`onRestoreInstanceState`)**: 위 7개 콜백 외에, 화면 회전 같은 **구성 변경(configuration change)** 으로 Activity가 소멸·재생성될 때 임시 UI 상태를 잃지 않도록 시스템은 `onSaveInstanceState(Bundle)`로 상태를 저장하고(주로 onStop 전후), 재생성 시 `onRestoreInstanceState(Bundle)`(또는 onCreate의 `savedInstanceState` 인자)로 복원한다. 이 둘은 정상적인 매 전이마다 호출되는 7대 콜백과 달리, **시스템이 필요하다고 판단할 때만** 호출되는 보조 콜백이다. 근거: [Android Developers — The activity lifecycle](https://developer.android.com/guide/components/activities/activity-lifecycle)
...
| `.apk` 설치 시 | **ART**가 AOT 컴파일 | `.oat` 기계어 파일 | 디바이스별 최적화 (※각주) |
| `.oat` 실행 | CPU 직접 실행 | 앱 동작 | Dalvik은 JIT라 이 단계가 실행 중 발생 |

> ※ 🟠 **현대 ART는 순수 AOT가 아니다**: Android 7.0(Nougat) 이후의 ART는 위 표처럼 설치 시 무조건 전부 AOT 컴파일하는 방식이 아니라, **인터프리터 + JIT + 프로파일 기반(profile-guided) AOT의 하이브리드**로 동작한다. 설치 직후엔 인터프리터/JIT로 실행하면서 자주 쓰이는 코드 경로를 프로파일로 수집하고, 기기가 충전·유휴 상태일 때 그 경로만 골라 AOT 컴파일한다. 즉 "설치 시 전체 AOT"는 5.0~6.0 모델이고, 7.0+는 더 똑똑한 혼합 방식이다. 근거: [Android Developers — Android Runtime (ART) and Dalvik](https://source.android.com/docs/core/runtime)
...
#### 1-보충. Dalvik / ART 런타임의 정체와 역사 (왜 자바 JVM을 안 쓰나)

> 위 빌드 과정에 등장한 **Dalvik(`.dex`)**·**ART(`dex2oat`)** 가 정확히 무엇인지 배경을 보강한다. (상세 비교는 아래 2번 표)

- **Dalvik이란**: 구글이 **안드로이드 앱을 실행하기 위해 직접 만든 가상 머신(VM)** 이다. 구글 엔지니어 **댄 본스타인(Dan Bornstein)** 이 설계했고, 이름은 그의 조상이 살던 **아이슬란드 어촌 마을 "Dalvík"** 에서 따왔다(기술적 의미 없음). 각 앱은 자기만의 VM 인스턴스(프로세스)에서 **격리**되어 실행된다.
- **왜 표준 자바 JVM을 그대로 안 썼나**: 데스크톱/서버용 JVM은 메모리·배터리가 넉넉한 환경 전제다. 구글은 **모바일의 제약(적은 RAM·배터리)** 에 맞춰, 여러 `.class`를 하나의 **`.dex`(Dalvik Executable)** 로 통합하고 **레지스터 기반** 구조(스택 기반 JVM과 대비)를 채택해 모바일 CPU에 더 적합하게 새로 설계했다.
- **"엔진"이라는 표현**: Dalvik·ART를 흔히 "엔진"이라 부르지만, 정확한 분류 용어는 **VM(가상 머신)** 또는 **런타임(Runtime)** 이다. 특히 **ART = Android RunTime** 으로, 이름 자체에 "런타임"이 들어 있다. 둘 다 "앱을 구동하는 핵심 실행기"라는 점에서 엔진이라 통칭해도 의미는 통한다.

**런타임 교체의 역사 (Dalvik → ART, 둘 다 구글 제작):**

| 시기 | 안드로이드 버전 | 런타임 | 비고 |
| :--- | :--- | :--- | :--- |
| 2008~ | 초기 ~ 4.x | **Dalvik** | JIT(실행 중 컴파일) 방식 |
| 2013 | 4.4 KitKat | ART **실험적** 도입 | 개발자 옵션으로 선택 가능 |
| **2014~** | **5.0 Lollipop 이후** | **ART 완전 대체** | Dalvik 제거, 현재 표준 |

> **핵심**: 현재 안드로이드는 **Dalvik이 아니라 ART**를 쓴다. 가장 큰 차이는 **컴파일 시점** — Dalvik은 실행 중에 그때그때 변환하는 **JIT**, ART는 **설치 시 미리 기계어로 변환(AOT, `dex2oat`)** 해 두어 실행이 빠르고 배터리에 유리하다(이후 버전은 AOT+JIT/프로파일 가이드 혼합). 즉 ART는 Dalvik의 **후계자**다.
...
## 🟢 2차 심층 확장 (Deep Dive Ⅱ) — 배포 포맷(AAB) 전환과 앱 프로세스 시작(Zygote)

> 본 절은 리서치 기반 2차 심층 확장으로, 1강 "빌드 → 실행 흐름"의 **배포 단계(APK/AAB)** 와 **앱 실행(프로세스 시작) 단계**를 최신 동향·내부 구조로 보강한다. 본문 빌드 표의 `.apk`/`.aab` 패키징과 "Zygote에서 fork한 새 프로세스에 로드"(심화 학습 1번 6항)에 직접 대응한다. 모든 주장은 공식 문서로 검증하였다.

### 1. AAB(Android App Bundle) vs APK — Play 스토어의 신규 앱 의무 포맷

1강 본문(핵심 정의 표·용어집)은 APK를 "단일 설치 파일", AAB를 "Play Store가 디바이스별 최적화 APK를 생성하는 신형 포맷"으로 정리했다. 이를 **현재 배포 표준** 관점에서 보강한다.

- **신규 앱 AAB 의무화(2021년 8월)**: Google Play는 **2021년 8월부터 신규 앱(new app)의 게시 포맷으로 AAB를 의무화**했다. 즉 신규 앱은 더 이상 APK를 직접 업로드할 수 없고 `.aab`를 업로드해야 한다. (이미 등록된 기존 앱은 적용 시점이 다름)
- **Dynamic Delivery(동적 전송)**: 개발자는 **전체 코드·리소스를 담은 단일 AAB** 하나만 업로드하고, **APK 생성·서명은 Google Play로 위임**한다. Play는 이 번들로부터 **기기 구성(화면 밀도·CPU ABI·언어 등)별로 최적화된 APK**(split APK)를 생성·서빙하므로, 사용자는 자기 기기에 필요한 코드·리소스만 내려받는다. 그 결과 다운로드 용량이 **유니버설 APK 대비 평균 약 15% 작아진다**.
- **Play App Signing(서명 키 관리)**: AAB 방식에서는 **2021년 8월 이후 모든 신규 앱이 Play App Signing을 사용**한다. 개발자는 **업로드 키**로 번들에 서명해 올리고, 최종 사용자에게 배포되는 APK의 **앱 서명 키는 Google이 안전하게 보관·관리**한다(키 분실·유출 위험 완화). 또한 AAB + Play Asset/Feature Delivery로 **150MB를 초과하는** 에셋/기능을 전송할 수 있다.

#### APK vs AAB 비교표

| 구분 | APK (Android Package) | AAB (Android App Bundle) |
| :--- | :--- | :--- |
| **확장자/성격** | `.apk` — 그 자체로 설치 가능한 **완성 패키지** | `.aab` — 설치 불가, **게시(publishing)용 번들** |
| **APK 생성 주체** | 개발자(빌드 시 최종 APK 완성) | **Google Play**(번들로부터 기기별 APK 생성) |
| **기기별 최적화** | 보통 유니버설 1개(모든 기기 공통) | **Dynamic Delivery**로 밀도·ABI·언어별 split APK |
| **다운로드 크기** | 큼(불필요 리소스 포함) | 평균 **약 15% 작음**(필요분만 전송) |
| **서명 키 관리** | 개발자가 직접 보관 | **Play App Signing**(Google이 앱 서명 키 보관) |
| **신규 앱 요건** | 직접 업로드 불가(신규) | **2021년 8월부터 신규 앱 의무** |

> **빌드 흐름 관점**: 본문 빌드 표의 "`.dex` + 리소스 + Manifest → `.apk` 또는 `.aab`" 단계에서, **AAB를 선택하면 최종 APK 분할·서명·생성이 Play 서버 측으로 이동**한다. 즉 "디바이스별 최적화 APK"라는 본문 각주(빌드 표 4행 "디바이스별 최적화")가 ART의 `dex2oat`(설치 시 기계어 최적화)뿐 아니라, **배포 단계에서도(Play의 split APK 생성)** 이중으로 일어나는 셈이다.
> 근거: [Android Developers — About Android App Bundles](https://developer.android.com/guide/app-bundle), [Android Developers — Android App Bundle FAQ](https://developer.android.com/guide/app-bundle/faq)

### 2. Zygote 프로세스 fork 구조 — 앱은 "복제되어" 시작한다

1강 심화 학습(예시 답안 1번 6항)에서 "시스템이 LAUNCHER Activity를 찾아 **Zygote(앱 프로세스 템플릿)에서 fork한 새 프로세스**에 로드"한다고 짧게 언급했다. 이 Zygote의 정체와 이점을 보강한다.

- **Zygote란**: 시스템 **부팅 시 시작되는 특별한 프로세스**로, **공통 프레임워크 코드·리소스(액티비티 테마 등)와 자주 쓰이는 클래스(preloaded classes)** 를 미리 로드해 둔 "원형(原型) 프로세스"다. (이름 Zygote = 생물학의 "접합자/수정란" — 모든 앱 프로세스가 여기서 갈라져 나옴)
- **fork로 앱 시작**: 새 앱을 띄울 때 시스템은 **Zygote를 `fork()`** 하여 자식 프로세스를 만들고, 그 위에 해당 앱의 코드를 로드·실행한다. 클래스 초기화·프레임워크 로딩을 매번 처음부터 하지 않으므로 **앱 시작 속도가 빨라진다**.
- **메모리 공유(copy-on-write) 이점**: fork 직후 자식은 부모(Zygote)의 메모리 페이지를 **공유**한다. 따라서 **프레임워크 코드·리소스를 위한 RAM 페이지 대부분이 모든 앱 프로세스에 걸쳐 공유**되어, 앱마다 같은 라이브러리를 중복 적재하지 않아 **전체 메모리 사용량이 절감**된다. 각 자식은 독립 PID·cgroup을 가져 **프로세스 격리**도 유지된다.

```
[부팅 시]
  init → Zygote 시작
         └─ 공통 프레임워크 클래스/리소스 preload (1회)

[앱 실행 시]  (앱 A, 앱 B, 앱 C …)
  Zygote ──fork()──▶ App A 프로세스 (공유 페이지 + 앱 A 코드)
         ──fork()──▶ App B 프로세스 (공유 페이지 + 앱 B 코드)
         ──fork()──▶ App C 프로세스 (공유 페이지 + 앱 C 코드)
   ▲ 프레임워크 코드/리소스 RAM 페이지는 COW로 공유 → 메모리 절감·시작 가속
```

| 항목 | Zygote fork 방식의 효과 |
| :--- | :--- |
| **시작 속도** | 프레임워크/preload 클래스 초기화를 앱마다 반복하지 않음 → 빠른 콜드 스타트 |
| **메모리** | 공통 코드·리소스 페이지를 COW로 공유 → 앱 수가 늘어도 중복 적재 최소화 |
| **격리** | fork 후 별도 PID·cgroup → 한 앱의 크래시가 다른 앱에 전파되지 않음 |

> **요점(1강 연결)**: 1강이 다룬 "ART가 `.dex`를 `dex2oat`로 컴파일"하는 것이 **실행 코드의 준비**라면, Zygote fork는 **그 코드를 올릴 프로세스의 준비**다. "한 번 준비해 두고 복제해서 나눠 쓴다"는 사상은 본문에서 강조한 **리소스 공유·재사용**(XML 분리, 리소스 한정자) 원리와 같은 맥락이다.
> 근거: [Android Open Source Project — About the Zygote processes](https://source.android.com/docs/core/runtime/zygote), [Android Developers — Overview of memory management](https://developer.android.com/topic/performance/memory-overview)
...
- Android Developers - About Android App Bundles (AAB/Dynamic Delivery): https://developer.android.com/guide/app-bundle
- Android Developers - Android App Bundle FAQ (신규 앱 의무화·Play App Signing): https://developer.android.com/guide/app-bundle/faq
- Android Open Source Project - About the Zygote processes: https://source.android.com/docs/core/runtime/zygote
- Android Developers - Overview of memory management (Zygote 공유 메모리): https://developer.android.com/topic/performance/memory-overview
