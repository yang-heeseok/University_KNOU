...
- **오개념 분석**
    ...
    - **분석**: 잘못된 생각이다. 모든 Activity, Service, Broadcast Receiver는 **반드시 Manifest에 `<activity>` 등으로 선언**되어야 시스템이 해당 컴포넌트의 존재를 인식한다(단, ContentProvider는 정적 등록 필수, BroadcastReceiver만 동적 등록 가능). 등록하지 않은 Activity를 `startActivity()`로 호출하면 `ActivityNotFoundException`이 발생한다.
...
> <a id="ox-2"></a>
> **2. 정답**: X
> **해설**: Manifest에 등록되지 않은 Activity를 호출하면 `ActivityNotFoundException`이 발생한다. 모든 4대 컴포넌트는 Manifest에 선언되어야 하며(BroadcastReceiver만 동적 등록 가능), 시스템이 인식하지 못한다.
