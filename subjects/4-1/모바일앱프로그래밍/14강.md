...
5. 🟡 **TimePickerDialog**: TimePicker 위젯을 이용해 사용자가 손쉽게 **시간 정보(시/분)** 를 입력할 수 있는 인터페이스를 제공. `OnTimeSetListener` 콜백을 통해 결과를 받으며, 24시간 형식 여부를 생성자 인자(`is24HourView`)로 지정한다. ⚠️ **실무 함정**: `TimePicker` 위젯에서 값을 직접 읽을 때 API가 갈린다 — **API 23(M)부터는 `getHour()`/`getMinute()`** 를 쓰고, **그 이전에는 `getCurrentHour()`/`getCurrentMinute()`** 인데 이 둘은 **deprecated**되었다. 따라서 구버전까지 지원하려면 `Build.VERSION.SDK_INT >= 23` 분기 처리가 필요하다(단, 콜백 `onTimeSet(view, h, m)`으로 값을 받으면 이 구분이 불필요). ([공식 문서](https://developer.android.com/reference/android/widget/TimePicker))
...
**AlertDialog 버튼의 본질**
> 버튼 이름 positive/negative/neutral은 단순한 **구분자**일 뿐이며 OS가 자동으로 부여하는 의미가 따로 없다. 실제 동작은 `OnClickListener.onClick()` 안의 코드가 결정한다. 즉 "확인"이라는 텍스트의 버튼을 negative 슬롯에 등록해도 동작 자체에는 문제가 없다(다만 UX 관례상 적절치 않을 뿐). 리스너를 `null`로 두면 버튼을 눌렀을 때 다이얼로그가 닫히기만 한다. 📎 콜백의 `which` 인자로 어떤 버튼이 눌렸는지 식별할 수 있는데, 그 상수값은 **음수**다: `DialogInterface.BUTTON_POSITIVE = -1`, `BUTTON_NEGATIVE = -2`, `BUTTON_NEUTRAL = -3`. ([DialogInterface](https://developer.android.com/reference/android/content/DialogInterface))
>
...
> ⚠️ **ProgressDialog 주의**: 위 도식의 `ProgressDialog`는 **API 26(Android 8.0)에서 deprecated**되었다. 진행 상태 표시는 인라인 `ProgressBar` 또는 알림(Notification)으로 대체하는 것이 권장된다. ([공식 문서](https://developer.android.com/reference/android/app/ProgressDialog))
...
**3. ⭐⭐ 📌 [적용/분석] AlertDialog로 "삭제하시겠습니까?" 확인창을 띄우고 사용자가 "삭제"를 누르면 `deleteItem()`을 실행하려고 한다. 가장 권장되는(정석) 코드 조합은?[(해설)](#prob-3)**
   > <보기>
   >
...
> - 다: **컴파일·실행 자체는 정상**이며 "삭제"를 눌렀을 때 `deleteItem()`도 정확히 호출된다. 즉 '다'는 "틀린 코드"가 아니다. 다만 긍정 액션(삭제)을 Neutral 슬롯에 배치하는 것은 **관례상 비권장**(긍정=Positive, 부정=Negative가 표준)이다. 따라서 발문이 묻는 "**가장 권장되는(정석) 코드 조합**"의 기준에서는 Positive/Negative 슬롯을 의도대로 사용한 **나**가 정답이다.
...
> - 💡 **보완**: 단말의 시스템 설정(12/24시간 표시)을 그대로 따르려면 `android.text.format.DateFormat.is24HourFormat(context)`로 사용자 설정값을 읽어 마지막 인자에 넘기는 것이 표준 패턴이다. 예: `new TimePickerDialog(ctx, listener, h, min, DateFormat.is24HourFormat(this))`. (근거: https://developer.android.com/reference/android/app/TimePickerDialog )
...
- **설명**:
  ...
  - ⚠️ **`setCancelable(false)` vs `setCanceledOnTouchOutside(false)` 구분**: 전자 `setCancelable(false)`는 **뒤로가기(Back) 키로 닫는 것까지 차단**(바깥 터치도 함께 막힘)하는 더 강한 설정이고, 후자 `setCanceledOnTouchOutside(false)`는 **다이얼로그 바깥 영역 터치로 닫히는 것만 차단**(뒤로가기 키로는 여전히 닫힘)한다. "Back은 허용하되 바깥 터치만 막고 싶다"면 후자를 써야 한다. ([공식 문서](https://developer.android.com/reference/android/app/Dialog#setCanceledOnTouchOutside(boolean)))
...
| **클릭 리스너** | DialogInterface.OnClickListener | `(dialog, which) -> {...}` | 다이얼로그 버튼 클릭 시 호출되는 콜백. `which`는 어떤 버튼인지 식별하는 값으로, 버튼 슬롯은 **음수 상수**다: `BUTTON_POSITIVE = -1`, `BUTTON_NEGATIVE = -2`, `BUTTON_NEUTRAL = -3` ([DialogInterface](https://developer.android.com/reference/android/content/DialogInterface)) |
...
## 🟢 심층 확장: 권장 Dialog 구현 패턴 (AppCompat · Material · Compose)

> 이 절은 공식 문서(developer.android.com) 기반의 리서치 심층 확장입니다. 본문에서 다룬 `android.app.AlertDialog`/`DatePickerDialog`/`TimePickerDialog`는 학습용으로 충분하지만, 실제 현대 앱에서는 아래 권장 패턴을 사용합니다.

### 1. 어떤 AlertDialog 클래스를 import 해야 하는가

같은 이름의 `AlertDialog`가 **두 개의 패키지**에 존재합니다. 무엇을 import 하느냐에 따라 외형과 동작이 달라집니다.

| 항목 | `android.app.AlertDialog` (플랫폼) | `androidx.appcompat.app.AlertDialog` (권장) |
| :--- | :--- | :--- |
| 제공 출처 | Android 프레임워크(OS 내장) | AndroidX AppCompat 라이브러리 |
| 테마 의존 | OS 네이티브 테마(Theme.Holo 등) | `Theme.AppCompat`(또는 그 자손) 필요 |
| 테마 속성 | `android:alertDialogTheme` | `alertDialogTheme` (android: 접두 없음) |
| 테마 일관성 | OS 버전마다 외형이 달라짐 | API 레벨 전반에서 일관된 외형 백포트 |
| 하위호환 | OS 기본 스타일 종속 | 구버전 기기에서도 최신 스타일 적용 |

```java
// 권장: AppCompat 버전을 import (액티비티는 AppCompatActivity 권장)
import androidx.appcompat.app.AlertDialog;

new AlertDialog.Builder(this)
    .setTitle("알림")
    .setMessage("정말 삭제하시겠습니까?")
    .setPositiveButton("확인", (dialog, which) -> { /* 처리 */ })
    .setNegativeButton("취소", null)
    .show();
```

> 핵심: 코드 형태는 동일하지만 **import 한 줄로 테마 일관성·하위호환이 결정**됩니다. 단, AppCompat 버전은 `Theme.AppCompat` 계열 테마가 적용된 컨텍스트가 필요하며, 그렇지 않으면 *"You need to use a Theme.AppCompat theme..."* 오류가 발생합니다.
> 근거: https://developer.android.com/reference/androidx/appcompat/app/AlertDialog , https://developer.android.com/reference/android/app/AlertDialog

> ⚠️ **테마 요구의 비대칭에 주의**: 두 클래스가 테마를 다루는 방식은 **대칭이 아니다**. **플랫폼** `android.app.AlertDialog`는 테마를 지정하지 않아도 **OS 기본 테마(Theme/Theme.Material 등)로 그대로 동작**하므로 `Theme.AppCompat`이 **불필요**하다. 반면 **`androidx.appcompat.app.AlertDialog`** 는 위젯 백포트를 위해 **반드시 `Theme.AppCompat`(또는 그 자손) 테마가 적용된 컨텍스트**가 있어야 하며, 그렇지 않으면 위 *"You need to use a Theme.AppCompat theme..."* 예외가 난다. 즉 "AppCompat 버전만 Theme.AppCompat이 필수"이고, 플랫폼 버전은 그런 제약이 없다. ([공식 문서](https://developer.android.com/reference/androidx/appcompat/app/AlertDialog))

### 2. 직접 show() 대신 DialogFragment 를 쓰는 이유 (구성 변경 생존)

본문 예제처럼 `builder.show()`를 직접 호출하면, **화면 회전 같은 구성 변경(configuration change)** 시 액티비티가 재생성되면서 다이얼로그가 사라지거나 윈도우 누수(window leak) 경고가 날 수 있습니다. 공식 문서는 다이얼로그를 `DialogFragment` 안에서 호스팅할 것을 권장합니다.

> "Using `DialogFragment` to manage the dialog makes it correctly handle lifecycle events such as when the user taps the Back button or rotates the screen."
> — developer.android.com (Dialogs)

```java
public class ConfirmDialogFragment extends DialogFragment {
    @NonNull @Override
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        return new AlertDialog.Builder(requireContext())   // androidx 버전
            .setTitle("알림")
            .setMessage("정말 삭제하시겠습니까?")
            .setPositiveButton("확인", (d, w) -> { /* 처리 */ })
            .setNegativeButton("취소", null)
            .create();
    }
}

// 표시: FragmentManager 가 상태를 관리 → 회전해도 자동 복원
new ConfirmDialogFragment().show(getSupportFragmentManager(), "confirm");
```

| 직접 `builder.show()` | `DialogFragment.show()` |
| :--- | :--- |
| 회전 시 다이얼로그 소실 가능 | FragmentManager 가 회전 후 자동 복원 |
| 윈도우/메모리 누수 위험 | 누수 회피에 유리 |
| 상태 저장 직접 처리 | 프래그먼트 생명주기로 상태 관리 |

> 근거: https://developer.android.com/develop/ui/views/components/dialogs

#### 2-1. 왜 DialogFragment는 회전에도 살아남는가 (생명주기 관점 심화)

`builder.show()`가 회전 시 사라지는 이유와 `DialogFragment`가 복원되는 이유를 **재생성 흐름**으로 따라가 보자.

1. **구성 변경 발생**: 화면 회전 시 시스템은 현재 액티비티를 `onDestroy()`까지 파괴하고 새 액티비티를 `onCreate()`부터 다시 만든다. 이때 직접 `show()`한 `AlertDialog`는 **옛 액티비티의 윈도우(Window)에 붙어 있던 객체**이므로 함께 사라진다. 파괴 직전 dismiss하지 않으면 *"Activity has leaked window... that was originally added here"* 라는 **윈도우 누수(window leak)** 로그가 남는다.
2. **FragmentManager의 상태 저장**: 반면 `DialogFragment`는 **`FragmentManager`** 가 관리한다. `FragmentManager`는 액티비티가 파괴되기 전 **자신이 호스팅하던 프래그먼트 목록과 인자(arguments)를 저장(saved state)** 하고, 새 액티비티의 `FragmentManager`가 이를 **자동 복원**한다.
3. **다이얼로그 재생성**: 복원 시 `DialogFragment.onCreateDialog()`가 **다시 호출**되어 다이얼로그가 새 윈도우에 재부착된다. 따라서 개발자가 회전 상태를 직접 저장/복원하지 않아도 다이얼로그가 그대로 떠 있는 것처럼 보인다.

| 처리 주체 | 구성 변경 시 동작 | 결과 |
| :--- | :--- | :--- |
| `builder.show()` (직접) | 옛 윈도우와 함께 소멸, 자동 재부착 없음 | 다이얼로그 소실 + 누수 경고 가능 |
| `DialogFragment` | FragmentManager가 상태 저장→복원, `onCreateDialog()` 재호출 | 회전 후에도 자동 유지 |

> 핵심: "다이얼로그를 **누가 소유하느냐**"가 갈림길이다. 액티비티 윈도우가 소유하면 액티비티와 함께 죽고, FragmentManager가 소유하면 구성 변경을 가로질러 복원된다. 그래서 공식 문서는 **모든 다이얼로그를 `DialogFragment`로 감쌀 것**을 권장한다.
> 근거: https://developer.android.com/develop/ui/views/components/dialogs , https://developer.android.com/guide/fragments/dialogs

### 3. Material Dialogs: MaterialAlertDialogBuilder / MaterialDatePicker / MaterialTimePicker

Material Components for Android 라이브러리는 Material 3 스타일의 다이얼로그·피커를 제공합니다.

- **MaterialAlertDialogBuilder**: `AlertDialog.Builder`와 거의 동일한 API에 Material 3 외형을 입힌 빌더. 단, 직접 호출하면 회전 시 다이얼로그가 유지되지 않으므로 `DialogFragment`의 `onCreateDialog()` 안에서 사용하는 것이 권장됩니다.
...
- **MaterialDatePicker / MaterialTimePicker**: 본문의 `DatePickerDialog`/`TimePickerDialog`를 대체하는 Material 피커. 두 클래스 모두 **`DialogFragment`를 상속**(`MaterialDatePicker<S>`의 클래스 계층: `Fragment → DialogFragment → MaterialDatePicker`)하므로 구성 변경 생존 이점을 기본 제공합니다. 옛 `TimePickerDialog`가 `Dialog`를 상속한 것과 대비됩니다.
...
datePicker.addOnPositiveButtonClickListener(selection -> {
    // selection: 선택된 날짜의 UTC epoch millis (Long)
});
...
timePicker.addOnPositiveButtonClickListener(v -> {
    int h = timePicker.getHour();   // [0,23]
    int m = timePicker.getMinute(); // [0,59]
});
timePicker.show(getSupportFragmentManager(), "time");
```

> 핵심: 콜백을 빌더 인자가 아니라 `addOnPositiveButtonClickListener()`(외 `addOnNegativeButtonClickListener`/`addOnCancelListener`/`addOnDismissListener`)로 **별도 등록**합니다. `DialogFragment` 하위 클래스이므로 `FragmentManager`로 표시하고 회전 시 자동 복원됩니다.
> 근거: https://developer.android.com/reference/com/google/android/material/datepicker/MaterialDatePicker , https://developer.android.com/reference/com/google/android/material/timepicker/MaterialTimePicker

#### 3-1. ⚠️ 실무 함정: MaterialDatePicker의 selection은 UTC epoch millis다

`MaterialDatePicker<Long>`의 `selection`(콜백으로 넘어오는 `Long`)과 내부 모든 시각은 **UTC 자정 기준 epoch milliseconds**다. 공식 문서는 "all the time represented in `MaterialDatePicker` is **timezone neutral UTC**"라고 명시한다. 따라서 이 값을 그대로 단말의 **로컬 타임존**(`Calendar.getInstance()`, `SimpleDateFormat` 기본값 등)으로 포매팅하면 타임존 오프셋만큼 **하루가 밀려 표시**될 수 있다(예: UTC+9의 한국에서 오전 9시 이전 변환 시 전날로 표시).

```java
datePicker.addOnPositiveButtonClickListener(selection -> {
    // selection: UTC 자정 기준 epoch millis
    // (1) 표시·포매팅은 반드시 UTC 타임존으로
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd", Locale.KOREA);
    sdf.setTimeZone(TimeZone.getTimeZone("UTC"));       // ★ 핵심: UTC 강제
    String text = sdf.format(new Date(selection));

    // (2) 또는 java.time 사용 시
    LocalDate date = Instant.ofEpochMilli(selection)
            .atZone(ZoneOffset.UTC)                     // ★ UTC로 해석
            .toLocalDate();
});
```
...
| 잘못된 처리 | 올바른 처리 |
| :--- | :--- |
| `selection`을 로컬 타임존으로 포매팅 | `TimeZone.getTimeZone("UTC")` / `ZoneOffset.UTC`로 포매팅 |
| `Calendar.getInstance()`(로컬)에 그대로 set | `Calendar.getInstance(TimeZone.getTimeZone("UTC"))` 사용 |
| 초기 선택값을 로컬 millis로 전달 | `Builder.setSelection()`에도 **UTC millis** 전달 |

> 핵심: MaterialDatePicker는 "날짜만" 다루므로 타임존을 일부러 중립(UTC)으로 둔다. 변환 지점에서 **항상 UTC를 명시**해야 "하루 밀림" 버그를 피한다. (옛 `DatePickerDialog`는 `Calendar` 로컬 필드 기반이라 이 함정이 없었던 점과 대비.)
> 근거: https://developer.android.com/reference/com/google/android/material/datepicker/MaterialDatePicker , https://m3.material.io/components/date-pickers/overview

> 🟢 **선택 가능 범위 제한 — `CalendarConstraints`**: `MaterialDatePicker.Builder`에 **`setCalendarConstraints(CalendarConstraints)`** 를 주면 선택 가능한 날짜 범위를 제한할 수 있다. `CalendarConstraints.Builder()`의 `setStart()`/`setEnd()`로 표시 월 범위를, `setValidator()`(예: `DateValidatorPointForward.now()` — 오늘 이후만 허용)로 **선택 가능한 날짜 조건**을 지정한다. 옛 `DatePickerDialog`의 `getDatePicker().setMinDate()/setMaxDate()`에 대응하는 현대 API다. ([공식 문서](https://developer.android.com/reference/com/google/android/material/datepicker/MaterialDatePicker))

### 4. Jetpack Compose 대응 (참고)

선언형 UI인 Jetpack Compose에서는 다이얼로그도 컴포저블로 표현하며, **표시 여부를 상태(state)로 제어**합니다(명령형 `show()`/`dismiss()` 없음).

```kotlin
// AlertDialog 컴포저블 (Material3)
...
// Material3 DatePicker (DatePickerDialog 컨테이너 + DatePicker 콘텐츠)
val dateState = rememberDatePickerState()
DatePickerDialog(
    onDismissRequest = { },
    confirmButton = { TextButton(onClick = { /* dateState.selectedDateMillis */ }) { Text("확인") } }
) {
    DatePicker(state = dateState)
}
```

> Compose에서는 선택 값을 `rememberDatePickerState()`의 `selectedDateMillis`로 읽습니다. (참고: Material3의 `DatePicker`/`DatePickerDialog`/`DateRangePicker`는 현재 experimental API)
> 근거: https://developer.android.com/develop/ui/compose/components/dialog , https://developer.android.com/develop/ui/compose/components/datepickers
...
- Android Developers - [Display dialogs with DialogFragment](https://developer.android.com/guide/fragments/dialogs)
- Android Material - [MaterialDatePicker (selection은 UTC epoch millis)](https://developer.android.com/reference/com/google/android/material/datepicker/MaterialDatePicker)
