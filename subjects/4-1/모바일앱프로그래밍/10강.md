...
3.  🟡 **이벤트**: 화면 위젯에서 발생하는 하나의 행위를 다루는 것으로서, **버튼 클릭/화면 터치/키 입력/체크박스 상태 변경** 등 사용자 상호작용을 말함. (웹의 "링크 클릭·입력값 변경"과 같은 개념을 안드로이드 위젯 맥락으로 옮긴 것.)
...
| **MotionEvent 액션** | `int act = e.getAction() & MotionEvent.ACTION_MASK;` | ACTION_DOWN/MOVE/UP/POINTER_DOWN 구분. `e.getActionMasked()`와 **동일 결과**(둘 다 포인터 인덱스 비트를 제거한 순수 액션 코드를 반환) |
| **DOWN 소비 규칙** | `if (act == ACTION_DOWN) return true;` | **`ACTION_DOWN`에서 `true`(소비)를 반환하지 않으면 이후 `ACTION_MOVE`/`ACTION_UP`이 같은 뷰로 전달되지 않는다.** 즉 한 제스처를 끝까지 받으려면 DOWN에서 반드시 소비해야 함 |
...
| **센서 등록(방어)** | `if (sensor == null \|\| !sm.registerListener(this, sensor, SENSOR_DELAY_UI)) { /* 센서 미탑재 안내 */ }` | `registerListener`의 **boolean 반환값** 활용 — 해당 센서가 기기에 없으면 `false` 반환 |
| **센서 해제** | `sm.unregisterListener(this);` | `onPause()`에서 반드시 호출 (배터리/누수 방지) |

> 🟡 **방어 코드 권장**: `getDefaultSensor()`는 센서가 없으면 `null`을 반환하고, `registerListener()`는 **등록 성공 여부를 `boolean`으로 반환**(센서가 없으면 `false`)한다. 따라서 `null` 체크와 함께 반환값을 확인해, 센서가 없는 기기에서는 사용자에게 안내하거나 대체 흐름으로 넘어가는 방어 코드를 작성하는 것이 안전하다.
...
> <a id="ox-7"></a>
> **7. 정답**: O
> **해설**: `new Handler()`(인수 없는 생성자)는 메인 루퍼에 무조건 연결되는 것이 아니라 **생성된 스레드의 `Looper`에 연결된다**(메인 스레드에서 생성하면 메인 루퍼, 다른 스레드에서 생성하면 그 스레드의 루퍼). 이 모호함 때문에 인수 없는 생성자는 **deprecated**되었고, 메인 스레드 실행을 명시하려면 `new Handler(Looper.getMainLooper())`가 권장된다. 본 문항은 `postDelayed()`로 등록한 `Runnable`이 **메인(UI) 스레드 메시지 큐**에서 실행되는지를 묻는 것이므로, 메인 루퍼와 연결된 Handler를 전제로 하면 O가 맞다. [근거: https://developer.android.com/reference/android/os/Handler ]

> <a id="ox-8"></a>
> **8. 정답**: O
> **해설**: `SensorEventListener`는 인터페이스이므로 `onSensorChanged()`·`onAccuracyChanged()` 두 추상 메소드를 **모두 구현해야 한다**(Java 8 default 메소드 아님). 다만 **구현 의무는 있으나 비워둘 수 있다** — 실무에서는 `onAccuracyChanged(Sensor, int)`를 의미 있는 동작 없이 **빈 구현(no-op)** 으로 두는 경우가 흔하다(정확도 변화에 따로 반응할 필요가 없을 때). 즉 "메소드 본문을 채워 의미 동작을 넣을 의무"가 아니라 "메소드를 오버라이드해 존재시킬 의무"다. ([SensorEventListener](https://developer.android.com/reference/android/hardware/SensorEventListener))
...
public class MainActivity extends AppCompatActivity {
    ...
    @Override public boolean onKeyDown(int keyCode, KeyEvent event) {
        tvLog.append("\n[Key] keyCode=" + keyCode
                ...
                // ※ ACTION_MULTIPLE(=2)는 API 29(Android 10)에서 deprecated되어 최신 입력 시스템에서는 실제로 전달되지 않는다. ([KeyEvent](https://developer.android.com/reference/kotlin/android/view/KeyEvent))
...
### 4) `SENSOR_DELAY_*` 4단계 비교표 — 샘플링 주기 vs 배터리 트레이드오프

`registerListener(listener, sensor, rate)`의 세 번째 인자(rate)는 센서 이벤트 전달 주기의 **힌트**다. 값이 작을수록(빠를수록) 반응성은 좋아지지만 **CPU 깨우기·이벤트 처리 횟수가 늘어 배터리를 더 소모**한다. 본문 Best Practice에서 "UI 갱신이면 `SENSOR_DELAY_FASTEST` 말고 `SENSOR_DELAY_UI`"라고 한 근거를 수치로 정리한다.

| 상수 | 지연(딜레이) | 대략 주파수 | 권장 용도 | 배터리/반응성 트레이드오프 |
| :--- | :--- | :--- | :--- | :--- |
| `SENSOR_DELAY_NORMAL` | 200,000μs (**~200ms**) | 약 5Hz | 화면 회전 감지 등 느린 변화 | **배터리 가장 절약**, 반응 느림(기본값) |
| `SENSOR_DELAY_UI` | 66,667μs (**~66.7ms**) | 약 15Hz | 일반 UI 갱신(나침반 바늘 등) | 균형. 화면 갱신 주기와 정합 |
| `SENSOR_DELAY_GAME` | 20,000μs (**~20ms**) | 약 50Hz | 게임·실시간 기울기 제어 | 반응성↑, 배터리 소모↑ |
| `SENSOR_DELAY_FASTEST` | 0μs (**가능한 최대 속도**) | 센서 HW 한계 | 고빈도 신호 수집(HAR 윈도잉 등) | **배터리 가장 많이 소모**. 꼭 필요할 때만 |

- 핵심 원칙: **목적에 필요한 만큼만 빠르게**. UI 표시에는 `SENSOR_DELAY_UI`로 충분하며, `SENSOR_DELAY_FASTEST`는 활동분류·신호처럼 고빈도 원시 데이터가 꼭 필요한 경우로 제한한다.
- 이 값들은 **요청 힌트**일 뿐 보장 주기가 아니다. 더 정밀한 제어가 필요하면 `registerListener(listener, sensor, samplingPeriodUs, maxReportLatencyUs)` 오버로드로 마이크로초 단위 주기를 직접 지정할 수 있다(센서가 지원하는 범위 내에서 근사). `maxReportLatencyUs`를 키우면 이벤트를 **배치(batching)** 로 모아 전달해 깨우기 횟수를 줄여 배터리를 더 아낄 수 있다.
- 배터리 관점에서, 사용하지 않는 동안의 `unregisterListener()`(onPause)도 위 rate 선택만큼 중요하다(본문 (2)·Best Practice 참조).
- 근거: SensorManager 상수(NORMAL/UI/GAME/FASTEST) — https://developer.android.com/reference/android/hardware/SensorManager , 센서 모니터링·배터리 — https://developer.android.com/develop/sensors-and-location/sensors/sensors_overview

### 5) 전통 `OnTouchListener` vs Jetpack Compose `pointerInput` 대응표

전통 View 시스템의 터치 처리(`onTouch`/`onTouchEvent` + `MotionEvent` 액션 분기)는 Jetpack Compose에서 `Modifier.pointerInput { ... }` 블록 안의 **제스처 감지기**로 대체된다. 콜백에서 액션 코드를 수동 분기하던 것을, Compose는 의도(탭/드래그 등) 단위의 고수준 API로 제공한다.

| 전통 View 시스템 | Jetpack Compose (`Modifier.pointerInput`) | 비고 |
| :--- | :--- | :--- |
| `setOnTouchListener` + `ACTION_DOWN`/`UP` 직접 분기 → 탭 판정 | `pointerInput(Unit){ detectTapGestures(onTap = { ... }) }` | 탭/롱프레스/더블탭을 `onTap`·`onLongPress`·`onDoubleTap` 람다로 분리 |
| `ACTION_MOVE` 좌표 추적으로 드래그 구현 | `pointerInput(Unit){ detectDragGestures { change, drag -> ... } }` | 드래그 시작·이동·종료를 콜백으로 |
| `GestureDetector`로 핀치/스케일 | `detectTransformGestures { _, pan, zoom, rot -> ... }` | 팬/줌/회전 통합 |
| `onClick`(접근성 미흡한 수동 터치) | `Modifier.clickable { ... }` | 접근성·리플 효과 자동 |

```kotlin
// Compose: 탭과 드래그를 선언적으로 처리
Box(
    Modifier
        .pointerInput(Unit) { detectTapGestures(onTap = { offset -> /* 탭 좌표 */ }) }
        .pointerInput(Unit) { detectDragGestures { change, dragAmount -> /* 드래그 */ } }
)
```

- 핵심 차이: 전통 방식은 `MotionEvent` 액션을 **저수준에서 직접 분기**하지만, Compose는 `detectTapGestures`/`detectDragGestures`로 **의도 단위 고수준 콜백**을 제공한다. 저수준 이벤트가 필요하면 `awaitPointerEventScope { awaitPointerEvent() }`로 내려갈 수 있다.
- 근거: [Handle user interactions (pointerInput / detectTapGestures)](https://developer.android.com/develop/ui/compose/touch-input/pointer-input)
