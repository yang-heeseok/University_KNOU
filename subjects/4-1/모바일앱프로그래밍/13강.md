...
## 📝 요약 (Summary)
1. 🔴 **AdapterView**: 항목에 해당하는 **여러 개의 차일드 뷰**를 가질 수 있고 사용자와의 상호 작용(클릭·선택)도 처리할 수 있는 ViewGroup 계열의 추상 뷰. 데이터는 직접 갖지 않고 **Adapter**로부터 공급받는다.
...
- **Good Practice (좋은 예시)**
  ...
  ```
- **설명**: ① AdapterView는 항상 **Adapter로 데이터를 주입**한다. ② Spinner는 닫힘/펼침 두 상태 레이아웃을 모두 지정하는 것이 표준. ③ Spinner 선택은 **`OnItemSelectedListener`**가 정답이며 `OnClickListener`는 의미상 부적절. ④ 데이터 변경 후에는 반드시 `notifyDataSetChanged()` 호출.
