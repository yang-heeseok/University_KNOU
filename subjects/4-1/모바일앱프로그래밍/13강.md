...
## 📝 요약 (Summary)
1. 🔴 **AdapterView**: 항목에 해당하는 **여러 개의 차일드 뷰**를 가질 수 있고 사용자와의 상호 작용(클릭·선택)도 처리할 수 있는 ViewGroup 계열의 추상 뷰. 데이터는 직접 갖지 않고 **Adapter**로부터 공급받는다. 📌 `AdapterView`는 형식상 `extends ViewGroup`인 **추상 클래스**이지만, 일반 ViewGroup의 자식 추가 계약(`addView()`/`removeView()`)을 **의도적으로 막아 둔 특수 케이스**다. ⚠️ 따라서 `addView()`/`removeView()`를 직접 호출하면 **`UnsupportedOperationException`이 발생**한다 — 자식 뷰는 오직 **Adapter를 통해서만** 관리되기 때문이다(오개념 4 참조). ([공식 문서](https://developer.android.com/reference/android/widget/AdapterView))
...
6. 🟢 **Spinner 프롬프트**: `android:prompt` 또는 `setPrompt()`로 팝업 상단에 안내 메시지를 표시할 수 있다. ⚠️ 다만 프롬프트는 `spinnerMode="dialog"`일 때 **다이얼로그 제목으로 표시**되며, 기본값인 `spinnerMode="dropdown"`(드롭다운) 모드에서는 **무시되는 것이 일반적**이다. ([공식 문서](https://developer.android.com/reference/android/widget/Spinner))
...
| **어댑터 getView 시그니처** 📎 빈출 | `View getView(int position, View convertView, ViewGroup parent)` | Adapter가 각 위치의 데이터를 항목 View로 변환·재활용(`convertView`)하는 핵심 메서드 ([Adapter](https://developer.android.com/reference/android/widget/Adapter)) |

> ⚠️ **빈출 함정 1 — `simple_list_item_1` + 커스텀 객체**: `ArrayAdapter`를 `android.R.layout.simple_list_item_1`(단일 `TextView` 레이아웃)과 함께 쓰고 **문자열이 아닌 커스텀 객체 리스트**를 넣으면, 각 항목은 그 객체의 **`toString()` 반환값으로 표시**된다. 따라서 원하는 표시 형식이 있으면 객체에 `toString()`을 오버라이드하거나, `getView()`를 재정의한 커스텀 어댑터를 써야 한다. ([공식 문서](https://developer.android.com/reference/android/widget/ArrayAdapter))
> ⚠️ **빈출 함정 2 — Spinner 모드**: Spinner는 `android:spinnerMode`로 펼침 방식이 갈린다. 기본값 **`"dropdown"`**(앵커 아래로 떨어지는 드롭다운 팝업) vs **`"dialog"`**(중앙 모달 다이얼로그). 6번 요약처럼 `prompt`는 **`"dialog"` 모드에서만 제목으로 표시**되고 `"dropdown"`에서는 일반적으로 무시된다. ([공식 문서](https://developer.android.com/reference/android/widget/Spinner))
...
- **오개념 분석**
    ...
    - **분석**: 틀림. `android:prompt` 속성 또는 `setPrompt()` 메서드로 안내 문구를 표시할 수 있다. ⚠️ 단, 프롬프트는 `spinnerMode="dialog"`일 때 **다이얼로그 제목**으로 나타나고, 기본 `dropdown` 모드에서는 **무시되는 것이 일반적**이라는 점에 유의한다. ([공식 문서](https://developer.android.com/reference/android/widget/Spinner))
...
### 3. 🟢 심층 확장: ListView → RecyclerView 마이그레이션

현재 강의는 ListView를 다루지만, 실무에서는 RecyclerView가 표준이다. RecyclerView는 ListView 대비 세 가지를 구조적으로 강제·분리한다. ① **ViewHolder 패턴을 강제**해 `findViewById` 반복을 줄이고, ② **`LayoutManager`로 배치 로직을 분리**(세로/그리드/가로)하며, ③ **항목 추가·삭제·이동 애니메이션**을 기본 지원한다. ListView의 ViewHolder는 **선택적 최적화**였지만 RecyclerView에서는 구조 자체가 ViewHolder를 **필수**로 만든다.

```java
// 1) ViewHolder 강제 — 어댑터가 RecyclerView.Adapter<VH>를 상속
...
```java
// 2) LayoutManager로 배치 분리 (세로 목록)
recyclerView.setLayoutManager(new LinearLayoutManager(this));
recyclerView.setAdapter(new MyAdapter(dataList));
```

**ListView → RecyclerView 항목별 마이그레이션 대조표**

| 항목 | ListView | RecyclerView | 변경 포인트 |
| :--- | :--- | :--- | :--- |
| **ViewHolder** | 선택적 최적화(`convertView`에 수동 태깅) | **`onCreateViewHolder()`로 강제** | 더 이상 `convertView == null` 분기를 직접 짜지 않음 |
| **배치(레이아웃)** | ListView=수직 고정, GridView=격자 별도 위젯 | **`LayoutManager`로 분리** (`LinearLayoutManager`/`GridLayoutManager`/`StaggeredGridLayoutManager`) | 같은 어댑터로 세로·격자·가로 전환 가능 |
| **항목 애니메이션** | 거의 없음(수동 구현) | **`ItemAnimator` 기본 제공**(`DefaultItemAnimator`) | 삽입/삭제/이동 시 자동 트랜지션 |
| **부분 갱신** | `notifyDataSetChanged()` 위주(전체 무효화) | `notifyItemInserted/Removed/Changed`, **`DiffUtil`** | 변경된 항목만 부분 갱신 |
| **클릭 처리** | `setOnItemClickListener` 내장 | **내장 없음** → ViewHolder/itemView에 직접 등록 | 마이그레이션 시 클릭 콜백 재배선 필요 |
| **구분선** | `android:divider` 속성 | **`ItemDecoration`**(`DividerItemDecoration`) | 데코레이터로 분리 |

> 마이그레이션의 가장 흔한 실수는 ListView의 `setOnItemClickListener`를 그대로 옮기려는 것이다. RecyclerView는 **항목 클릭 리스너를 내장하지 않으므로** ViewHolder의 `itemView`에 직접 `setOnClickListener`를 달거나 `getBindingAdapterPosition()`으로 위치를 얻어야 한다.
> 근거: https://developer.android.com/develop/ui/views/layout/recyclerview

> 🟢 **RecyclerView 우위 사례 보강 — `ConcatAdapter`**: ListView 한 화면에 "헤더 + 본문 목록 + 푸터(또는 로딩 스피너)"처럼 **이질적 영역을 함께** 표시하려면 단일 어댑터에서 `getItemViewType()` 분기를 직접 짜야 했다. RecyclerView는 **`ConcatAdapter(headerAdapter, listAdapter, footerAdapter)`** 로 여러 어댑터를 **순서대로 연결(합성)** 해 하나의 RecyclerView에 표시할 수 있어, 각 영역을 독립 어댑터로 분리·재사용할 수 있다(무한 스크롤의 로딩 푸터 패턴에 특히 유용). ([공식 문서](https://developer.android.com/reference/androidx/recyclerview/widget/ConcatAdapter))

### 4. 🟢 심층 확장: ListAdapter + DiffUtil — notifyDataSetChanged() 대비 효율

`notifyDataSetChanged()`는 데이터셋 전체가 무효화되었다고 알리므로, **화면 밖 항목까지 포함해 모두 다시 바인딩·재그리기**하고 **항목 변경 애니메이션도 발생하지 않는다**("notifyDataSetChanged() tells RecyclerView that the entire list is potentially invalid, causing RecyclerView to rebind and redraw every item"). 반면 **`DiffUtil`** 은 두 리스트의 차이를 계산해 최소 갱신 연산(삽입/삭제/이동/변경)만 산출한다.

**`ListAdapter`** 는 내부에서 `AsyncListDiffer`로 **백그라운드 스레드**에서 diff를 계산하고, 최소 갱신만 디스패치하며 **변경 항목 애니메이션을 자동 적용**한다. `submitList()`로 새 리스트만 넘기면 된다.

```java
public class ItemAdapter extends ListAdapter<Item, ItemAdapter.VH> {
    protected ItemAdapter() { super(DIFF); }

    static final DiffUtil.ItemCallback<Item> DIFF =
        new DiffUtil.ItemCallback<Item>() {
            @Override public boolean areItemsTheSame(@NonNull Item a, @NonNull Item b) {
                return a.getId() == b.getId();          // 같은 항목인가(보통 ID)
            }
            @Override public boolean areContentsTheSame(@NonNull Item a, @NonNull Item b) {
                return a.equals(b);                      // 내용이 동일한가
            }
        };
    // onCreateViewHolder / onBindViewHolder 구현 ...
}

// 갱신: 새 리스트만 제출하면 diff·애니메이션 자동 처리
adapter.submitList(newList);
```

| 항목 | `notifyDataSetChanged()` | `ListAdapter` + `DiffUtil` |
| :--- | :--- | :--- |
| **갱신 범위** | 전체 재바인딩(화면 밖 포함) | 변경분만 부분 갱신 |
| **애니메이션** | 없음 | 삽입/삭제/이동 자동 |
| **diff 계산** | 없음(전부 무효화) | 백그라운드 스레드 비동기 |
| **API** | 데이터 교체 후 수동 호출 | `submitList()` 한 번 |

> 같은 항목인데 내용만 바뀐 경우 `getChangePayload()`로 페이로드를 넘기면 깜빡임(blinking) 없이 변경 필드만 갱신할 수 있다.

**`submitList(list, commitCallback)` — diff 완료 시점 알기**

`ListAdapter`/`AsyncListDiffer`의 diff 계산은 **비동기(백그라운드)** 이므로, "리스트가 실제로 화면에 반영(commit)된 직후"에 후속 작업(예: 스크롤을 맨 위로 이동, 빈 화면 토글)을 하려면 **2-인자 오버로드** `submitList(List, Runnable commitCallback)`을 쓴다.

```java
adapter.submitList(newList, () -> {
    // diff 계산이 끝나고 새 리스트가 어댑터에 commit된 직후 호출
    recyclerView.scrollToPosition(0);
    emptyView.setVisibility(newList.isEmpty() ? View.VISIBLE : View.GONE);
});
```

⚠️ 주의: 이 `commitCallback`은 **항상 실행된다는 보장이 없다**. 이전 리스트가 commit되기 전에 새 리스트를 다시 `submitList()`하면, **이전 호출의 콜백은 건너뛰어진다**("If a new list is submitted while a previous list is being committed, the previous commitCallback isn't run"). 즉 콜백은 "이 리스트가 실제로 반영됐을 때만" 보장되는 일회성 신호로 다뤄야 한다.

> 근거: https://developer.android.com/reference/androidx/recyclerview/widget/ListAdapter , https://developer.android.com/reference/androidx/recyclerview/widget/AsyncListDiffer

### 5. 🟢 심층 확장: Jetpack Compose 대응 — LazyColumn / items

선언형 UI인 Jetpack Compose에서 RecyclerView에 해당하는 것이 **`LazyColumn`**(가로는 `LazyRow`)이다. 어댑터·ViewHolder·LayoutManager 없이 **화면에 보이는 항목만 구성(lazy composition)** 하여 대량 리스트를 효율적으로 렌더링한다. 뷰 재활용 대신 **리컴포지션** 패러다임을 쓰며, 화면을 벗어난 컴포저블은 폐기되고 새로 들어오는 항목은 새로 방출된다.
...
`items()`는 `LazyListScope`의 DSL 함수로 컬렉션을 펼치고, 단일 항목은 `item { }`을 쓴다. `key`를 지정하면 항목 식별로 리컴포지션을 최적화한다. RecyclerView 대비 어댑터/ViewHolder/LayoutManager 보일러플레이트가 사실상 사라진다.

**격자(GridView 대응) → `LazyVerticalGrid`**

RecyclerView의 `GridLayoutManager`(격자, 본문 GridView의 현대 대응)에 해당하는 것이 Compose의 **`LazyVerticalGrid`**(가로 격자는 `LazyHorizontalGrid`)이다. 열 개수는 `GridCells`로 지정하는데, **고정 열 수**는 `GridCells.Fixed(n)`, **최소 너비 기반 적응형**은 `GridCells.Adaptive(minSize)`를 쓴다.

```kotlin
LazyVerticalGrid(
    columns = GridCells.Adaptive(minSize = 128.dp)   // 화면 폭에 맞춰 열 수 자동 결정
) {
...
> `GridCells.Adaptive(128.dp)`는 "각 셀이 최소 128dp 너비를 갖되 가능한 한 많은 열을 채우는" 방식으로, 폰/태블릿 양쪽에서 한 코드로 반응형 격자를 구성한다. ViewModel·`StateFlow`로 데이터를 끌어오면 리스트 변경 시 리컴포지션으로 자동 갱신된다(RecyclerView의 `DiffUtil` 역할을 `key` 기반 리컴포지션이 대신함).
> 근거: https://developer.android.com/develop/ui/compose/lists , https://developer.android.com/develop/ui/compose/migrate/migration-scenarios/recycler-view
...
> <a id="ox-7"></a>
> **7. 정답: O**
> **해설**: `android:prompt` 속성 또는 `setPrompt()` 메서드로 안내 메시지 표시 가능. ⚠️ 단, 실제 표시 위치는 모드에 따라 다르다 — `spinnerMode="dialog"`이면 **다이얼로그 제목**으로 표시되고, 기본값 `dropdown` 모드에서는 **무시되는 것이 일반적**이다. ([공식 문서](https://developer.android.com/reference/android/widget/Spinner))
...
- Android Developers - [Migrate RecyclerView to Lazy list (LazyColumn)](https://developer.android.com/develop/ui/compose/migrate/migration-scenarios/recycler-view)
...
- Android Developers - [AsyncListDiffer (submitList commitCallback)](https://developer.android.com/reference/androidx/recyclerview/widget/AsyncListDiffer)
