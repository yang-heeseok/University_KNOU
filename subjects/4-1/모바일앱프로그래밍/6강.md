...
2. AbsoluteLayout의 역할과 속성(`layout_x`, `layout_y` 속성)대해 이해할 수 있다.
...
| **`layout_x`** | 자식의 **좌상단 x 좌표**(좌→우 방향) — 값은 dimension이라 `dp`/`px` 모두 허용(공식 예시는 `px` 사용). AbsoluteLayout 자체가 deprecated라 "단위 기본값"을 단정하기는 어렵다 | x좌표, 가로 위치 |
| **`layout_y`** | 자식의 **좌상단 y 좌표**(상→하 방향) — 값은 dimension이라 `dp`/`px` 모두 허용(공식 예시는 `px` 사용) | y좌표, 세로 위치 |
| **deprecated 이유** | 기기별 화면 크기·해상도·DPI가 모두 달라 같은 좌표가 다른 시각적 결과를 낳음 → 적응형(adaptive) UI 불가 | 호환성, 적응형 UI 한계 |
| **deprecated 시점** | **API 3(Android 1.5 Cupcake)부터 이미 deprecated**로 표시됨. 그럼에도 교재가 다루는 이유는 ① "관계/좌표/누적"이라는 배치 패러다임 대비를 위한 학습용이며 ② 절대 좌표가 왜 적응형 UI에 부적합한지를 반례로 보여 주기 때문. 실무에서는 사용 금지. 근거: https://developer.android.com/reference/android/widget/AbsoluteLayout | API 3, Cupcake, 학습용 |
...
### 3. 🟢 심층 확장 — RelativeLayout → ConstraintLayout 마이그레이션

> 🟢 **2차 심층 확장**: 공식 문서로 검증한 보강이다. RelativeLayout은 deprecated는 아니지만, 공식 문서가 **성능과 도구 지원을 이유로 ConstraintLayout 사용을 명시적으로 권장**한다. ConstraintLayout은 RelativeLayout처럼 "형제·부모와의 관계"로 배치하되, **더 유연하고 플랫한(중첩 없는) 계층**으로 측정 비용을 줄인다. 근거: https://developer.android.com/develop/ui/views/layout/constraint-layout · https://developer.android.com/develop/ui/views/layout/relative

#### 왜 마이그레이션하나 (성능·평탄화 관점)

- **공통점**: ConstraintLayout도 RelativeLayout과 동일하게 "모든 View가 형제 View 및 부모 레이아웃과의 **관계(relationship)** 에 따라 배치"되는 모델이다. 따라서 관계 기반 사고방식이 그대로 이전된다.
- **개선점**: ConstraintLayout은 RelativeLayout/중첩 LinearLayout을 **단일 플랫 계층**으로 대체해 중첩 ViewGroup을 제거하고, **다중 measure 패스(레이아웃 패스 중복)** 를 줄인다. 공식 문서는 "더 나은 성능과 도구 지원을 위해 RelativeLayout 대신 ConstraintLayout으로 빌드하라"고 권고한다. ConstraintLayout은 FrameLayout을 제외하면 대체로 더 효율적이며 중첩의 필요를 없앤다. 근거: https://developer.android.com/develop/ui/views/layout/relative · https://developer.android.com/develop/ui/views/layout/improving-layouts/optimizing-layouts

#### 주요 속성 매핑표 (RelativeLayout → ConstraintLayout)

| RelativeLayout 속성 | ConstraintLayout 대응 속성 | 의미 |
| :--- | :--- | :--- |
| `layout_alignParentTop="true"` | `layout_constraintTop_toTopOf="parent"` | 부모 상단에 맞춤 |
| `layout_alignParentBottom="true"` | `layout_constraintBottom_toBottomOf="parent"` | 부모 하단에 맞춤 |
| `layout_alignParentStart="true"` | `layout_constraintStart_toStartOf="parent"` | 부모 시작(좌, RTL 안전) 끝에 맞춤 |
| `layout_alignParentEnd="true"` | `layout_constraintEnd_toEndOf="parent"` | 부모 끝(우, RTL 안전)에 맞춤 |
| `layout_below="@id/a"` | `layout_constraintTop_toBottomOf="@id/a"` | a의 아래(=내 상단을 a의 하단에) |
| `layout_above="@id/a"` | `layout_constraintBottom_toTopOf="@id/a"` | a의 위(=내 하단을 a의 상단에) |
| `layout_toEndOf="@id/a"` | `layout_constraintStart_toEndOf="@id/a"` | a의 오른쪽 옆(=내 시작을 a의 끝에) |
| `layout_toStartOf="@id/a"` | `layout_constraintEnd_toStartOf="@id/a"` | a의 왼쪽 옆(=내 끝을 a의 시작에) |
| `layout_centerHorizontal="true"` | `Start_toStartOf="parent"` + `End_toEndOf="parent"` | 양쪽 부모에 제약 → 가로 중앙 |
| `layout_centerVertical="true"` | `Top_toTopOf="parent"` + `Bottom_toBottomOf="parent"` | 위·아래 부모에 제약 → 세로 중앙 |
| `layout_centerInParent="true"` | 위 가로 중앙 + 세로 중앙 4개 제약 | 정중앙 |
| `layout_alignBaseline="@id/a"` | `layout_constraintBaseline_toBaselineOf="@id/a"` | 글자 baseline 정렬 |

> **마이그레이션 팁**: ConstraintLayout에서는 "중앙 정렬"이 단일 boolean이 아니라 **반대쪽 두 제약을 동시에** 거는 방식(양쪽 제약 → 가운데로 당겨짐)으로 표현된다. RelativeLayout의 `centerInParent="true"` 하나가 ConstraintLayout에서는 상·하·좌·우 4개 제약으로 펼쳐진다는 점이 가장 큰 사고 전환 포인트다. 또한 `Left/Right` 대신 `Start/End`를 쓰면 RTL(오른쪽→왼쪽) 언어에서도 자동으로 좌우가 뒤바뀌어 안전하다.
...
> - ① 오답 이유: `layout_alignParentTop`은 위쪽 끝 → 요구사항(우하단)과 반대.
...
**18.** AbsoluteLayout의 `layout_x`, `layout_y`를 dp로 명시해도 디바이스별 일관성 문제가 완전히 해결되지 않는다. [(#ox-18)](#ox-18) **(O / X)**
...
> <a id="ox-13"></a>
> **13. 정답**: X
> **해설**: A↔B 순환 참조는 RelativeLayout의 의존성 그래프에서 처리 불가능한 사이클을 만든다. aapt(리소스 컴파일러)나 inflate(레이아웃 로드) 단계에서는 잡히지 않고, **런타임 측정(measure/layout) 단계**에 이르러서야 `IllegalStateException: Circular dependencies cannot exist in RelativeLayout`이 발생한다. 절대 피해야 할 안티패턴.
...
> <a id="ox-18"></a>
> **18. 정답**: O
> **해설**: `layout_x`/`layout_y`는 dimension 값이라 `px`뿐 아니라 `dp`도 쓸 수 있다(공식 예시는 `px`를 사용). dp로 명시하면 밀도(density)에 따른 보정은 되지만, 화면 **크기**(작은 폰 vs 태블릿)와 **종횡비** 차이까지는 보정되지 않는다. 절대 좌표 자체가 적응형 UI의 본질적 한계이며, AbsoluteLayout이 deprecated된 이유다.
...
> 3. **FrameLayout — z-order = 추가 순서**: `addView` 호출 순서가 곧 자식의 그리기 순서다. 마지막 호출이 가장 위에 그려진다. 그러나 `foreground`는 모든 자식보다 한 단계 더 뒤(가장 위)에서 그려진다. 구체적으로 `View.draw()`는 ① 배경 → ② `onDraw()`(콘텐츠) → ③ `dispatchDraw()`(자식들) → ④ `onDrawForeground()` 순으로 진행하며, foreground는 이 마지막 단계인 **`onDrawForeground()`** 에서 그려진다(터치 핫스팟 전달용 `drawableHotspotChanged()`는 그리기 순서와 무관). 그래서 워터마크/오버레이 효과에 최적. 근거: https://developer.android.com/reference/android/view/View#onDrawForeground(android.graphics.Canvas)
...
### 1. ConstraintLayout이 RelativeLayout을 대체한다

`RelativeLayout`은 "다른 뷰/부모와의 관계"로 배치하는 방식이다. `ConstraintLayout`은 같은 관계 기반 배치를 **제약(constraint)** 으로 표현하면서, RelativeLayout이 못 하는 것(가중 분배, 가이드라인, 비율, 평탄한 계층)까지 제공한다. 그래서 Android Studio 레이아웃 에디터의 기본 레이아웃도 ConstraintLayout이다. 공식 문서도 "ConstraintLayout은 RelativeLayout과 마찬가지로 형제·부모와의 관계로 배치하지만 더 유연하고, 중첩 없는 평탄한 계층을 만든다"고 명시한다.

#### 1.1 RelativeLayout 속성 ↔ ConstraintLayout 제약 대응표

| RelativeLayout 속성 | ConstraintLayout 제약 |
|---------------------|------------------------|
| `layout_alignParentTop` | `layout_constraintTop_toTopOf="parent"` |
| `layout_alignParentBottom` | `layout_constraintBottom_toBottomOf="parent"` |
| `layout_alignParentStart` | `layout_constraintStart_toStartOf="parent"` |
| `layout_alignParentEnd` | `layout_constraintEnd_toEndOf="parent"` |
| `layout_centerHorizontal` | Start+End를 parent에 연결(양쪽 제약 → 중앙) |
| `layout_centerVertical` | Top+Bottom을 parent에 연결 |
| `layout_centerInParent` | 네 방향 모두 parent에 연결 |
| `layout_toEndOf="@id/a"` | `layout_constraintStart_toEndOf="@id/a"` |
| `layout_toStartOf="@id/a"` | `layout_constraintEnd_toStartOf="@id/a"` |
| `layout_below="@id/a"` | `layout_constraintTop_toBottomOf="@id/a"` |
| `layout_above="@id/a"` | `layout_constraintBottom_toTopOf="@id/a"` |
| `layout_alignBaseline` | `layout_constraintBaseline_toBaselineOf="@id/a"` |
| (해당 없음) | `layout_constraintWidth_percent` / 가이드라인 / 배리어 / 가중 체인 |

#### 1.2 성능·표현력 비교

| 항목 | RelativeLayout | ConstraintLayout |
|------|----------------|------------------|
| 배치 방식 | 형제/부모 관계 | 형제/부모 제약 + 가이드라인·배리어·비율 |
| 측정 패스 | 복잡한 의존 시 자식 **2회 측정** 가능 | 제약 솔버가 보통 1회로 해결(최적화됨) |
| 계층 깊이 | 가중 분배·복잡 정렬 시 중첩 필요 | 단일 계층으로 평탄화 가능 |

> 💡 RelativeLayout으로 표현하던 거의 모든 배치는 ConstraintLayout 제약으로 1:1 변환되며, 계층이 평탄해져 측정 비용이 줄어든다.

근거: ConstraintLayout 가이드 — https://developer.android.com/develop/ui/views/layout/constraint-layout , RelativeLayout API — https://developer.android.com/reference/android/widget/RelativeLayout

### 2. FrameLayout과 Jetpack Compose `Box`

`FrameLayout`의 "자식을 겹쳐 쌓는다"는 모델은 Compose의 `Box`에 그대로 대응된다. 둘 다 자식을 좌상단 기준으로 포개고, **나중에 선언된 자식이 위(앞)에 그려진다**.

#### 2.1 FrameLayout ↔ Compose Box 대응표

| FrameLayout (XML) | Compose `Box` |
|-------------------|----------------|
| `<FrameLayout>` 컨테이너 | `Box { ... }` |
| `android:layout_gravity="center"` | `Modifier.align(Alignment.Center)` |
| `layout_gravity="bottom\|end"` | `Modifier.align(Alignment.BottomEnd)` |
| 자식 선언 순서 = 쌓임 순서 | 자식 선언 순서 = 그리기 순서 |

```kotlin
Box(Modifier.fillMaxSize()) {
    Image(painterResource(R.drawable.background), null, Modifier.fillMaxSize())
    Text("중앙 텍스트", Modifier.align(Alignment.Center))   // 위에 겹침
}
```

#### 2.2 z-order(겹침 순서) 제어: 그리기 순서, elevation, zIndex

기본적으로 **그리기 순서(선언 순서)** 가 곧 z-order다. 늦게 그려진 것이 앞에 온다.

| 수단 | View 시스템 | Compose | 효과 |
|------|-------------|---------|------|
| 그리기 순서 | 자식 추가 순서 | 컴포저블 선언 순서 | 늦은 것이 위 |
| **elevation** | `android:elevation` | `Modifier.shadow(elevation)` | 그림자 + 높은 elevation이 위 |
| **zIndex** | — | `Modifier.zIndex(n)` | 같은 부모 내 그리기 순서 명시적 재정의(큰 값이 위) |

- `Modifier.zIndex(n)`: 더 큰 zIndex를 가진 자식이 작은 자식 위에 그려지며, 값이 같으면 부모가 배치한 원래 순서를 따른다(기본 zIndex=0).
- `elevation`은 단순 z-order뿐 아니라 **그림자(shadow)** 까지 동반한다. 평면적 겹침만 원하면 선언 순서나 `zIndex`가 적합하다.

근거: FrameLayout API — https://developer.android.com/reference/android/widget/FrameLayout , Compose `Box`/레이아웃 기초 — https://developer.android.com/develop/ui/compose/layouts/basics , `Modifier.zIndex` — https://developer.android.com/reference/kotlin/androidx/compose/ui/Modifier

### 3. 🟢 최신 사실 — `foreground`/`foregroundGravity`는 API 23(M)부터 모든 View 공통 속성으로 일반화됨

교재(및 본 노트의 정리하기/연습문제)는 `foreground`/`foregroundGravity`를 **FrameLayout의 속성**으로 다루며, **시험 기준으로는 이 분류가 정답**이다(FrameLayout 단원의 핵심 3속성: `foreground`/`foregroundGravity`/`measureAllChildren`). 다만 최신 안드로이드의 실제 구현을 보면, **`android:foreground`와 `android:foregroundGravity`는 API 23(Android 6.0, Marshmallow)부터 FrameLayout 전용이 아니라 `android.view.View`의 공통 속성으로 끌어올려져(generalize) 일반화**되었다. 즉 API 23 이상에서는 **모든 View(LinearLayout·RelativeLayout 등 ViewGroup 포함)** 가 자기 콘텐츠 위에 전경 Drawable을 덮어 그릴 수 있다.

| 구분 | API 23 이전 | API 23(M) 이후 |
| :--- | :--- | :--- |
| `foreground` 지원 범위 | **FrameLayout(및 일부 하위)만** | **모든 `View`** (ViewGroup 포함)로 일반화 |
| `foregroundGravity` 지원 범위 | FrameLayout만 | 모든 View |
| 시험 정답 분류 | FrameLayout 속성 | (교재 기준) FrameLayout 속성 — 변동 없음 |

> 📌 **요점**: "시험에서 `foreground` 계열은 FrameLayout 속성"이라는 답은 **교재 기준으로 그대로 유효**하다. 위 사실은 "실무에서 일반 LinearLayout 등에도 `android:foreground`가 먹는 이유"를 설명하는 **최신 보완**일 뿐이며, 이는 해당 속성이 API 23에서 `View` 클래스로 이동했기 때문이다. 근거: https://developer.android.com/reference/android/view/View#attr_android:foreground
