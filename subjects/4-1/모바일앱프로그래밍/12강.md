...
3. 🔴 **프래그먼트 생명주기**: 프래그먼트 **추가 단계**(onAttach → onCreate → onCreateView → **onViewCreated** → **onViewStateRestored** → ~~onActivityCreated~~(deprecated)) / **실행 단계**(onStart → onResume → onPause → onStop) / **종료 단계**(onDestroyView → onDestroy → onDetach)로 구성된다. ⚠️ 최신 권장 시퀀스는 `onAttach → onCreate → onCreateView → onViewCreated → onViewStateRestored → onStart → onResume`이며, `onActivityCreated()`는 **deprecated**(View 관련 초기화는 `onViewCreated`, 상태 복원은 `onViewStateRestored`로 대체). ([공식 문서](https://developer.android.com/guide/fragments/lifecycle))
...
5. 🟡 **프래그먼트 트랜잭션(FragmentTransaction)**: `beginTransaction()`으로 시작 → `add() / replace() / remove() / hide() / show()` 등으로 변경을 쌓고 → `commit()`으로 일괄 적용. **추가·변경·삭제 기능을 제공**한다.
   > 🟡 **`commit()` vs `commitNow()` vs `commitAllowingStateLoss()`**: `commit()`은 메인 스레드 큐에 등록 후 **비동기** 실행(백 스택 사용 가능), `commitNow()`는 **즉시 동기** 실행하되 백 스택과 함께 쓸 수 없다. 핵심 주의점은 **`onSaveInstanceState()` 이후에 `commit()`/`commitNow()`를 호출하면 `IllegalStateException`("Can not perform this action after onSaveInstanceState")이 발생**한다는 점이다. 상태 손실을 감수하고라도 강행해야 하면 `commitAllowingStateLoss()`를 쓰지만, 트랜잭션이 복원되지 않을 수 있으므로 남용은 피한다. [근거: https://developer.android.com/reference/androidx/fragment/app/FragmentTransaction ]
...
| **정적 컨테이너** | `<fragment android:name="..." android:id="@+id/frag1" .../>` ⚠️ | 액티비티 XML에 직접 박아넣기 (교체 불가). ⚠️ **구형 `<fragment>` 태그 대신 `FragmentContainerView`가 현재 표준 권장** (z-order·window insets 버그 회피, 심층확장 3 참조) |
| **FragmentContainerView (권장)** | `<androidx.fragment.app.FragmentContainerView android:id="@+id/container" android:name="..." .../>` | 정적 배치·동적 컨테이너 모두에 권장되는 표준 컨테이너. `android:name` 지정 시 내부적으로 `FragmentTransaction`으로 추가 |
| **동적 컨테이너** | `<FrameLayout android:id="@+id/container" .../>` | 런타임에 트랜잭션으로 교체 가능. ⚠️ 신규 코드는 `FrameLayout`보다 `FragmentContainerView` 권장 |
...
| **commit 비동기 주의** | `ft.commit();` (비동기) ↔ `fm.executePendingTransactions();` / `ft.commitNow();` | ⚠️ **`commit()`은 메인 스레드 큐에 등록되는 비동기**라, 호출 **직후 `findFragmentByTag()`가 아직 `null`**일 수 있다. 추가 결과를 즉시 참조해야 하면 `commit()` 후 `executePendingTransactions()`를 호출하거나, 처음부터 동기 실행되는 `commitNow()`를 쓴다(단 `commitNow()`는 `addToBackStack`과 함께 못 씀) |
| **태그로 찾기** | `MyFragment f = (MyFragment) fm.findFragmentByTag("TAG");` | 살아있는 프래그먼트 참조 획득 (⚠️ `commit()` 직후 즉시 호출하면 비동기 때문에 `null` 가능 — 위 행 참조) |
...
| **추가** | `onViewCreated(View, Bundle)` | `onCreateView` 직후 호출. **View 초기화·리스너 등록·ViewBinding 사용은 여기서** 수행 (View가 보장되는 시점) |
| **추가** | `onViewStateRestored(Bundle)` | 저장된 **View 계층 상태가 복원된** 시점. 복원된 위젯 상태에 의존하는 처리는 여기서 |
| **추가** | ~~`onActivityCreated(Bundle)`~~ ⚠️**deprecated** | 호스트 액티비티의 `onCreate()` 완료 시점. **deprecated** — View 초기화는 `onViewCreated`, 상태 복원은 `onViewStateRestored`로 대체 |
...
graph TD
    ...
    subgraph "프래그먼트 생명주기"
      ...
      L3 --> L3a[onViewCreated] --> L3b[onViewStateRestored]
      L3b --> L4[onStart/onResume]
...
| **프래그먼트 → 프래그먼트 (권장 대안)** | **Fragment Result API** (androidx, 1회성 결과 전달) | (수신) `setFragmentResultListener("key", this, (k, bundle) -> {...})` / (송신) `setFragmentResult("key", bundle)` |

> **공식 권장 대안 — Fragment Result API**: 콜백 인터페이스를 직접 정의하지 않고도, `FragmentManager`를 통해 프래그먼트끼리(또는 프래그먼트↔액티비티) **느슨하게 결합된 1회성 결과**를 주고받을 수 있다. 수신 측은 `setFragmentResultListener(requestKey, lifecycleOwner, listener)`로 같은 `requestKey`의 결과를 구독하고, 송신 측은 `setFragmentResult(requestKey, Bundle)`로 전달한다. 인터페이스/캐스팅·강결합 없이 동작하며, 수신 측이 `STARTED` 상태가 될 때 결과가 안전하게 전달된다. ([공식 문서](https://developer.android.com/guide/fragments/communicate))
...
### 3. 🟢 심층 확장: FragmentContainerView — 정적 `<fragment>` 태그 대신 권장

레이아웃에 프래그먼트를 정적으로 배치할 때, 공식 문서는 구형 `<fragment>` 태그 대신 **`FragmentContainerView` 사용을 강력히 권장**한다("It is strongly recommended that the container is a FragmentContainerView ... when performing fragment transactions"). `<fragment>` 태그는 프래그먼트가 자신의 `FragmentManager` 상태를 벗어나게 만들고, **종료(exit) 애니메이션이 다른 프래그먼트 위에 그려지는 z-order 버그**와 **window insets 분배 문제**를 일으킨다. `FragmentContainerView`는 이를 해결한다.

```xml
<!-- 권장: FragmentContainerView -->
...
`android:name`을 지정하면 직접 인플레이트가 아니라 **내부적으로 `FragmentTransaction`을 통해** 프래그먼트를 추가하므로, `<fragment>` 태그에서 발생하던 라이프사이클·구성 변경 불일치가 사라진다. 동적으로 교체할 컨테이너로 쓸 때는 `android:name`을 생략하고 트랜잭션으로 채운다.

### 4. 🟢 심층 확장: Navigation Component — NavGraph/NavController + Safe Args

수동 `FragmentTransaction` 대신, 프래그먼트 전환을 **NavGraph(내비게이션 그래프)** 로 정의하고 **`NavController`** 로 이동시키는 방식이 권장된다. 인자는 **Safe Args** Gradle 플러그인이 생성하는 타입 안전 클래스로 전달한다. 공식 문서는 타입 안전성 때문에 데이터 전달에 Safe Args를 강력히 권장한다("Safe Args is strongly recommended for navigating and passing data, because it ensures type safety").

```kotlin
// build.gradle 플러그인: androidx.navigation.safeargs.kotlin
// 이동(타입 안전): 목적지별 생성 Directions 클래스
val action = ListFragmentDirections.actionListToDetail(itemId = 42)
findNavController().navigate(action)

// 수신 측: 생성된 Args 클래스 + navArgs() 델리게이트
val args: DetailFragmentArgs by navArgs()
val id = args.itemId   // String 키 없이 타입 보장
```

#### 수동 FragmentTransaction 대비 표

| 항목 | 수동 FragmentTransaction | Navigation Component |
| :--- | :--- | :--- |
| **전환 정의** | 코드에 분산(add/replace/commit) | NavGraph(XML)로 한곳에 시각화 |
| **인자 전달** | `Bundle` + String 키(런타임 오류 위험) | Safe Args 생성 클래스(**컴파일 타임** 타입 검사) |
| **백스택 관리** | `addToBackStack` 수동 호출 | 그래프 기반 자동 관리 |
| **딥링크** | 직접 구현 | 그래프에 선언적으로 지원 |
| **오류 시점** | 키 오타·타입 불일치가 런타임 크래시 | 빌드 시점에 검출 |

> Safe Args는 목적지마다 `XxxDirections`(이동용)와 `XxxArgs`(수신용) 클래스를 생성해 `Bundle`의 String 키 오타·타입 불일치를 컴파일 단계에서 차단한다. 단, `int`·`float`·`long`·`bool` 등 원시형은 non-null이고 `String`·`Parcelable`·`Serializable`만 null을 허용한다.

### 5. 🟢 심층 확장: 상태 공유 — 공유 ViewModel vs Fragment Result API

공식 가이드의 기준은 명확하다: **"To share persistent data with custom APIs, use a ViewModel. For a one-time result with data that can be placed in a Bundle, use the Fragment Result API."**

| 구분 | 공유 ViewModel (`by activityViewModels()`) | Fragment Result API |
| :--- | :--- | :--- |
| **범위** | 호스트 액티비티 범위로 여러 프래그먼트가 동일 인스턴스 공유 | FragmentManager를 통한 1회성 결과 전달 |
| **적합한 경우** | 지속적·반응형 데이터 공유, 실시간 동기화(목록↔필터 등) | 단발성 결과(다이얼로그 선택, QR 스캔 결과 반환 등) |
| **관찰 방식** | `LiveData`/`StateFlow` 관찰로 자동 갱신 | `setFragmentResultListener`로 1회 수신 |
| **결합도** | 프래그먼트끼리 서로를 몰라도 됨(액티비티 범위 공유) | 동일 `requestKey`만 공유, 느슨한 결합 |
| **주의점** | 일회성 이벤트에 쓰면 화면 재생성 시 재전달 문제 | 지속 상태 공유에는 부적합 |

```kotlin
// 공유 ViewModel: 같은 액티비티의 두 프래그먼트가 동일 인스턴스 공유
private val sharedVm: SharedViewModel by activityViewModels()
```

원칙: **지속적으로 공유·관찰할 상태는 공유 ViewModel**, **단발성 결과 반환은 Fragment Result API**를 쓴다. 일회성 이벤트를 공유 ViewModel에 두면 화면 재생성 시 이벤트가 다시 전달되는 문제가 생길 수 있다.

### 6. 🟢 심층 확장: `getChildFragmentManager()` vs `getParentFragmentManager()` — 중첩 프래그먼트

본문 요약 4번의 `getSupportFragmentManager()`는 **액티비티**가 자신의 프래그먼트를 관리할 때 쓰는 매니저다. 그런데 프래그먼트 **안에 또 프래그먼트**를 넣는 **중첩(nested) 프래그먼트** 구조(예: `ViewPager2` 페이지 안에 탭 콘텐츠 프래그먼트)에서는 "어느 FragmentManager를 쓰느냐"가 달라진다. 프래그먼트는 **두 개의 서로 다른 FragmentManager**를 본다.

| 메서드 | 무엇을 가리키나 | 누가 호출하나 | 용도 |
| :--- | :--- | :--- | :--- |
| `getParentFragmentManager()` | **나를 담고 있는** 호스트(부모 프래그먼트 또는 액티비티)의 FragmentManager | 자식 프래그먼트가 **부모/호스트 쪽**으로 | 형제 프래그먼트 교체, 부모에게 결과 전달(`setFragmentResult`) |
| `getChildFragmentManager()` | **내 안에서** 관리하는 자식 프래그먼트들의 FragmentManager | 부모 프래그먼트가 **자기 자식들을** | 프래그먼트 안에 중첩 프래그먼트 add/replace, 자식 결과 수신 |

```kotlin
// [부모 프래그먼트] 자기 컨테이너 안에 자식 프래그먼트를 동적으로 추가
childFragmentManager.beginTransaction()
    .replace(R.id.inner_container, InnerFragment())
    .commit()

// [부모 프래그먼트] 자식이 보낸 결과를 수신할 때 — 반드시 childFragmentManager 사용
childFragmentManager.setFragmentResultListener("requestKey", this) { key, bundle ->
    val value = bundle.getString("bundleKey")
}

// [자식 프래그먼트] 부모에게 결과 전달 — parentFragmentManager로 송신
setFragmentResult("requestKey", bundleOf("bundleKey" to result))
```

> **공식 가이드 핵심**: "To pass a result from a child fragment to a parent, use `getChildFragmentManager()` from the parent fragment instead of `getParentFragmentManager()` when calling `setFragmentResultListener()`." — 즉 **부모가 자식의 결과를 받으려면 `getChildFragmentManager()`** 로 리스너를 등록해야 하며, `getParentFragmentManager()`를 쓰면 매니저 범위가 어긋나 결과를 받지 못한다.

> ⚠️ **흔한 버그**: 중첩 구조에서 `getParentFragmentManager()`와 `getChildFragmentManager()`를 혼동하면 트랜잭션이 엉뚱한 매니저에 쌓여 프래그먼트가 보이지 않거나 결과 콜백이 호출되지 않는다. "**내 자식은 child, 내 호스트는 parent**"로 외운다.

- 근거: [Communicate with fragments — Pass results between a parent and child fragment](https://developer.android.com/guide/fragments/communicate#parent-child) · [Fragment#getChildFragmentManager()](https://developer.android.com/reference/androidx/fragment/app/Fragment#getChildFragmentManager())

### 7. 🟢 심층 확장: `Fragment(@LayoutRes)` 생성자 — `onCreateView` 오버라이드 생략

본문 코드 예시(`DetailFragment`)는 `onCreateView()`를 오버라이드해 `inflater.inflate(...)`로 레이아웃을 직접 부풀린다. AndroidX Fragment에는 더 간결한 현대 패턴이 있다: **레이아웃 리소스 ID를 받는 생성자 `Fragment(@LayoutRes int contentLayoutId)`** 를 호출하면, 프레임워크가 **적절한 시점에 그 레이아웃을 자동으로 inflate**해 주므로 `onCreateView()`를 **아예 작성하지 않아도 된다**.

```kotlin
// 현대 패턴: 생성자에 레이아웃 ID만 넘기면 onCreateView 불필요
class DetailFragment : Fragment(R.layout.fragment_detail) {
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        // View 초기화·리스너 등록·ViewBinding 은 여기서 (View가 보장되는 시점)
        val id = arguments?.getInt("ID", -1) ?: -1
        view.findViewById<TextView>(R.id.tvDetail).text = "선택된 ID: $id"
    }
}
```

```java
// Java도 동일 — super(레이아웃ID) 호출
public class DetailFragment extends Fragment {
    public DetailFragment() { super(R.layout.fragment_detail); }  // ★ onCreateView 생략
...
| 구분 | `onCreateView()` 직접 오버라이드(본문 예시) | `Fragment(@LayoutRes)` 생성자(현대) |
| :--- | :--- | :--- |
| **레이아웃 inflate** | 개발자가 직접 `inflater.inflate(...)` 호출 | 프레임워크가 자동으로 처리 |
| **코드량** | `onCreateView` 메서드 필요 | 생성자 한 줄, View 초기화는 `onViewCreated`로 |
| **View 초기화 위치** | `onCreateView` 또는 `onViewCreated` | `onViewCreated`(권장) |
| **적합한 경우** | inflate 과정에 커스텀 로직이 필요할 때 | 단순히 XML 레이아웃을 그대로 띄울 때 |

> **공식 문서 핵심**: "Subclasses of Fragment can ... call into a constructor on Fragment that takes an `R.layout` ID, indicating the layout that should be used for this fragment **as an alternative to overriding `onCreateView()`**." 그리고 "automatically inflates the view at the appropriate time. The inflated layout can be configured in `onViewCreated()`." — 즉 단순 레이아웃 표시라면 생성자 방식이 더 간결하고, View 설정은 `onViewCreated()`에서 한다(생명주기 표의 `onViewCreated` 시점과 일치).

> 📌 **본문 패턴과의 호환**: 생성자 방식을 써도 `setArguments(Bundle)` + `newInstance()` 패턴(오개념 3·문제6 참조)은 그대로 유효하다. 인자는 여전히 `Bundle`로 넘기고, 생성자에는 **레이아웃 ID만** 전달한다(인자 데이터를 생성자로 넘기면 안 됨).

- 근거: [Create a fragment (Fragment(@LayoutRes) 생성자)](https://developer.android.com/guide/fragments/create) · [Fragment | API reference](https://developer.android.com/reference/androidx/fragment/app/Fragment) · 1.0 릴리스 노트(생성자 도입): [Fragment releases](https://developer.android.com/jetpack/androidx/releases/fragment)

### 8. 🟢 심층 확장: Compose Navigation — 프래그먼트 없는 화면 전환

지금까지(섹션 4)는 **프래그먼트를 화면 단위**로 두고 `NavController`로 전환했다. Jetpack Compose에서는 **프래그먼트·`FragmentTransaction` 자체가 사라지고**, 화면이 **컴포저블 함수**가 된다. `androidx.navigation.compose`의 `NavHost` 안에서 각 목적지를 `composable("route")`로 정의하고, 같은 `NavController`로 `navigate("route")` 하여 이동한다.

```kotlin
// build.gradle: implementation("androidx.navigation:navigation-compose:...")
...
| 항목 | 프래그먼트 + FragmentTransaction (전통) | Compose Navigation (`navigation-compose`) |
| :--- | :--- | :--- |
| **화면 단위** | `Fragment` 클래스 + 레이아웃 XML | `@Composable` 함수 |
| **전환 수단** | `ft.replace(...).addToBackStack().commit()` | `navController.navigate("route")` |
| **백 스택** | `addToBackStack()` 수동 | `NavHost`가 라우트 기반 자동 관리 |
| **인자 전달** | `setArguments(Bundle)` | 라우트 경로 인자 또는 타입 안전 라우트(Kotlin DSL) |
| **컨테이너** | `FragmentContainerView`/`FrameLayout` | `NavHost` 컴포저블 |

> 흐름 정리: View 시스템에서는 *프래그먼트 ↔ 트랜잭션 ↔ FragmentManager* 가 핵심이지만, Compose로 넘어가면 *컴포저블 ↔ navigate ↔ NavController/NavHost* 로 대응 개념이 옮겨간다. 단일 액티비티 + 컴포저블 화면 구조가 권장된다.
> - 근거: [Navigation with Compose](https://developer.android.com/develop/ui/compose/navigation)
...
> **해설**: 추가 단계는 항상 `onAttach → onCreate → onCreateView → (onViewCreated → onViewStateRestored)` 순(과거의 `onActivityCreated`는 deprecated되어 이 둘로 분리·대체됨 — 본문 요약 3번·생명주기 표와 일관). 실행 단계는 `onStart → onResume → onPause → onStop`. 종료 단계는 `onDestroyView → onDestroy → onDetach`. ②만 이 순서를 지킨다. ①·③은 onAttach와 onCreate의 선후 관계가 어긋나고, ④는 onDetach가 onDestroyView보다 먼저라 잘못이다. (정답 ② 유지)
...
>
> **(5) ViewBinding 메모리 누수 방지 (필수 패턴)**: 프래그먼트는 인스턴스보다 View의 수명이 짧다(`onDestroyView`에서 View만 먼저 파괴됨). 따라서 `onDestroyView()`에서 **binding 참조를 `null`로 해제**해야 누수가 발생하지 않는다. 또한 LiveData·Flow 관찰 시에는 `getActivity()`/`this`가 아니라 **`getViewLifecycleOwner()`** 를 LifecycleOwner로 사용해, View가 파괴되면 관찰도 자동 해제되도록 해야 한다.
> ```java
> private FragmentDetailBinding binding;          // nullable
> @Override public View onCreateView(LayoutInflater i, ViewGroup c, Bundle b) {
>     binding = FragmentDetailBinding.inflate(i, c, false);
>     return binding.getRoot();
> }
> @Override public void onViewCreated(@NonNull View v, Bundle b) {
>     super.onViewCreated(v, b);
>     viewModel.getData().observe(getViewLifecycleOwner(), d -> binding.tvDetail.setText(d));
> }
> @Override public void onDestroyView() {
>     super.onDestroyView();
>     binding = null;                             // ★ View 누수 방지
> }
> ```
...
**15.** `add(..., "TAG").commit()` 호출 **직후** 곧바로 `findFragmentByTag("TAG")`를 호출하면 항상 추가된 프래그먼트를 반환한다. [(해설)](#ox-15) **(O / X)**
...
> <a id="ox-15"></a>
> **15. 정답**: X
> **해설**: `commit()`은 **비동기**라 큐에 등록만 되고 즉시 실행되지 않으므로, 호출 **직후** `findFragmentByTag()`는 아직 추가가 반영되지 않아 **`null`을 반환할 수 있다**. 즉시 참조가 필요하면 `commit()` 후 `fm.executePendingTransactions()`로 보류 트랜잭션을 강제 실행하거나, 처음부터 동기 실행되는 `commitNow()`를 사용한다(단 `commitNow()`는 `addToBackStack`과 함께 쓸 수 없음). ([FragmentTransaction](https://developer.android.com/reference/androidx/fragment/app/FragmentTransaction) · [FragmentManager#executePendingTransactions](https://developer.android.com/reference/androidx/fragment/app/FragmentManager#executePendingTransactions()))
...
3. **생명주기 동기화**: 큐를 처리하면서 각 op에 대해 (a) `replace`된 기존 프래그먼트는 `onPause → onStop → onDestroyView → onDestroy → onDetach`, (b) 새 프래그먼트는 `onAttach → onCreate → onCreateView → onViewCreated → onViewStateRestored → onStart → onResume` 순으로 콜백을 호출한다. (⚠️ 과거의 `onActivityCreated`는 deprecated되어 `onViewCreated`/`onViewStateRestored`로 분리·대체되었다.)
...
- Android Developers: [FragmentContainerView](https://developer.android.com/reference/androidx/fragment/app/FragmentContainerView)
- Android Developers: [Fragment transactions](https://developer.android.com/guide/fragments/transactions)
- Android Developers: [Navigation 시작하기](https://developer.android.com/guide/navigation)
- Android Developers: [Pass data between destinations (Safe Args)](https://developer.android.com/guide/navigation/use-graph/pass-data)
- Android Developers: [Create a fragment (Fragment(@LayoutRes) 생성자)](https://developer.android.com/guide/fragments/create)
- Android Developers: [Fragment#getChildFragmentManager() / getParentFragmentManager()](https://developer.android.com/reference/androidx/fragment/app/Fragment)
- Android Developers: [Communicate between a parent and child fragment](https://developer.android.com/guide/fragments/communicate#parent-child)
