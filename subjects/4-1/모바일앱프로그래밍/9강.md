...
### 안드로이드 이벤트 처리 방식 (4가지 + 람다 변형)

> ⚠️ 매니페스트(AndroidManifest.xml)는 **앱 구성 정보(액티비티 등록, 권한 등)** 를 선언하는 곳이지, **이벤트 처리 위치가 아니다**. (📎 9강 기출 Q1)

> 📌 **개수 정정**: 교재가 말하는 "이벤트 처리 **4가지** 방식"은 아래 표의 **①~④**(콜백 재정의 / 리스너 인터페이스 구현 / 익명 이너 클래스 / 액티비티 자신이 리스너)를 가리킨다. **⑤ 람다식은 독립된 5번째 방식이 아니라 ②~④(특히 익명 이너 클래스)를 간결하게 쓰는 "변형(표기법)"** 이므로, "4가지"에 포함되지 않는 **별도 변형**으로 본다. 따라서 표에서도 ⑤를 회색 변형 행으로 구분한다.
...
| (＋변형) | **람다식(SAM 변환)** *— 4가지에 포함 안 됨, ②~④의 간결 표기* | ②~④의 간결 표현. **추상 메소드가 1개인 인터페이스(SAM)** 에만 적용 가능 | `OnClickListener`처럼 메소드 1개짜리에만 OK. 메소드가 여러 개인 리스너는 람다로 변환 불가 |
...
- **해결**:
  ...
  - 📌 **주의(도달 범위)**: **버튼처럼 클릭 가능한 View 위를 터치하면 그 터치를 버튼이 소비(consume)** 하므로, 액티비티의 `onTouchEvent()`에는 **도달하지 않는다**. `Activity.onTouchEvent()`로 들어오는 것은 **어떤 자식 View도 소비하지 않은 터치**, 즉 사실상 **버튼·위젯이 없는 빈 영역을 터치한 경우**뿐이다. 그래서 "화면 전체 로그"라고 해도 버튼 위 탭은 빠질 수 있음을 전제로 설계해야 한다(누락 없는 전역 로깅이 필요하면 `dispatchTouchEvent()` 오버라이드가 더 적합).
...
> 🔁 **대칭 사실(중요)**: 반대로, 최초에 `setOnClickListener(리스너)`를 **non-null 리스너로 호출하면 그 View는 자동으로 `clickable=true`가 된다**(`setOnClickListener`의 공식 동작 — 리스너를 등록하면 내부적으로 클릭 가능 상태를 켠다). 그래서 평소 클릭 불가였던 `TextView`에 클릭 리스너만 달아도 곧바로 클릭이 동작하는 것이다. 정리하면 **non-null 등록 → `clickable` 자동 ON**, 그러나 **`null`로 해제 → `clickable`은 자동 OFF되지 않고 유지**(비대칭)다. 근거: https://developer.android.com/reference/android/view/View#setOnClickListener(android.view.View.OnClickListener)
...
- 🟢 **롱클릭 ↔ 클릭 소비 관계**: `OnLongClickListener.onLongClick()`이 **`true`를 반환하면 그 롱클릭이 "소비"** 된 것으로 처리되어, 같은 제스처의 **후속 `ACTION_UP`에서 `performClick()`이 호출되지 않는다 → `OnClickListener.onClick()`도 발화되지 않는다**. 반대로 `onLongClick()`이 `false`를 반환하면 롱프레스가 소비되지 않아, 손을 뗄 때 일반 클릭으로 이어져 `onClick()`이 호출될 수 있다. 즉 "길게 눌렀다가 떼면 클릭까지 두 번 발생"하는 것을 막으려면 **`onLongClick`에서 `true`를 반환**하면 된다. 근거: https://developer.android.com/guide/topics/ui/accessibility/custom-views · https://developer.android.com/reference/android/view/View.OnLongClickListener
