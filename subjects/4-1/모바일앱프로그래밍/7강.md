...
- **관련 단원**: [5강](5강.md) (LinearLayout, 중첩 시 가장 흔히 결합) · [8강. 도형 및 소리 출력](8강.md) (다음 강의)
...
>
> ⚠️ **함정 — 셀의 `layout_width`는 무시된다**: `TableRow`의 자식(셀)은 기본 `layout_width`가 **`MATCH_PARENT`로 강제**되어, 셀에 `wrap_content`나 고정 dp를 직접 지정해도 **무시**된다(폭은 열(column) 단위로 TableLayout이 합의해 결정). 반면 `layout_height`의 기본값은 `WRAP_CONTENT`이고 개별 지정이 가능하다. 그래서 셀 폭은 `layout_width`가 아니라 `stretchColumns`/`shrinkColumns`/`layout_column`/`layout_span` 같은 **열 기반 속성**으로 제어해야 한다. 시험·실무 빈출 함정. 근거: https://developer.android.com/reference/android/widget/TableRow.LayoutParams
...
| ⚠️ **단위 대비** | `setTextSize(float)` **1-인자** 버전은 인자를 **sp 단위로 해석**(다른 단위는 2-인자 `setTextSize(int unit, float size)`로 명시). 반면 XML `android:textSize`는 **단위를 직접 명시**(`"20sp"`, `"16dp"` 등)해야 한다 — 코드/XML 혼동 주의 | TextView 계열 | `tv.setTextSize(20)`(=20sp) vs `android:textSize="20sp"` |
| ⚠️ **클래스별 단위 차이** | ⚠️ 위 "1-인자 = sp" 규칙은 **`TextView` 계열 한정**이다. 8강 Canvas에서 쓰는 **`Paint.setTextSize(float)`는 단위가 sp가 아니라 픽셀(px)** 이다(밀도/접근성 글꼴 배율 미반영). 즉 같은 이름의 `setTextSize`라도 대상 클래스에 따라 단위가 다르므로 혼동에 주의한다. 근거: `TextView.setTextSize`=sp — https://developer.android.com/reference/android/widget/TextView#setTextSize(float) · `Paint.setTextSize`=px — https://developer.android.com/reference/android/graphics/Paint#setTextSize(float) | TextView=sp / Paint=px | `tv.setTextSize(20)`(=20sp) vs `paint.setTextSize(20)`(=20px) |
...
### 3. 🟢 심층 확장 — 격자(grid) 배치 대안 4종 비교: 언제 무엇을 쓰나

> 🟢 **2차 심층 확장**: 공식 문서로 검증한 보강이다. "표/격자형 UI"를 만드는 방법은 TableLayout 외에도 여러 가지가 있다. 각각의 모델·셀 병합 방식·스크롤(대량 데이터) 적합성을 비교해 "언제 무엇을 쓸지" 판단 기준을 정리한다. 근거(GridLayout): https://developer.android.com/reference/android/widget/GridLayout

| 대안 | 배치 모델 | 셀 병합 / 위치 지정 | 스크롤·대량 데이터 | 언제 쓰나 |
| :--- | :--- | :--- | :--- | :--- |
| **TableLayout** | `TableRow`(행) 안에 자식이 열을 형성. 열 폭은 행 간 공유 | `layout_span`(병합), `layout_column`(열 위치). 셀 `layout_width`는 무시 | ❌ 자체 스크롤 없음(ScrollView로 감싸야 함, 가상화 없음) | **소량의 정적 라벨/입력 쌍** 폼(설정·로그인). 행 수가 적고 고정적일 때 |
| **GridLayout** | 행·열 격자를 `rowSpec`/`columnSpec`으로 직접 지정. TableLayout과 달리 **중첩 없이** 2차원 배치 가능 | `GridLayout.spec(start, size)`로 행/열 **span** 지정. `rowCount`/`columnCount`로 격자 크기 정의 | ❌ 가상화 없음 | **불규칙한 셀 크기·병합이 많은** 정적 격자(계산기 키패드, 대시보드 타일). LinearLayout 중첩을 평탄화하고 싶을 때. 근거: https://developer.android.com/reference/android/widget/GridLayout |
| **ConstraintLayout + Flow** | ConstraintLayout 헬퍼 `Flow`가 참조된 View들을 가로/세로로 자동 줄바꿈(wrap)하며 가상 격자처럼 흐르게 배치 | `flow_wrapMode`(none/chain/aligned), `flow_maxElementsWrap`로 한 줄 개수 제어 | ❌ 가상화는 아님(View가 실제로 inflate됨) | **개수가 가변적인 칩/태그**를 평탄 계층으로 흐르게 배치. 반응형 줄바꿈이 필요할 때. 근거: https://developer.android.com/develop/ui/views/layout/constraint-layout |
| **Compose `LazyVerticalGrid`** | Jetpack Compose의 **지연(lazy) 격자**. 화면에 보이는 셀만 컴포즈/재활용 | `GridCells.Fixed(n)`/`Adaptive(minSize)`로 열 수 결정, `span`으로 셀 병합 | ✅ **가상화(virtualization)** — 수천 개 아이템도 메모리 효율적 | **대량/스크롤되는 격자**(이미지 갤러리, 상품 그리드). 동적 데이터가 많을 때 |

> **선택 가이드(요약)**:
> - **소량·정적 폼(라벨/입력)** → **TableLayout** (XML이 짧고 행/열 모델이 명시적).
> - **정적이지만 불규칙·병합 많은 격자** → **GridLayout** (중첩 제거, spec 기반 2차원).
> - **개수 가변·반응형 줄바꿈(칩/태그)** → **ConstraintLayout + Flow** (평탄 계층 유지).
> - **대량·스크롤** → **Compose `LazyVerticalGrid`** (가상화로 성능 확보). View 시스템이라면 `RecyclerView` + `GridLayoutManager`가 동일 역할.
>
> **핵심 구분점**: TableLayout·GridLayout·Flow는 **모든 셀을 실제로 inflate**하므로 데이터가 많아지면 메모리·measure 비용이 선형으로 증가한다. 반면 `LazyVerticalGrid`(및 RecyclerView)는 **보이는 셀만 그리는 가상화**를 제공해, "표가 화면을 넘어 스크롤되는가 / 아이템이 많은가"가 둘을 가르는 결정적 기준이다.
...
> 그래서 "**가능하면 계층을 얕게(평탄하게) 유지하고** ConstraintLayout으로 평탄화하라"는 가이드가 나온 것이다. 흔히 인용되는 "트리 깊이 10단계 이하"는 **딱 떨어지는 공식 수치라기보다 일반적 통념**에 가깝다 — 공식 가이드는 정확한 한계 숫자를 못 박지 않고 "깊고 불필요한 중첩을 줄여 measure/layout 비용을 낮추라"는 **정성적 권고**를 한다. 근거: https://developer.android.com/topic/performance/rendering/optimizing-view-hierarchies
...
### 1. TableLayout의 대안 — GridLayout / ConstraintLayout Flow / Compose

`TableLayout`은 `TableRow` 단위로 행을 만들고 각 행이 셀을 갖는 **행 중심** 구조라, 셀이 자동으로 줄바꿈되거나 스크롤되어야 하는 경우엔 부적합하고 중첩이 늘기 쉽다. 또 자체적으로 스크롤하지 못해 `ScrollView`로 감싸야 하며 데이터를 모두 미리 채워야 한다. 격자/표 형태 UI는 용도에 따라 다음 대안이 더 적합하다.

#### 1.1 격자 레이아웃 대안 비교표

| 방식 | 핵심 특징 | 적합한 경우 | 한계 |
|------|-----------|-------------|------|
| **TableLayout** | `TableRow`로 행 정의, 열 자동 정렬, `stretch/shrinkColumns` | 폼처럼 행·열 수가 고정된 표 | 셀 자동 줄바꿈·스크롤 불가, 중첩 유발 |
| **GridLayout** | 행·열 인덱스(`layout_row`/`layout_column`)로 배치, 셀 병합(`columnSpan`) | TableRow 없이 평탄한 격자, 셀 병합 필요 | 동적 대량 데이터엔 부적합(미재활용) |
| **ConstraintLayout Flow** | 가상 헬퍼가 자식을 행/열로 흘려보내며 줄바꿈(wrap) | 개수 가변, 한 ConstraintLayout 안에서 칩/태그 흐름 | 진짜 표 정렬보다는 흐름 배치 |
| **Compose `Row`+`Column`** | 선언형으로 행/열 조합, 적은 셀 수 | 고정 소규모 격자 | 대량은 비효율 |
| **Compose `LazyVerticalGrid`** | 칼럼 수(`GridCells.Fixed/Adaptive`)로 격자 + **지연/재활용** | 스크롤되는 대량 격자(갤러리 등) | 단순 고정 표엔 과함 |

> 💡 **고정 폼 표** → GridLayout(같은 UI에서 TableLayout보다 빠르고 메모리도 적게 쓴다 — 아래 근거 참조), **가변 흐름** → ConstraintLayout Flow, **스크롤 대량 격자** → `LazyVerticalGrid`(RecyclerView `GridLayoutManager`에 해당). TableLayout은 단순 고정 표 외에는 대안을 우선 고려한다.
>
> 📌 **"GridLayout이 TableLayout보다 빠르고 메모리도 적다"의 근거**: `GridLayout`은 Android 4.0(API 14)에서 도입될 때, **표/격자 UI를 만들 때 `TableLayout`이나 중첩 `LinearLayout`이 강제하는 깊은 뷰 중첩을 제거**하도록 설계되었다. 구글 안드로이드 개발자 블로그의 GridLayout 소개 글은 GridLayout이 행·열을 **하나의 평탄한 ViewGroup**으로 표현해 중첩 단계를 줄이고, 이로써 measure/layout 패스가 단순해진다고 설명한다(중첩이 줄면 measure 횟수·뷰 객체 수가 줄어 그만큼 시간·메모리가 절약된다 — 같은 UI 기준). 즉 "더 빠르고 가볍다"는 절대 벤치마크 수치라기보다 **깊은 중첩 제거에서 오는 구조적 이점**으로 이해하는 것이 정확하다. 근거: https://android-developers.googleblog.com/2011/11/new-layout-widgets-space-and-gridlayout.html · https://developer.android.com/reference/android/widget/GridLayout

```kotlin
// Compose: 스크롤되는 격자 — 항목 재활용으로 대량에도 효율적
LazyVerticalGrid(columns = GridCells.Fixed(3)) {
    items(photoList) { photo -> PhotoCell(photo) }
}
```

근거: GridLayout API — https://developer.android.com/reference/android/widget/GridLayout , ConstraintLayout `Flow` — https://developer.android.com/reference/androidx/constraintlayout/helper/widget/Flow , `LazyVerticalGrid` — https://developer.android.com/develop/ui/compose/lists#lazy-grids

### 2. 중첩 깊이가 성능을 해치는 원리와 측정 비용

#### 2.1 왜 깊은 계층이 느린가 — 측정 횟수의 폭발

레이아웃은 `measure → layout → draw`를 거치는데, 깊은 중첩이 비싼 이유는 **measure 패스가 계층을 따라 재귀**하기 때문이다.

- 부모는 자식을 측정하려고 자식의 `onMeasure()`를 호출하고, 그 자식은 또 자기 자식을 측정한다 → 측정 비용이 **계층 깊이에 따라 누적**된다.
- 더 나쁜 경우는 **이중 측정(double taxation)** 위젯이다. `weight`를 쓰는 `LinearLayout`(5강 심층확장 참고)이나 복잡한 의존을 가진 `RelativeLayout`은 자식을 **두 번** 측정한다.
- 이런 "2회 측정" 컨테이너가 **중첩**되면 측정 횟수가 깊이에 대해 곱셈으로 늘어난다. 예: 2-pass 컨테이너가 3단계 중첩되면 최악의 경우 가장 안쪽 자식이 약 2×2×2 = **8회** 측정될 수 있다.

```
LinearLayout(weight)   ← 2회 측정
  └ RelativeLayout      ← 2회 측정
      └ LinearLayout(weight)  ← 2회 측정
          └ 실제 콘텐츠   ← 최대 8회 측정 (2×2×2)
```

> 이것이 "깊은 중첩을 줄여라", "merge로 ViewGroup을 없애라"가 강조되는 근본 이유다. ConstraintLayout으로 **평탄화**하면 깊이가 줄고, 제약 솔버가 보통 한 번의 패스로 해결한다.

#### 2.2 진단·권고 도구

| 도구 | 무엇을 보여주나 | 활용 |
|------|-----------------|------|
| **Layout Inspector** | 런타임 뷰 계층 트리, 각 뷰 속성 | 실제 중첩 깊이·불필요한 ViewGroup 식별 |
| **lint** | 정적 분석 경고 | 비효율 레이아웃 경고로 사전 차단 |
| **GPU 렌더링 프로파일** | 프레임별 렌더 시간 막대 | 측정/그리기 병목 시각화 |

**대표 lint 경고:**

- **`UselessParent`** — 자식이 하나뿐이고 배경/패딩 등 고유 기능이 없는 ViewGroup → 제거하거나 `<merge>`로 대체 권고.
- **`UselessLeaf`** — 자식도 없고 보이지도 않는 뷰 → 삭제 권고.

> 💡 권고 요약: ① 계층을 **평탄하게**(ConstraintLayout), ② 2-pass 컨테이너(weight LinearLayout/복잡 RelativeLayout) **중첩 피하기**, ③ 불필요한 부모는 `<merge>`/lint 권고로 제거, ④ Layout Inspector로 실측 깊이 확인.

근거: 레이아웃 계층 최적화 — https://developer.android.com/topic/performance/rendering/optimizing-view-hierarchies , Layout Inspector — https://developer.android.com/studio/debug/layout-inspector , lint 검사 — https://developer.android.com/studio/write/lint
