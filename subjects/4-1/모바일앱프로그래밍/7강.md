...
> <a id="ox-7"></a>
> **7. 정답**: O (전통적 Java 기준)
> **해설**: 전통적 Java에서 `findViewById`는 `View` 반환이라 캐스팅 필요. (Android Studio 2017+ 이후/API 26+에서는 제네릭 `<T extends View> T findViewById(int)`로 변경되어 캐스팅 생략 가능. 교재는 전통적 형태 기준 다룬다.)
