...
> 💡 **고정 폼 표** → GridLayout(같은 UI에서 TableLayout보다 빠르고 메모리도 적게 쓴다), **가변 흐름** → ConstraintLayout Flow, **스크롤 대량 격자** → `LazyVerticalGrid`(RecyclerView `GridLayoutManager`에 해당). TableLayout은 단순 고정 표 외에는 대안을 우선 고려한다.
