...
- [ ] **필수**: [7강. 레이아웃 3 (TableLayout · 레이아웃 중첩 · Java 속성 변경)](7강.md) - 커스텀 뷰/속성 변경 기반 완료? (※ 이벤트 처리는 7강이 아니라 [9강. 이벤트 처리 1](9강.md)에서 다룬다 — 강의 번호 혼동 주의)
- [ ] **권장**: 안드로이드 리소스 폴더 구조 (`res/raw`, `res/drawable`) 이해
- [ ] **권장**: 그래픽스의 기본 좌표계 (좌상단 (0,0), 픽셀 단위)
- **관련 단원**: [9강. 이벤트 처리 1](9강.md) (다음 강의)
...
2. 🔴 **Antialiasing(AntiAlias)**: 이미지·도형의 외곽선이 **곡선·사선(대각선)이거나 텍스트 경계**처럼 픽셀 격자와 어긋날 때 나타나는 계단식 모서리(jagged edge, 계단현상)를, 경계 픽셀을 배경과 부드럽게 섞어 매끄럽게 보이게 하는 렌더링 기술. (수직·수평선은 픽셀 격자에 딱 맞아 계단현상이 거의 없다.) Paint 객체의 `setAntiAlias(true)` 또는 생성자 플래그 `Paint.ANTI_ALIAS_FLAG`로 활성화한다.
...
4. 🔴 **SoundPool**: 음원 파일을 프로젝트의 `res/raw` 폴더에 미리 준비한 뒤, `load()` 메소드로 메모리에 올리고 `play()` 메소드로 재생하는 클래스. 짧은 효과음(게임 효과음, 버튼 클릭음 등)에 최적화되어 있다.
5. 🟡 **AudioManager**: 출력 장비와 오디오를 관리하는 역할을 하며, 별도의 효과음 파일을 준비하지 않고도 시스템에서 제공하는 오디오 파일을 사용할 수 있다는 장점이 있음. 비프음, 키톤 등 `playSoundEffect()`, `loadSoundEffects()` 등을 제공.
   ...
   > ⚠️ **실무 단서**: `playSoundEffect()`로 내는 소리는 **시스템 설정의 "터치음(touch sounds)"** 상태에 종속된다. 사용자가 설정에서 터치음을 끄면(내부적으로 `Settings.System.SOUND_EFFECTS_ENABLED = 0`) `playSoundEffect()`를 호출해도 **아무 소리가 나지 않는다** — 코드가 멀쩡한데 무음인 흔한 원인이다. 즉 이 효과음은 "내가 항상 울릴 수 있는 소리"가 아니라 "사용자 설정을 존중하는 시스템 효과음"이다(반드시 울려야 하는 소리는 `SoundPool` 등 직접 재생 경로를 쓴다). 근거: https://developer.android.com/reference/android/media/AudioManager#playSoundEffect(int)
6. 🟡 **Toast**: 짧은 시간 동안 화면에 잠시 떴다가 사라지는 간단한 메시지 출력 위젯. `Toast.makeText(context, text, duration).show()` 패턴으로 호출하며, 별도의 UI를 차지하지 않아 알림용으로 적합. `duration`은 `LENGTH_SHORT`/`LENGTH_LONG` 두 상수만 허용한다.
   > 📌 **시간 보충**: 흔히 `LENGTH_SHORT`≈2초, `LENGTH_LONG`≈3.5초로 소개되지만, 이 수치는 **플랫폼 내부 상수**(`Toast` 구현의 `SHORT_DURATION_TIMEOUT`/`LONG_DURATION_TIMEOUT`)에서 비롯된 근사값일 뿐이며, **공개 API가 밀리초(ms) 값을 보장하지는 않는다**. 따라서 정확한 표시 시간을 코드로 지정·의존할 수 없고(임의 ms가 필요하면 Snackbar 등 대안), OS 버전·접근성 설정에 따라 달라질 수 있다. 근거: https://developer.android.com/reference/android/widget/Toast#LENGTH_SHORT
...
| **SoundPool 생성** | `SoundPool sp = new SoundPool.Builder().setMaxStreams(5).build();` | 동시 재생 가능한 효과음 풀. ⚠️ `setMaxStreams`를 호출하지 않으면 **기본값이 1**이라 효과음이 동시에 1개만 재생되므로, 여러 음을 겹쳐 재생하려면 반드시 명시해야 한다 |
...
- **오개념 분석**:
    ...
    - **오개념 4**: "`SoundPool.load()` 직후 곧바로 `play()`를 호출하면 바로 소리가 난다."
    - **분석**: 틀렸다. `load()`는 **비동기**(음원을 백그라운드에서 메모리로 디코딩)이므로, 로드가 끝나기 전에 `play()`를 호출하면 **로드 미완료로 무음**이 되는 흔한 버그가 생긴다. `setOnLoadCompleteListener`를 등록해 콜백의 `status == 0`(성공)을 확인한 뒤 `play()`를 호출해야 안전하다. (아래 "코드로 확인하기"의 SoundPool 예제에 콜백 패턴이 구현되어 있다.) 근거: https://developer.android.com/reference/android/media/SoundPool
- **AI 학습 포인트**: "AntiAlias가 글꼴에는 효과가 없다는 주장이 맞는지, 안드로이드 Paint의 `ANTI_ALIAS_FLAG`와 `setSubpixelText()`/`setLinearText()` 차이를 들어 검증해 줘."

### 3. 🟢 심층 확장 — onDraw 성능 베스트 프랙티스 & MediaPlayer vs SoundPool

> 🟢 **2차 심층 확장**: 공식 문서로 검증한 보강이다. (1) Custom View `onDraw()`에서 객체 할당을 피해야 하는 이유, (2) 긴 음악/스트리밍 vs 짧은 효과음 다중재생의 도구 선택.

#### (1) Custom View `onDraw()` 성능 베스트 프랙티스 — 객체 할당 금지

`onDraw()`는 매 프레임(60fps라면 초당 60번, 한 프레임 ≈ 16.6ms 예산) 호출될 수 있다. **`onDraw()` 내부에서 `Paint`/`Path`/`Bitmap` 같은 객체를 `new`로 새로 생성하면** 짧은 시간에 대량의 객체가 young generation을 채워 **가비지 컬렉션(GC)** 을 유발한다. GC는 CPU 경합을 만들어 렌더링 작업을 지연시키고 **버벅임(jank)** 과 배터리 소모를 키운다. 따라서 `Paint` 등은 **생성자/`init()`에서 미리 멤버 변수로 생성**해 두고 `onDraw()`에서는 **재사용만** 해야 한다. 공식 가이드도 "onDraw()에서 할당을 제거하라(애니메이션 중에는 절대 할당 금지)"고 명시한다. 근거: https://developer.android.com/topic/performance/vitals/render · https://developer.android.com/training/custom-views/optimizing-view

```java
public class CircleView extends View {
    // ✅ 좋음: Paint를 멤버로 미리 한 번만 생성 (매 프레임 재사용)
    private final Paint mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    private final Path mPath = new Path();   // Path도 멤버로 미리 생성 후 reset()해서 재사용
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        // ❌ 나쁨(절대 금지): Paint p = new Paint(...);  // 매 프레임 새 객체 → GC 유발 → jank
        // ✅ 미리 만든 멤버를 재사용
        canvas.drawCircle(getWidth() / 2f, getHeight() / 2f, 100f, mPaint);

        mPath.reset();                       // 새로 만들지 않고 내용만 비워 재사용
        mPath.moveTo(0, 0);
...
> **요점**: "매 프레임 호출되는 코드에서는 새 객체를 만들지 않는다." `Paint`는 멤버로, `Path`는 멤버로 만든 뒤 `reset()`으로 재사용한다. 색·굵기 변경도 가급적 `onDraw` 밖(또는 상태가 바뀔 때만)에서 한다.

#### (2) MediaPlayer vs SoundPool — 사용처 비교

| 구분 | MediaPlayer | SoundPool |
| :--- | :--- | :--- |
| **주 용도** | **긴 오디오/동영상**, 네트워크 **스트리밍**, 배경 음악 | **짧은 효과음**(버튼 클릭음, 게임 SE), UI 피드백음 |
| **로딩/디코딩** | 재생 시 fetch·decode (스트리밍 가능, 큰 파일 OK) | 미리 메모리에 **PCM으로 압축 해제**해 로드 → 즉시 재생 |
| **첫 재생 지연** | 디코딩 때문에 상대적으로 큼 | 사전 로드되어 거의 **지연 없음** |
| **동시 재생(중첩)** | 보통 인스턴스당 1개 스트림 중심 | **여러 효과음 동시(overlap) 재생** 설계(`setMaxStreams`) |
| **파일 크기 권장** | 큰 파일도 가능(스트리밍) | **1MB 미만**의 짧은 음원 권장 |
| **언제** | 음악 플레이어, 동영상, 라디오/팟캐스트 스트리밍 | 게임 효과음, 클릭/알림 SE, 빠르고 반복적인 짧은 소리 |

> **선택 기준**: "**길고 1회성/스트리밍이면 MediaPlayer**, **짧고 반복·동시 재생이면 SoundPool**." SoundPool은 1MB 미만의 짧은 음원을 미리 로드해 반복·중첩 재생에 최적화돼 있고, MediaPlayer는 fetch·decode가 유연해 긴 오디오/비디오·네트워크 스트리밍에 적합하다. 근거: https://developer.android.com/reference/android/media/SoundPool · https://developer.android.com/media/platform/mediaplayer
...
| 표시/소멸 콜백 | **API 30(Android 11)부터 `Toast.addCallback(Toast.Callback)`** 으로 토스트가 **표시될 때(`onToastShown`)·사라질 때(`onToastHidden`)** 콜백을 받을 수 있다(여러 토스트의 표시 완료 시점 추적·체이닝에 유용). 같은 변경에서 백그라운드 앱의 **텍스트 토스트는 표시되지 않도록** 동작이 바뀐 점도 함께 알아 둔다 | developer.android.com/about/versions/11/behavior-changes-11 |
...
- 🟢 **안티앨리어싱 기본값 차이**: View 시스템에서는 `Paint`에 `ANTI_ALIAS_FLAG`/`setAntiAlias(true)`를 **명시해야** 외곽이 부드러워진다. 반면 Compose `Canvas`의 `DrawScope`에서 `drawCircle`/`drawPath` 등 도형 그리기 함수는 인자에 `Paint` 플래그를 따로 주지 않아도 **기본적으로 안티앨리어싱이 켜져(ON)** 있다(필요 시 `style`/`blendMode` 등으로 조정). 즉 "안티앨리어싱을 켜는 플래그를 깜빡해 외곽이 거칠어지는" 실수가 Compose에서는 기본값 덕분에 잘 발생하지 않는다.
- 근거: https://developer.android.com/develop/ui/compose/graphics/draw/overview · https://developer.android.com/reference/kotlin/androidx/compose/ui/graphics/drawscope/DrawScope#drawCircle(androidx.compose.ui.graphics.Color,kotlin.Float,androidx.compose.ui.geometry.Offset,kotlin.Float,androidx.compose.ui.graphics.drawscope.DrawStyle,androidx.compose.ui.graphics.ColorFilter,androidx.compose.ui.graphics.BlendMode)
